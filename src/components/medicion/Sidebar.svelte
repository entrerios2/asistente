<script lang="ts">
    ...
    async function loadDevices() {
        try {
            ...
            if (provider.listDevices) {
                ...
                inputDevices = devices.filter((d: any) => d.direction === 'input').map((d: any) => ({ id: d.id, name: d.name }));
                outputDevices = devices.filter((d: any) => d.direction === 'output').map((d: any) => ({ id: d.id, name: d.name }));
            } else if (navigator.mediaDevices && navigator.mediaDevices.enumerateDevices) {
                const devices = await navigator.mediaDevices.enumerateDevices();
                inputDevices = devices.filter(d => d.kind === 'audioinput').map(d => ({ id: d.deviceId || 'default', name: d.label || 'Micrófono del Sistema' }));
                outputDevices = devices.filter(d => d.kind === 'audiooutput').map(d => ({ id: d.deviceId || 'default', name: d.label || 'Altavoces del Sistema' }));
            }
        } catch (e) {
        ...
        if (inputDevices.length === 0) {
            inputDevices = [
                { id: 'in-default', name: 'Micrófono de Sistema (Predeterminado)' },
        ...
        if (outputDevices.length === 0) {
            outputDevices = [
                { id: 'out-default', name: 'Altavoces de Sistema (Predeterminado)' },
        ...
        if (stored) {
            try {
                ...
                if (config.isDarkMode !== undefined) {
                    ...
                    document.documentElement.classList.toggle('dark', config.isDarkMode);
                }
    ...
    function ensureMockSnapshots() {
        const existingSnaps = traceManager.traces.filter(t => t.type === 'snapshot');
        if (existingSnaps.length === 0) {
            ...
            for (let i = 0; i < dataLen; i++) {
                ...
                const ripple1 = 5 * Math.sin(Math.log2(f) * 2 * Math.PI) * Math.cos(f / 3000);
                const ripple2 = 8 * Math.cos(Math.log10(f) * 4 * Math.PI) + (f > 8000 ? -15 : 0);
                curve1[i] = Math.max(-120, Math.min(10, baseDb + ripple1 + (Math.random() * 1.5 - 0.75)));
                curve2[i] = Math.max(-120, Math.min(10, baseDb + ripple2 - 5 + (Math.random() * 2 - 1)));
            }
    ...
    function captureActiveLive() {
        ...
        if (liveTrace) {
            traceManager.captureSnapshot(liveTrace.id, `Instantánea #${traceManager.traces.filter(t => t.type === 'snapshot').length + 1}`, mode === 'manual' ? 'manual' : 'secuencial');
            ...
            for (let i = 0; i < dataLen; i++) {
                ...
                const ripple = 6 * Math.sin(Math.log10(f) * 3 * Math.PI) + Math.random() * 2;
...
<aside class="w-[380px] h-full bg-[#0a0a0c] border-r border-[#1a1a24]/50 flex flex-row text-gray-200 select-none">
    ...
    <div class="flex-1 h-full overflow-hidden flex flex-col bg-[#0a0a0c]">
        ...
        {:else if activeTab === 'snaps'}
            <div class="flex-1 p-5 overflow-y-auto flex flex-col gap-5" id="panel-snaps">
                <!-- Controles Superiores (Acciones Globales & Ordenamiento) -->
                <div class="flex flex-col gap-3.5 bg-[#121216]/40 border border-[#1a1a24]/50 rounded-xl p-4">
                    <div class="flex justify-between items-center">
                        <div class="flex items-center gap-1.5">
                            <span class="material-symbols-outlined text-[#a855f7] text-lg">photo_album</span>
                            <h3 class="text-xs font-bold text-gray-300 uppercase tracking-wider">Historial de Capturas</h3>
                        </div>
                        <span class="text-[10px] font-mono font-bold bg-[#a855f7]/15 text-[#a855f7] px-2 py-0.5 rounded-full">
                    ...
                    <div class="flex justify-between items-center gap-3 pt-2 border-t border-[#1a1a24]/20">
                        ...
                        <div class="flex bg-[#121216] p-0.5 rounded-md border border-[#1a1a24]/40">
                            <button 
                                class="px-2 py-1 text-[10px] font-semibold rounded transition-all cursor-pointer min-h-[24px]
                                       {sortOrder === 'desc' ? 'bg-[#a855f7] text-white shadow' : 'text-gray-400 hover:text-gray-200'}"
                            ...
                            <button 
                                class="px-2 py-1 text-[10px] font-semibold rounded transition-all cursor-pointer min-h-[24px]
                                       {sortOrder === 'asc' ? 'bg-[#a855f7] text-white shadow' : 'text-gray-400 hover:text-gray-200'}"
                ...
                <div class="flex-1 flex flex-col gap-3 min-h-0 overflow-y-auto pr-1">
                    {#if sortedSnapshots.length === 0}
                        <div class="flex-1 flex flex-col items-center justify-center p-8 text-center border border-dashed border-[#1a1a24] rounded-xl bg-[#121216]/5 gap-3">
                            <span class="material-symbols-outlined text-gray-600 text-3xl">add_photo_alternate</span>
                            <div class="flex flex-col gap-1">
                                <p class="text-xs text-gray-300 font-semibold">Sin instantáneas guardadas</p>
                                <p class="text-[10px] text-gray-500">Mida una señal y captúrela, o haga clic abajo para restaurar datos simulados de referencia.</p>
                            </div>
                            <button
                    ...
                    {:else}
                        <ul class="flex flex-col gap-3">
                            {#each sortedSnapshots as snap (snap.id)}
                                <li class="bg-[#121216]/30 border border-[#1a1a24]/50 rounded-xl p-3.5 flex flex-col gap-3 hover:border-gray-700/40 transition-all">
                                    <!-- Cabecera del Item: Nombre editable / ojito / borrar -->
                                    <div class="flex justify-between items-start gap-2">
                                        <div class="flex items-center gap-2 flex-1 min-w-0">
                                            <!-- Indicador de Color -->
                                            <span class="w-3 h-3 rounded-full flex-shrink-0" style="background-color: {snap.color}"></span>
                                        ...
                                        <div class="flex items-center gap-1">
                                            <button
                                                class="w-7 h-7 rounded flex items-center justify-center transition-all cursor-pointer min-h-[28px] min-w-[28px]
                                                       {snap.visible ? 'text-gray-300 hover:text-white hover:bg-white/5' : 'text-gray-600 hover:text-gray-400 hover:bg-white/5'}"
                                                onclick={() => snap.visible = !snap.visible}
                                                title={snap.visible ? 'Ocultar curva' : 'Mostrar curva'}
                                            >
                                            ...
                                            <button
                                                ...
                                                onclick={() => traceManager.removeTrace(snap.id)}
                                    ...
                                    <div class="flex items-center gap-3 text-[9px] text-gray-500 font-mono">
                                        ...
                                        <span class="flex items-center gap-1">
                                            <span class="material-symbols-outlined text-[11px]">{snap.source === 'manual' ? 'touch_app' : 'auto_mode'}</span>
                                            {snap.source === 'manual' ? 'Manual' : 'Secuencial'}
                                        </span>
                                        {#if snap.metric}
                                            <span class="bg-[#121216]/50 border border-white/5 px-1.5 py-0.5 rounded text-[8px] uppercase tracking-wide">
                                                {snap.metric}
                                            </span>
                                        {/if}
                                    </div>
                                    ...
                                    <div class="flex flex-col gap-1 border-t border-[#1a1a24]/30 pt-2.5 mt-0.5">
                                        <div class="flex justify-between items-center text-[9px] text-gray-400">
                                            <span class="font-medium">Compensación Y (Y-Offset)</span>
                                            <span 
                                                class="font-mono font-bold text-[#a855f7] cursor-pointer hover:text-white"
                                                ondblclick={() => snap.offsetY = 0}
                                                title="Doble clic para restablecer a 0 dB"
                                            >
                                                {snap.offsetY > 0 ? `+${snap.offsetY}` : snap.offsetY} dB
                                            </span>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            <span class="text-[8px] font-mono text-gray-600">-50</span>
                                            <input 
                                                type="range" 
                                                min="-50" 
                                                max="50" 
        ...
        {:else if activeTab === 'config'}
            <div class="flex-1 p-5 overflow-y-auto flex flex-col gap-5" id="panel-config">
                <!-- AUDIO HARDWARE CARD -->
                <div class="flex flex-col gap-4 bg-[#121216]/40 border border-[#1a1a24]/50 rounded-xl p-4">
                    <div class="flex items-center gap-2 border-b border-[#1a1a24]/30 pb-2">
                        <span class="material-symbols-outlined text-[#3b82f6] text-lg">speaker_group</span>
                        <h3 class="text-xs font-bold text-gray-300 uppercase tracking-wider">Hardware de Audio</h3>
                    </div>
                    ...
                    <div class="flex flex-col gap-1.5">
                        <label class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Canales de Entrada Activos</label>
                            ...
                            {#each [0, 1, 2, 3] as chIdx}
                                <button
                                    class="py-1.5 rounded-lg text-xs font-mono font-bold border transition-all cursor-pointer min-h-[36px]
                                           {uiStore.inChannels[chIdx] ? 'bg-[#3b82f6]/10 border-[#3b82f6] text-[#3b82f6] shadow' : 'bg-[#121216] border-[#1a1a24] text-gray-500 hover:text-gray-300 hover:bg-white/5'}"
                    ...
                    <div class="flex flex-col gap-1.5">
                        <label class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Canales de Salida Activos</label>
                        <div class="grid grid-cols-4 gap-2">
                            {#each [0, 1, 2, 3] as chIdx}
                                <button
                                    class="py-1.5 rounded-lg text-xs font-mono font-bold border transition-all cursor-pointer min-h-[36px]
                                           {uiStore.outChannels[chIdx] ? 'bg-[#3b82f6]/10 border-[#3b82f6] text-[#3b82f6] shadow' : 'bg-[#121216] border-[#1a1a24] text-gray-500 hover:text-gray-300 hover:bg-white/5'}"
                    ...
                    <div class="grid grid-cols-2 gap-3 pt-2 border-t border-[#1a1a24]/20 items-center">
                        <div class="flex flex-col gap-1.5">
                            ...
                            >
                                ...
                                <option value="Loopback">Loopback interno</option>
                            </select>
                        </div>

                        <div class="flex flex-col gap-1.5 justify-end h-full">
                            <label class="flex items-center gap-2 text-xs text-gray-300 cursor-pointer py-2">
                                <input 
                                    type="checkbox" 
                ...
                <div class="flex flex-col gap-4 bg-[#121216]/40 border border-[#1a1a24]/50 rounded-xl p-4">
                    <div class="flex items-center gap-2 border-b border-[#1a1a24]/30 pb-2">
                        <span class="material-symbols-outlined text-[#00ff88] text-lg">grid_view</span>
                        <h3 class="text-xs font-bold text-gray-300 uppercase tracking-wider">Pantalla y Preferencias</h3>
                    </div>

                    <!-- Grilla Predeterminada -->
                    <div class="flex flex-col gap-2">
                        <label class="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Distribución de Grilla por Defecto</label>
                            ...
                            {#each ['1x1', '1x2', '2x1', '2x2', '3x1', '3x2'] as layoutOpt}
                                <button
                                    class="flex flex-col items-center justify-center p-2 rounded-lg border transition-all cursor-pointer min-h-[56px] group
                                           {uiStore.layout === layoutOpt ? 'bg-[#00ff88]/10 border-[#00ff88] text-[#00ff88]' : 'bg-[#121216] border-[#1a1a24] text-gray-400 hover:text-gray-200'}"
                                    onclick={() => uiStore.setLayout(layoutOpt)}
                                >
                                    <!-- Representación miniatura ultra-precisa de la grilla [filas]x[columnas] -->
                                    <div class="grid gap-[2px] w-6 h-4 mb-1.5 opacity-60 group-hover:opacity-100 transition-opacity"
                                         style="grid-template-rows: repeat({parseInt(layoutOpt.split('x')[0])}, minmax(0, 1fr)); grid-template-columns: repeat({parseInt(layoutOpt.split('x')[1])}, minmax(0, 1fr));">
                                        {#each Array.from({ length: parseInt(layoutOpt.split('x')[0]) * parseInt(layoutOpt.split('x')[1]) }) as _}
                                            <div class="bg-current rounded-[1px] w-full h-full"></div>
                                        {/each}
                                    </div>
                                    <span class="text-[10px] font-mono font-bold">{layoutOpt}</span>
                                </button>
                            {/each}
                        </div>
                    </div>
                    ...
                    <div class="flex justify-between items-center pt-2 border-t border-[#1a1a24]/20">
                        <div class="flex flex-col gap-0.5">
                            ...
                            <span class="text-[10px] text-gray-500">Alterna entre Modo Oscuro y Claro</span>
                        </div>
                        ...
                        >
                            <span class="p-1 rounded-md flex items-center justify-center transition-all duration-200
                                         {!uiStore.isDarkMode ? 'bg-amber-500/10 text-amber-500 font-bold' : 'text-gray-500'}"
                                  title="Modo Claro"
                            >
                                <span class="material-symbols-outlined text-[16px]">light_mode</span>
                            </span>
                            <span class="p-1 rounded-md flex items-center justify-center transition-all duration-200
                                         {uiStore.isDarkMode ? 'bg-[#3b82f6]/10 text-[#3b82f6] font-bold' : 'text-gray-500'}"
                                  title="Modo Oscuro"
                            >
