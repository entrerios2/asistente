...
Como el FSK viaja acústicamente por el aire, **cualquier dispositivo corriendo la PWA** en la audiencia (ej. el celular de un espectador o un operador asistente) puede escuchar la secuencia de prueba que emite el PA y autocalibrarse. 
...
Se documentará el cambio de un sistema puramente local (IndexedDB) a un **Ecosistema Colaborativo respaldado por Google Apps Script (GAS)**. Esta integración potenciará todas las etapas del proyecto:
