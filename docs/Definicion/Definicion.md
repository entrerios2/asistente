...
- **3.2.3. Calibración:** Herramientas matemáticas de alineamiento DSP. La arquitectura visual y operativa de esta sección (modos Secuencial y Manual integrados, sistema de grilla multi-vista, gestor de snapshots unificado y estrategias responsivas) está formalmente documentada en su propio anexo: [`docs/UX_Medicion.md`](UX_Medicion.md). El operador avanzado utiliza estas herramientas e inyecta filtros manualmente. *Asistencia Optativa:* **Wizard APST** (flujo guiado de secuencias para análisis predictivo del recinto y sistema, §4.13), **AutoEq** (medición automatizada y sugerencia de filtros PEQ) y el **Asistente de Ring-Out** (herramienta de escucha pasiva de alta sensibilidad para detectar y sugerir atenuación de resonancias incipientes mientras el operador sube los faders de micrófonos vivos).
