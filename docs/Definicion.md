...
- **3.2.3. Calibración:** Herramientas matemáticas de alineamiento DSP. El operador avanzado utiliza el analizador de espectro (RTA) e inyecta filtros manualmente. *Asistencia Optativa:* **Wizard APST** (flujo guiado de secuencias para análisis predictivo del recinto y sistema, §4.13), **AutoEq** (medición automatizada y sugerencia de filtros PEQ) y el **Asistente de Ring-Out** (herramienta de escucha pasiva de alta sensibilidad para detectar y sugerir atenuación de resonancias incipientes mientras el operador sube los faders de micrófonos vivos).
