(function() {
	var uI = Object.defineProperty, aA = (g, I) => () => (g && (I = g(g = 0)), I), cg = (g, I) => {
		let A = {};
		for (var C in g) uI(A, C, {
			get: g[C],
			enumerable: !0
		});
		return I || uI(A, Symbol.toStringTag, { value: "Module" }), A;
	};
	typeof window < "u" && import("webfft").then((g) => {
		g && g.default && new g.default(8192);
	}).catch(() => {});
	function lg(g, I) {
		let A = 0;
		for (let C = 0; C < I; C++) A = A << 1 | g & 1, g >>= 1;
		return A;
	}
	function bI(g, I, A) {
		const C = g.length, B = Math.log2(C);
		for (let E = 0; E < C; E++) {
			const r = lg(E, B);
			if (r > E) {
				const o = g[E], t = I[E];
				g[E] = g[r], I[E] = I[r], g[r] = o, I[r] = t;
			}
		}
		for (let E = 2; E <= C; E <<= 1) {
			const r = E >> 1, o = (A ? 2 : -2) * Math.PI / E, t = Math.cos(o), a = Math.sin(o);
			for (let c = 0; c < C; c += E) {
				let w = 1, n = 0;
				for (let h = 0; h < r; h++) {
					const D = g[c + h], l = I[c + h], R = c + h + r, f = w * g[R] - n * I[R], N = w * I[R] + n * g[R];
					g[c + h] = D + f, I[c + h] = l + N, g[R] = D - f, I[R] = l - N;
					const M = w * t - n * a;
					n = w * a + n * t, w = M;
				}
			}
		}
		if (A) for (let E = 0; E < C; E++) g[E] /= C, I[E] /= C;
	}
	function JI(g, I, A) {
		const C = g.length, B = I || new Float32Array(C), E = A || new Float32Array(C);
		return B.set(g), E.fill(0), bI(B, E, !1), {
			real: B,
			imag: E
		};
	}
	function fg(g, I, A, C) {
		const B = g.length, E = A || new Float32Array(B), r = C || new Float32Array(B);
		return E.set(g), r.set(I), bI(E, r, !0), E;
	}
	function Fg(g, I, A, C, B, E, r) {
		const o = B.length;
		for (let t = 0; t < o; t++) {
			const a = A[t] * A[t] + C[t] * C[t] + 1e-12, c = (g[t] * A[t] + I[t] * C[t]) / a, w = (I[t] * A[t] - g[t] * C[t]) / a;
			E && (E[t] = c), r && (r[t] = w);
			const n = Math.sqrt(c * c + w * w);
			B[t] = 20 * Math.log10(n + 1e-8);
		}
	}
	function Rg(g, I, A, C, B) {
		const E = B.length;
		for (let r = 0; r < E; r++) {
			const o = A[r] * A[r] + C[r] * C[r] + 1e-12, t = (g[r] * A[r] + I[r] * C[r]) / o, a = (I[r] * A[r] - g[r] * C[r]) / o;
			B[r] = Math.atan2(a, t) * (180 / Math.PI);
		}
	}
	function Ng(g, I, A = 48e3) {
		let C = 0;
		const B = g.length;
		for (let E = 0; E < B; E++) C += g[E], I[E] = C;
	}
	function yg(g, I, A) {
		const C = A.length;
		A[0] = 0;
		const B = 2 * Math.PI * I;
		for (let E = 1; E < C; E++) {
			let r = g[E] - g[E - 1];
			for (; r > Math.PI;) r -= 2 * Math.PI;
			for (; r < -Math.PI;) r += 2 * Math.PI;
			A[E] = -r / B * 1e3;
		}
	}
	function LI(g) {
		let I = 0, A = 0;
		const C = g.length;
		for (let B = 0; B < C; B++) {
			const E = Math.abs(g[B]);
			E > I && (I = E), A += g[B] * g[B];
		}
		return {
			peakDb: 20 * Math.log10(I + 1e-9),
			rmsDb: 20 * Math.log10(Math.sqrt(A / Math.max(1, C)) + 1e-9)
		};
	}
	var Mg = class {
		depth;
		bins;
		bufferReal;
		bufferImag;
		writeIdx = 0;
		count = 0;
		lpfReal;
		lpfImag;
		lastValidReal;
		lastValidImag;
		constructor(g, I = 16) {
			this.bins = g, this.depth = I, this.bufferReal = Array.from({ length: I }, () => new Float32Array(g)), this.bufferImag = Array.from({ length: I }, () => new Float32Array(g)), this.lpfReal = new Float32Array(g), this.lpfImag = new Float32Array(g), this.lastValidReal = new Float32Array(g), this.lastValidImag = new Float32Array(g);
		}
		processFIFO(g, I, A, C, B) {
			if (B !== void 0 && B > -120) for (let E = 0; E < this.bins; E++) {
				const r = Math.sqrt(g[E] * g[E] + I[E] * I[E]);
				20 * Math.log10(r + 1e-12) < B ? (g[E] = this.lastValidReal[E], I[E] = this.lastValidImag[E]) : (this.lastValidReal[E] = g[E], this.lastValidImag[E] = I[E]);
			}
			this.bufferReal[this.writeIdx].set(g), this.bufferImag[this.writeIdx].set(I), this.writeIdx = (this.writeIdx + 1) % this.depth, this.count < this.depth && this.count++, A.fill(0), C.fill(0);
			for (let E = 0; E < this.count; E++) for (let r = 0; r < this.bins; r++) A[r] += this.bufferReal[E][r], C[r] += this.bufferImag[E][r];
			for (let E = 0; E < this.bins; E++) A[E] /= this.count, C[E] /= this.count;
		}
		processLPF(g, I, A, C, B) {
			for (let E = 0; E < this.bins; E++) this.lpfReal[E] += (g[E] - this.lpfReal[E]) * B, this.lpfImag[E] += (I[E] - this.lpfImag[E]) * B, A[E] = this.lpfReal[E], C[E] = this.lpfImag[E];
		}
		setDepth(g) {
			g !== this.depth && (this.depth = Math.max(1, Math.min(64, g)), this.bufferReal = Array.from({ length: this.depth }, () => new Float32Array(this.bins)), this.bufferImag = Array.from({ length: this.depth }, () => new Float32Array(this.bins)), this.lastValidReal = new Float32Array(this.bins), this.lastValidImag = new Float32Array(this.bins), this.writeIdx = 0, this.count = 0);
		}
		reset() {
			this.writeIdx = 0, this.count = 0, this.lpfReal.fill(0), this.lpfImag.fill(0), this.lastValidReal.fill(0), this.lastValidImag.fill(0);
		}
	};
	function Gg(g, I, A, C, B, E, r, o, t) {
		const a = g.length, c = a * 2, w = 1e-10;
		for (let n = 0; n < a; n++) {
			const h = A[n] * A[n] + C[n] * C[n] + w, D = (g[n] * A[n] + I[n] * C[n]) / h, l = (I[n] * A[n] - g[n] * C[n]) / h;
			E[n] = D, r[n] = l;
		}
		for (let n = 1; n < a; n++) E[c - n] = E[n], r[c - n] = -r[n];
		fg(E, r, o, t), B.set(o);
	}
	function kg(g, I, A, C = 48e3) {
		const B = g.length, E = Math.round(A / 1e3 * C), r = Math.round(I / 2 / 1e3 * C), o = Math.max(0, E - r), t = Math.min(B - 1, E + r), a = Math.round(r * .2);
		for (let c = 0; c < B; c++) if (c < o || c > t) g[c] = 0;
		else if (c < o + a) {
			const w = (c - o) / a, n = .5 * (1 - Math.cos(w * Math.PI));
			g[c] *= n;
		} else if (c > t - a) {
			const w = (t - c) / a, n = .5 * (1 - Math.cos(w * Math.PI));
			g[c] *= n;
		}
	}
	var Yg = class {
		cache = {};
		getWindow(g, I) {
			const A = `${g}_${I}`;
			if (!this.cache[A]) {
				const C = new Float32Array(g);
				let B = 0, E = 0;
				for (let o = 0; o < g; o++) {
					let t = 1;
					const a = 2 * Math.PI * o / (g - 1);
					if (I === "Hann") t = .5 * (1 - Math.cos(a));
					else if (I === "Hamming") t = .54 - .46 * Math.cos(a);
					else if (I === "FlatTop") t = 1 - 1.93 * Math.cos(a) + 1.29 * Math.cos(2 * a) - .388 * Math.cos(3 * a) + .0322 * Math.cos(4 * a);
					else if (I === "BlackmanHarris") t = .35875 - .48829 * Math.cos(a) + .14128 * Math.cos(2 * a) - .01168 * Math.cos(3 * a);
					else if (I === "HFT223D") t = 1 - 1.98298997309 * Math.cos(a) + 1.75556083063 * Math.cos(2 * a) - 1.19037717712 * Math.cos(3 * a) + .56155440797 * Math.cos(4 * a) - .17296769663 * Math.cos(5 * a) + .03233247087 * Math.cos(6 * a) - .00324954578 * Math.cos(7 * a) + .0001380104 * Math.cos(8 * a) - 132725e-11 * Math.cos(9 * a);
					else if (I === "Exponential") {
						const c = g / 5;
						t = Math.exp(-o / c);
					}
					C[o] = t, B += t, E += t * t;
				}
				const r = B / g;
				for (let o = 0; o < g; o++) C[o] /= r;
				this.cache[A] = C;
			}
			return this.cache[A];
		}
		apply(g, I) {
			if (I === "Rectangular") return;
			const A = g.length, C = this.getWindow(A, I);
			for (let B = 0; B < A; B++) g[B] *= C[B];
		}
	}, sI = class {
		b0;
		b1;
		b2;
		a1;
		a2;
		z1 = 0;
		z2 = 0;
		constructor(g, I, A, C, B, E) {
			this.b0 = g / C, this.b1 = I / C, this.b2 = A / C, this.a1 = B / C, this.a2 = E / C;
		}
		process(g) {
			for (let I = 0; I < g.length; I++) {
				const A = g[I], C = this.b0 * A + this.z1;
				this.z1 = this.b1 * A - this.a1 * C + this.z2, this.z2 = this.b2 * A - this.a2 * C, g[I] = C;
			}
		}
		reset() {
			this.z1 = 0, this.z2 = 0;
		}
	};
	function dg(g, I, A) {
		const C = 2 * Math.PI * g / A, B = Math.sin(C) / (2 * I);
		return new sI(1, -2 * Math.cos(C), 1, 1 + B, -2 * Math.cos(C), 1 - B);
	}
	function Sg(g, I, A) {
		const C = 2 * Math.PI * g / A, B = Math.sin(C) / (2 * I);
		return new sI(B, 0, -B, 1 + B, -2 * Math.cos(C), 1 - B);
	}
	function Ug(g, I, A) {
		const C = 2 * Math.PI * g / A, B = Math.sin(C) / (2 * I), E = Math.cos(C);
		return new sI((1 - E) / 2, 1 - E, (1 - E) / 2, 1 + B, -2 * E, 1 - B);
	}
	function KI(g, I) {
		switch (g) {
			case "Notch1k": return dg(1e3, 10, I);
			case "BP100": return Sg(100, 1, I);
			case "LP200": return Ug(200, .7071, I);
			default: return null;
		}
	}
	const Hg = {
		Slow: {
			gain: 132731.3202,
			k: [
				.4600089841,
				-2.6653917847,
				6.200654795,
				-7.2408808951,
				4.2453678122
			]
		},
		Medium: {
			gain: 5908.173436,
			k: [
				.2116396822,
				-1.3993115731,
				3.752522757,
				-5.1097576527,
				3.5394905611
			]
		},
		Fast: {
			gain: 350.8023803,
			k: [
				.0448577871,
				-.3690099172,
				1.271946008,
				-2.321921842,
				2.2829085146
			]
		}
	};
	var _A = class {
		gain;
		k;
		x = [
			0,
			0,
			0,
			0,
			0,
			0
		];
		y = [
			0,
			0,
			0,
			0,
			0,
			0
		];
		p = 3;
		constructor(g) {
			const I = Hg[g];
			this.gain = I.gain, this.k = I.k;
		}
		ptr(g) {
			let I = this.p + g;
			return I > 5 && (I -= 6), I;
		}
		process(g) {
			if (g !== g) return this.y[this.ptr(5)];
			this.p = this.ptr(1);
			const I = this.ptr(5);
			return this.x[I] = g / this.gain, this.y[I] = this.x[this.ptr(0)] * 1 + this.x[this.ptr(1)] * 5 + this.x[this.ptr(2)] * 10 + this.x[this.ptr(3)] * 10 + this.x[this.ptr(4)] * 5 + this.x[I] * 1 + this.y[this.ptr(0)] * this.k[0] + this.y[this.ptr(1)] * this.k[1] + this.y[this.ptr(2)] * this.k[2] + this.y[this.ptr(3)] * this.k[3] + this.y[this.ptr(4)] * this.k[4], this.y[I];
		}
		reset() {
			this.x.fill(0), this.y.fill(0);
		}
	}, vg = class {
		filtersReal;
		filtersImag;
		bins;
		currentFreq;
		constructor(g, I = "Medium") {
			this.bins = g, this.currentFreq = I, this.filtersReal = Array.from({ length: g }, () => new _A(I)), this.filtersImag = Array.from({ length: g }, () => new _A(I));
		}
		setFrequency(g) {
			g !== this.currentFreq && (this.currentFreq = g, this.filtersReal = Array.from({ length: this.bins }, () => new _A(g)), this.filtersImag = Array.from({ length: this.bins }, () => new _A(g)));
		}
		process(g, I, A, C) {
			for (let B = 0; B < this.bins; B++) A[B] = this.filtersReal[B].process(g[B]), C[B] = this.filtersImag[B].process(I[B]);
		}
		reset() {
			for (let g = 0; g < this.bins; g++) this.filtersReal[g].reset(), this.filtersImag[g].reset();
		}
	};
	let $A = null, AI = null, II = 0;
	function mg(g) {
		return (!$A || II !== g) && ($A = new Float32Array(g), AI = new Float32Array(g), II = g), $A;
	}
	function ug(g) {
		return (!AI || II !== g) && ($A = new Float32Array(g), AI = new Float32Array(g), II = g), AI;
	}
	function DI(g, I, A, C) {
		if (C <= 0 || I <= 0) return;
		const B = A / 2 / I, E = Math.pow(2, 1 / (2 * C)), r = mg(I);
		for (let o = 1; o < I; o++) {
			const t = o * B, a = t / E, c = t * E, w = Math.max(1, Math.floor(a / B)), n = Math.min(I - 1, Math.ceil(c / B));
			let h = 0;
			const D = n - w + 1;
			for (let l = w; l <= n; l++) h += g[l];
			r[o] = h / D;
		}
		r[0] = g[0], g.set(r);
	}
	function bg(g, I, A, C) {
		if (C <= 0 || I <= 0) return;
		const B = A / 2 / I, E = Math.pow(2, 1 / (2 * C)), r = Math.PI / 180, o = 180 / Math.PI, t = ug(I);
		for (let a = 1; a < I; a++) {
			const c = a * B, w = c / E, n = c * E, h = Math.max(1, Math.floor(w / B)), D = Math.min(I - 1, Math.ceil(n / B));
			let l = 0, R = 0;
			for (let f = h; f <= D; f++) {
				const N = g[f] * r;
				l += Math.sin(N), R += Math.cos(N);
			}
			t[a] = Math.atan2(l, R) * o;
		}
		t[0] = g[0], g.set(t);
	}
	var qI, Jg = aA((() => {
		qI = (() => {
			var g = self.location.href;
			return (function(I = {}) {
				var A = I, C, B;
				A.ready = new Promise((Q, e) => {
					C = Q, B = e;
				});
				var E = Object.assign({}, A), r = !0, o = !1, t = "";
				function a(Q) {
					return A.locateFile ? A.locateFile(Q, t) : t + Q;
				}
				var c;
				(r || o) && (o ? t = self.location.href : typeof document < "u" && document.currentScript && (t = document.currentScript.src), g && (t = g), t.indexOf("blob:") !== 0 ? t = t.substr(0, t.replace(/[?#].*/, "").lastIndexOf("/") + 1) : t = "", o && (c = (Q) => {
					var e = new XMLHttpRequest();
					return e.open("GET", Q, !1), e.responseType = "arraybuffer", e.send(null), new Uint8Array(e.response);
				})), A.print || console.log.bind(console);
				var w = A.printErr || console.error.bind(console);
				Object.assign(A, E), E = null, A.arguments && A.arguments, A.thisProgram && A.thisProgram, A.quit && A.quit;
				var n;
				A.wasmBinary && (n = A.wasmBinary), A.noExitRuntime, typeof WebAssembly != "object" && j("no native wasm support detected");
				var h, D, l = !1, R, f;
				function N() {
					var Q = h.buffer;
					A.HEAP8 = R = new Int8Array(Q), A.HEAP16 = new Int16Array(Q), A.HEAP32 = new Int32Array(Q), A.HEAPU8 = f = new Uint8Array(Q), A.HEAPU16 = new Uint16Array(Q), A.HEAPU32 = new Uint32Array(Q), A.HEAPF32 = new Float32Array(Q), A.HEAPF64 = new Float64Array(Q);
				}
				var M = [], d = [], b = [];
				function K() {
					if (A.preRun) for (typeof A.preRun == "function" && (A.preRun = [A.preRun]); A.preRun.length;) _(A.preRun.shift());
					x(M);
				}
				function W() {
					x(d);
				}
				function T() {
					if (A.postRun) for (typeof A.postRun == "function" && (A.postRun = [A.postRun]); A.postRun.length;) p(A.postRun.shift());
					x(b);
				}
				function _(Q) {
					M.unshift(Q);
				}
				function q(Q) {
					d.unshift(Q);
				}
				function p(Q) {
					b.unshift(Q);
				}
				var Y = 0, H = null, u = null;
				function IA(Q) {
					Y++, A.monitorRunDependencies && A.monitorRunDependencies(Y);
				}
				function O(Q) {
					if (Y--, A.monitorRunDependencies && A.monitorRunDependencies(Y), Y == 0 && (H !== null && (clearInterval(H), H = null), u)) {
						var e = u;
						u = null, e();
					}
				}
				function j(Q) {
					A.onAbort && A.onAbort(Q), Q = "Aborted(" + Q + ")", w(Q), l = !0, Q += ". Build with -sASSERTIONS for more info.";
					var e = new WebAssembly.RuntimeError(Q);
					throw B(e), e;
				}
				var $ = "data:application/octet-stream;base64,";
				function AA(Q) {
					return Q.startsWith($);
				}
				var J = "data:application/octet-stream;base64,";
				AA(J) || (J = a(J));
				function tA(Q) {
					if (Q == J && n) return new Uint8Array(n);
					var e = cA(Q);
					if (e) return e;
					if (c) return c(Q);
					throw "sync fetching of the wasm failed: you can preload it to Module['wasmBinary'] manually, or emcc.py will do that for you when generating HTML (but not JS)";
				}
				function QA(Q, e) {
					var s, k = tA(Q);
					return s = new WebAssembly.Module(k), [new WebAssembly.Instance(s, e), s];
				}
				function eA() {
					var Q = { a: m };
					function e(s, k) {
						var S = s.exports;
						return D = S, h = D.c, N(), D.j, q(D.d), O("wasm-instantiate"), S;
					}
					if (IA("wasm-instantiate"), A.instantiateWasm) try {
						return A.instantiateWasm(Q, e);
					} catch (s) {
						w("Module.instantiateWasm callback failed with error: " + s), B(s);
					}
					return e(QA(J, Q)[0]);
				}
				var x = (Q) => {
					for (; Q.length > 0;) Q.shift()(A);
				}, CA = (Q, e, s) => f.copyWithin(Q, e, e + s), EA = (Q) => {
					j("OOM");
				}, gA = (Q) => {
					f.length, Q >>>= 0, EA(Q);
				};
				function BA(Q) {
					return A["_" + Q];
				}
				var iA = (Q, e) => {
					R.set(Q, e);
				}, nA = (Q) => {
					for (var e = 0, s = 0; s < Q.length; ++s) {
						var k = Q.charCodeAt(s);
						k <= 127 ? e++ : k <= 2047 ? e += 2 : k >= 55296 && k <= 57343 ? (e += 4, ++s) : e += 3;
					}
					return e;
				}, oA = (Q, e, s, k) => {
					if (!(k > 0)) return 0;
					for (var S = s, G = s + k - 1, F = 0; F < Q.length; ++F) {
						var v = Q.charCodeAt(F);
						if (v >= 55296 && v <= 57343) {
							var X = Q.charCodeAt(++F);
							v = 65536 + ((v & 1023) << 10) | X & 1023;
						}
						if (v <= 127) {
							if (s >= G) break;
							e[s++] = v;
						} else if (v <= 2047) {
							if (s + 1 >= G) break;
							e[s++] = 192 | v >> 6, e[s++] = 128 | v & 63;
						} else if (v <= 65535) {
							if (s + 2 >= G) break;
							e[s++] = 224 | v >> 12, e[s++] = 128 | v >> 6 & 63, e[s++] = 128 | v & 63;
						} else {
							if (s + 3 >= G) break;
							e[s++] = 240 | v >> 18, e[s++] = 128 | v >> 12 & 63, e[s++] = 128 | v >> 6 & 63, e[s++] = 128 | v & 63;
						}
					}
					return e[s] = 0, s - S;
				}, sA = (Q, e, s) => oA(Q, f, e, s), DA = (Q) => {
					var e = nA(Q) + 1, s = mA(e);
					return sA(Q, s, e), s;
				}, wA = typeof TextDecoder < "u" ? new TextDecoder("utf8") : void 0, y = (Q, e, s) => {
					for (var k = e + s, S = e; Q[S] && !(S >= k);) ++S;
					if (S - e > 16 && Q.buffer && wA) return wA.decode(Q.subarray(e, S));
					for (var G = ""; e < S;) {
						var F = Q[e++];
						if (!(F & 128)) {
							G += String.fromCharCode(F);
							continue;
						}
						var v = Q[e++] & 63;
						if ((F & 224) == 192) {
							G += String.fromCharCode((F & 31) << 6 | v);
							continue;
						}
						var X = Q[e++] & 63;
						if ((F & 240) == 224 ? F = (F & 15) << 12 | v << 6 | X : F = (F & 7) << 18 | v << 12 | X << 6 | Q[e++] & 63, F < 65536) G += String.fromCharCode(F);
						else {
							var Z = F - 65536;
							G += String.fromCharCode(55296 | Z >> 10, 56320 | Z & 1023);
						}
					}
					return G;
				}, U = (Q, e) => Q ? y(f, Q, e) : "", rA = function(Q, e, s, k, S) {
					var G = {
						string: (z) => {
							var zA = 0;
							return z != null && z !== 0 && (zA = DA(z)), zA;
						},
						array: (z) => {
							var zA = mA(z.length);
							return iA(z, zA), zA;
						}
					};
					function F(z) {
						return e === "string" ? U(z) : e === "boolean" ? !!z : z;
					}
					var v = BA(Q), X = [], Z = 0;
					if (k) for (var FA = 0; FA < k.length; FA++) {
						var MA = G[s[FA]];
						MA ? (Z === 0 && (Z = bA()), X[FA] = MA(k[FA])) : X[FA] = k[FA];
					}
					var LA = v.apply(null, X);
					function L(z) {
						return Z !== 0 && kA(Z), F(z);
					}
					return LA = L(LA), LA;
				}, hA = function(Q, e, s, k) {
					var S = !s || s.every((G) => G === "number" || G === "boolean");
					return e !== "string" && S && !k ? BA(Q) : function() {
						return rA(Q, e, s, arguments, k);
					};
				}, m = {
					b: CA,
					a: gA
				}, V = eA();
				V.d, A._kiss_fft_free = V.e, A._free = V.f, A._kiss_fft_alloc = V.g, A._malloc = V.h, A._kiss_fft = V.i, V.__errno_location;
				var bA = V.k, kA = V.l, mA = V.m;
				function JA(Q) {
					try {
						for (var e = atob(Q), s = new Uint8Array(e.length), k = 0; k < e.length; ++k) s[k] = e.charCodeAt(k);
						return s;
					} catch {
						throw new Error("Converting base64 string to bytes failed.");
					}
				}
				function cA(Q) {
					if (AA(Q)) return JA(Q.slice($.length));
				}
				A.ccall = rA, A.cwrap = hA;
				var RA;
				u = function Q() {
					RA || i(), RA || (u = Q);
				};
				function i() {
					if (Y > 0 || (K(), Y > 0)) return;
					function Q() {
						RA || (RA = !0, A.calledRun = !0, !l && (W(), C(A), A.onRuntimeInitialized && A.onRuntimeInitialized(), T()));
					}
					A.setStatus ? (A.setStatus("Running..."), setTimeout(function() {
						setTimeout(function() {
							A.setStatus("");
						}, 1), Q();
					}, 1)) : Q();
				}
				if (A.preInit) for (typeof A.preInit == "function" && (A.preInit = [A.preInit]); A.preInit.length > 0;) A.preInit.pop()();
				return i(), I;
			});
		})();
	})), dA, hI, pI, cI, TI, Lg = aA((() => {
		Jg(), dA = qI({}), hI = dA.cwrap("kiss_fft_alloc", "number", [
			"number",
			"number",
			"number",
			"number"
		]), pI = dA.cwrap("kiss_fft", "void", [
			"number",
			"number",
			"number"
		]), cI = dA.cwrap("kiss_fft_free", "void", ["number"]), TI = class {
			constructor(g) {
				this.size = g, this.fcfg = hI(this.size, !1), this.icfg = hI(this.size, !0), this.inptr = dA._malloc(this.size * 8), this.cin = new Float32Array(dA.HEAPU8.buffer, this.inptr, this.size * 2);
			}
			fft = function(g) {
				const I = dA._malloc(this.size * 8), A = new Float32Array(dA.HEAPU8.buffer, I, this.size * 2);
				this.cin.set(g), pI(this.fcfg, this.inptr, I);
				let C = new Float32Array(this.size * 2);
				return C.set(A), dA._free(I), C;
			};
			dispose() {
				cI(this.fcfg), cI(this.icfg), dA._free(this.inptr);
			}
		};
	}));
	function NA(g) {
		if (this.size = g | 0, this.size <= 1 || (this.size & this.size - 1) !== 0) throw new Error("FFT size must be a power of two and bigger than 1");
		this._csize = g << 1;
		for (var I = new Array(this.size * 2), A = 0; A < I.length; A += 2) {
			const t = Math.PI * A / this.size;
			I[A] = Math.cos(t), I[A + 1] = -Math.sin(t);
		}
		this.table = I;
		for (var C = 0, B = 1; this.size > B; B <<= 1) C++;
		this._width = C % 2 === 0 ? C - 1 : C, this._bitrev = new Array(1 << this._width);
		for (var E = 0; E < this._bitrev.length; E++) {
			this._bitrev[E] = 0;
			for (var r = 0; r < this._width; r += 2) {
				var o = this._width - r - 2;
				this._bitrev[E] |= (E >>> r & 3) << o;
			}
		}
		this._out = null, this._data = null, this._inv = 0;
	}
	var Kg = aA((() => {
		NA.prototype.fromComplexArray = function(I, A) {
			for (var C = A || new Array(I.length >>> 1), B = 0; B < I.length; B += 2) C[B >>> 1] = I[B];
			return C;
		}, NA.prototype.createComplexArray = function() {
			const I = new Array(this._csize);
			for (var A = 0; A < I.length; A++) I[A] = 0;
			return I;
		}, NA.prototype.toComplexArray = function(I, A) {
			for (var C = A || this.createComplexArray(), B = 0; B < C.length; B += 2) C[B] = I[B >>> 1], C[B + 1] = 0;
			return C;
		}, NA.prototype.completeSpectrum = function(I) {
			for (var A = this._csize, C = A >>> 1, B = 2; B < C; B += 2) I[A - B] = I[B], I[A - B + 1] = -I[B + 1];
		}, NA.prototype.transform = function(I, A) {
			if (I === A) throw new Error("Input and output buffers must be different");
			this._out = I, this._data = A, this._inv = 0, this._transform4(), this._out = null, this._data = null;
		}, NA.prototype.realTransform = function(I, A) {
			if (I === A) throw new Error("Input and output buffers must be different");
			this._out = I, this._data = A, this._inv = 0, this._realTransform4(), this._out = null, this._data = null;
		}, NA.prototype.inverseTransform = function(I, A) {
			if (I === A) throw new Error("Input and output buffers must be different");
			this._out = I, this._data = A, this._inv = 1, this._transform4();
			for (var C = 0; C < I.length; C++) I[C] /= this.size;
			this._out = null, this._data = null;
		}, NA.prototype._transform4 = function() {
			var I = this._out, A = this._csize, C = 1 << this._width, B = A / C << 1, E, r, o = this._bitrev;
			if (B === 4) for (E = 0, r = 0; E < A; E += B, r++) {
				const D = o[r];
				this._singleTransform2(E, D, C);
			}
			else for (E = 0, r = 0; E < A; E += B, r++) {
				const D = o[r];
				this._singleTransform4(E, D, C);
			}
			var t = this._inv ? -1 : 1, a = this.table;
			for (C >>= 2; C >= 2; C >>= 2) {
				B = A / C << 1;
				var c = B >>> 2;
				for (E = 0; E < A; E += B) for (var w = E + c, n = E, h = 0; n < w; n += 2, h += C) {
					const D = n, l = D + c, R = l + c, f = R + c, N = I[D], M = I[D + 1], d = I[l], b = I[l + 1], K = I[R], W = I[R + 1], T = I[f], _ = I[f + 1], q = N, p = M, Y = a[h], H = t * a[h + 1], u = d * Y - b * H, IA = d * H + b * Y, O = a[2 * h], j = t * a[2 * h + 1], $ = K * O - W * j, AA = K * j + W * O, J = a[3 * h], tA = t * a[3 * h + 1], QA = T * J - _ * tA, eA = T * tA + _ * J, x = q + $, CA = p + AA, EA = q - $, gA = p - AA, BA = u + QA, iA = IA + eA, nA = t * (u - QA), oA = t * (IA - eA), sA = x + BA, DA = CA + iA, wA = x - BA, y = CA - iA, U = EA + oA, rA = gA - nA, hA = EA - oA, m = gA + nA;
					I[D] = sA, I[D + 1] = DA, I[l] = U, I[l + 1] = rA, I[R] = wA, I[R + 1] = y, I[f] = hA, I[f + 1] = m;
				}
			}
		}, NA.prototype._singleTransform2 = function(I, A, C) {
			const B = this._out, E = this._data, r = E[A], o = E[A + 1], t = E[A + C], a = E[A + C + 1], c = r + t, w = o + a, n = r - t, h = o - a;
			B[I] = c, B[I + 1] = w, B[I + 2] = n, B[I + 3] = h;
		}, NA.prototype._singleTransform4 = function(I, A, C) {
			const B = this._out, E = this._data, r = this._inv ? -1 : 1, o = C * 2, t = C * 3, a = E[A], c = E[A + 1], w = E[A + C], n = E[A + C + 1], h = E[A + o], D = E[A + o + 1], l = E[A + t], R = E[A + t + 1], f = a + h, N = c + D, M = a - h, d = c - D, b = w + l, K = n + R, W = r * (w - l), T = r * (n - R), _ = f + b, q = N + K, p = M + T, Y = d - W, H = f - b, u = N - K, IA = M - T, O = d + W;
			B[I] = _, B[I + 1] = q, B[I + 2] = p, B[I + 3] = Y, B[I + 4] = H, B[I + 5] = u, B[I + 6] = IA, B[I + 7] = O;
		}, NA.prototype._realTransform4 = function() {
			var I = this._out, A = this._csize, C = 1 << this._width, B = A / C << 1, E, r, o = this._bitrev;
			if (B === 4) for (E = 0, r = 0; E < A; E += B, r++) {
				const G = o[r];
				this._singleRealTransform2(E, G >>> 1, C >>> 1);
			}
			else for (E = 0, r = 0; E < A; E += B, r++) {
				const G = o[r];
				this._singleRealTransform4(E, G >>> 1, C >>> 1);
			}
			var t = this._inv ? -1 : 1, a = this.table;
			for (C >>= 2; C >= 2; C >>= 2) {
				B = A / C << 1;
				var c = B >>> 1, w = c >>> 1, n = w >>> 1;
				for (E = 0; E < A; E += B) for (var h = 0, D = 0; h <= n; h += 2, D += C) {
					var l = E + h, R = l + w, f = R + w, N = f + w, M = I[l], d = I[l + 1], b = I[R], K = I[R + 1], W = I[f], T = I[f + 1], _ = I[N], q = I[N + 1], p = M, Y = d, H = a[D], u = t * a[D + 1], IA = b * H - K * u, O = b * u + K * H, j = a[2 * D], $ = t * a[2 * D + 1], AA = W * j - T * $, J = W * $ + T * j, tA = a[3 * D], QA = t * a[3 * D + 1], eA = _ * tA - q * QA, x = _ * QA + q * tA, CA = p + AA, EA = Y + J, gA = p - AA, BA = Y - J, iA = IA + eA, nA = O + x, oA = t * (IA - eA), sA = t * (O - x), DA = CA + iA, wA = EA + nA, y = gA + sA, U = BA - oA;
					if (I[l] = DA, I[l + 1] = wA, I[R] = y, I[R + 1] = U, h === 0) {
						var rA = CA - iA, hA = EA - nA;
						I[f] = rA, I[f + 1] = hA;
						continue;
					}
					if (h !== n) {
						var m = gA, V = -BA, bA = CA, kA = -EA, mA = -t * sA, JA = -t * oA, cA = -t * nA, RA = -t * iA, i = m + mA, Q = V + JA, e = bA + RA, s = kA - cA, k = E + w - h, S = E + c - h;
						I[k] = i, I[k + 1] = Q, I[S] = e, I[S + 1] = s;
					}
				}
			}
		}, NA.prototype._singleRealTransform2 = function(I, A, C) {
			const B = this._out, E = this._data, r = E[A], o = E[A + C], t = r + o, a = r - o;
			B[I] = t, B[I + 1] = 0, B[I + 2] = a, B[I + 3] = 0;
		}, NA.prototype._singleRealTransform4 = function(I, A, C) {
			const B = this._out, E = this._data, r = this._inv ? -1 : 1, o = C * 2, t = C * 3, a = E[A], c = E[A + C], w = E[A + o], n = E[A + t], h = a + w, D = a - w, l = c + n, R = r * (c - n), f = h + l, N = D, M = -R, d = h - l, b = D, K = R;
			B[I] = f, B[I + 1] = 0, B[I + 2] = N, B[I + 3] = M, B[I + 4] = d, B[I + 5] = 0, B[I + 6] = b, B[I + 7] = K;
		};
	})), wI, qg = aA((() => {
		Kg(), wI = class {
			constructor(g) {
				this.size = g, this.indutnyFft = new NA(g);
			}
			fft(g) {
				const I = new Float32Array(2 * this.size);
				return this.indutnyFft.transform(I, g), I;
			}
		};
	})), PI, pg = aA((() => {
		PI = (() => {
			var g = self.location.href;
			return (function(I = {}) {
				var A = I, C, B;
				A.ready = new Promise((i, Q) => {
					C = i, B = Q;
				});
				var E = Object.assign({}, A), r = !0, o = !1, t = "";
				function a(i) {
					return A.locateFile ? A.locateFile(i, t) : t + i;
				}
				var c;
				(r || o) && (o ? t = self.location.href : typeof document < "u" && document.currentScript && (t = document.currentScript.src), g && (t = g), t.indexOf("blob:") !== 0 ? t = t.substr(0, t.replace(/[?#].*/, "").lastIndexOf("/") + 1) : t = "", o && (c = (i) => {
					var Q = new XMLHttpRequest();
					return Q.open("GET", i, !1), Q.responseType = "arraybuffer", Q.send(null), new Uint8Array(Q.response);
				})), A.print || console.log.bind(console);
				var w = A.printErr || console.error.bind(console);
				Object.assign(A, E), E = null, A.arguments && A.arguments, A.thisProgram && A.thisProgram, A.quit && A.quit;
				var n;
				A.wasmBinary && (n = A.wasmBinary), A.noExitRuntime, typeof WebAssembly != "object" && j("no native wasm support detected");
				var h, D, l = !1, R, f;
				function N() {
					var i = h.buffer;
					A.HEAP8 = R = new Int8Array(i), A.HEAP16 = new Int16Array(i), A.HEAP32 = new Int32Array(i), A.HEAPU8 = f = new Uint8Array(i), A.HEAPU16 = new Uint16Array(i), A.HEAPU32 = new Uint32Array(i), A.HEAPF32 = new Float32Array(i), A.HEAPF64 = new Float64Array(i);
				}
				var M = [], d = [], b = [];
				function K() {
					if (A.preRun) for (typeof A.preRun == "function" && (A.preRun = [A.preRun]); A.preRun.length;) _(A.preRun.shift());
					x(M);
				}
				function W() {
					x(d);
				}
				function T() {
					if (A.postRun) for (typeof A.postRun == "function" && (A.postRun = [A.postRun]); A.postRun.length;) p(A.postRun.shift());
					x(b);
				}
				function _(i) {
					M.unshift(i);
				}
				function q(i) {
					d.unshift(i);
				}
				function p(i) {
					b.unshift(i);
				}
				var Y = 0, H = null, u = null;
				function IA(i) {
					Y++, A.monitorRunDependencies && A.monitorRunDependencies(Y);
				}
				function O(i) {
					if (Y--, A.monitorRunDependencies && A.monitorRunDependencies(Y), Y == 0 && (H !== null && (clearInterval(H), H = null), u)) {
						var Q = u;
						u = null, Q();
					}
				}
				function j(i) {
					A.onAbort && A.onAbort(i), i = "Aborted(" + i + ")", w(i), l = !0, i += ". Build with -sASSERTIONS for more info.";
					var Q = new WebAssembly.RuntimeError(i);
					throw B(Q), Q;
				}
				var $ = "data:application/octet-stream;base64,";
				function AA(i) {
					return i.startsWith($);
				}
				var J = "data:application/octet-stream;base64,";
				AA(J) || (J = a(J));
				function tA(i) {
					if (i == J && n) return new Uint8Array(n);
					var Q = JA(i);
					if (Q) return Q;
					if (c) return c(i);
					throw "sync fetching of the wasm failed: you can preload it to Module['wasmBinary'] manually, or emcc.py will do that for you when generating HTML (but not JS)";
				}
				function QA(i, Q) {
					var e, s = tA(i);
					return e = new WebAssembly.Module(s), [new WebAssembly.Instance(e, Q), e];
				}
				function eA() {
					var i = { a: hA };
					function Q(e, s) {
						var k = e.exports;
						return D = k, h = D.b, N(), D.e, q(D.c), O("wasm-instantiate"), k;
					}
					if (IA("wasm-instantiate"), A.instantiateWasm) try {
						return A.instantiateWasm(i, Q);
					} catch (e) {
						w("Module.instantiateWasm callback failed with error: " + e), B(e);
					}
					return Q(QA(J, i)[0]);
				}
				var x = (i) => {
					for (; i.length > 0;) i.shift()(A);
				}, CA = (i) => {
					j("OOM");
				}, EA = (i) => {
					f.length, i >>>= 0, CA(i);
				};
				function gA(i) {
					return A["_" + i];
				}
				var BA = (i, Q) => {
					R.set(i, Q);
				}, iA = (i) => {
					for (var Q = 0, e = 0; e < i.length; ++e) {
						var s = i.charCodeAt(e);
						s <= 127 ? Q++ : s <= 2047 ? Q += 2 : s >= 55296 && s <= 57343 ? (Q += 4, ++e) : Q += 3;
					}
					return Q;
				}, nA = (i, Q, e, s) => {
					if (!(s > 0)) return 0;
					for (var k = e, S = e + s - 1, G = 0; G < i.length; ++G) {
						var F = i.charCodeAt(G);
						if (F >= 55296 && F <= 57343) {
							var v = i.charCodeAt(++G);
							F = 65536 + ((F & 1023) << 10) | v & 1023;
						}
						if (F <= 127) {
							if (e >= S) break;
							Q[e++] = F;
						} else if (F <= 2047) {
							if (e + 1 >= S) break;
							Q[e++] = 192 | F >> 6, Q[e++] = 128 | F & 63;
						} else if (F <= 65535) {
							if (e + 2 >= S) break;
							Q[e++] = 224 | F >> 12, Q[e++] = 128 | F >> 6 & 63, Q[e++] = 128 | F & 63;
						} else {
							if (e + 3 >= S) break;
							Q[e++] = 240 | F >> 18, Q[e++] = 128 | F >> 12 & 63, Q[e++] = 128 | F >> 6 & 63, Q[e++] = 128 | F & 63;
						}
					}
					return Q[e] = 0, e - k;
				}, oA = (i, Q, e) => nA(i, f, Q, e), sA = (i) => {
					var Q = iA(i) + 1, e = kA(Q);
					return oA(i, e, Q), e;
				}, DA = typeof TextDecoder < "u" ? new TextDecoder("utf8") : void 0, wA = (i, Q, e) => {
					for (var s = Q + e, k = Q; i[k] && !(k >= s);) ++k;
					if (k - Q > 16 && i.buffer && DA) return DA.decode(i.subarray(Q, k));
					for (var S = ""; Q < k;) {
						var G = i[Q++];
						if (!(G & 128)) {
							S += String.fromCharCode(G);
							continue;
						}
						var F = i[Q++] & 63;
						if ((G & 224) == 192) {
							S += String.fromCharCode((G & 31) << 6 | F);
							continue;
						}
						var v = i[Q++] & 63;
						if ((G & 240) == 224 ? G = (G & 15) << 12 | F << 6 | v : G = (G & 7) << 18 | F << 12 | v << 6 | i[Q++] & 63, G < 65536) S += String.fromCharCode(G);
						else {
							var X = G - 65536;
							S += String.fromCharCode(55296 | X >> 10, 56320 | X & 1023);
						}
					}
					return S;
				}, y = (i, Q) => i ? wA(f, i, Q) : "", U = function(i, Q, e, s, k) {
					var S = {
						string: (L) => {
							var z = 0;
							return L != null && L !== 0 && (z = sA(L)), z;
						},
						array: (L) => {
							var z = kA(L.length);
							return BA(L, z), z;
						}
					};
					function G(L) {
						return Q === "string" ? y(L) : Q === "boolean" ? !!L : L;
					}
					var F = gA(i), v = [], X = 0;
					if (s) for (var Z = 0; Z < s.length; Z++) {
						var FA = S[e[Z]];
						FA ? (X === 0 && (X = V()), v[Z] = FA(s[Z])) : v[Z] = s[Z];
					}
					var MA = F.apply(null, v);
					function LA(L) {
						return X !== 0 && bA(X), G(L);
					}
					return MA = LA(MA), MA;
				}, rA = function(i, Q, e, s) {
					var k = !e || e.every((S) => S === "number" || S === "boolean");
					return Q !== "string" && k && !s ? gA(i) : function() {
						return U(i, Q, e, arguments, s);
					};
				}, hA = { a: EA }, m = eA();
				m.c, A._fftCross = m.d, m.__errno_location, A._malloc = m.f, A._free = m.g;
				var V = m.h, bA = m.i, kA = m.j;
				function mA(i) {
					try {
						for (var Q = atob(i), e = new Uint8Array(Q.length), s = 0; s < Q.length; ++s) e[s] = Q.charCodeAt(s);
						return e;
					} catch {
						throw new Error("Converting base64 string to bytes failed.");
					}
				}
				function JA(i) {
					if (AA(i)) return mA(i.slice($.length));
				}
				A.ccall = U, A.cwrap = rA;
				var cA;
				u = function i() {
					cA || RA(), cA || (u = i);
				};
				function RA() {
					if (Y > 0 || (K(), Y > 0)) return;
					function i() {
						cA || (cA = !0, A.calledRun = !0, !l && (W(), C(A), A.onRuntimeInitialized && A.onRuntimeInitialized(), T()));
					}
					A.setStatus ? (A.setStatus("Running..."), setTimeout(function() {
						setTimeout(function() {
							A.setStatus("");
						}, 1), i();
					}, 1)) : i();
				}
				if (A.preInit) for (typeof A.preInit == "function" && (A.preInit = [A.preInit]); A.preInit.length > 0;) A.preInit.pop()();
				return RA(), I;
			});
		})();
	}));
	function Tg(g) {
		this.size = g, this.n = g * 8, this.ptr = KA._malloc(this.n * 4), this.ri = new Uint8Array(KA.HEAPU8.buffer, this.ptr, this.n), this.ii = new Uint8Array(KA.HEAPU8.buffer, this.ptr + this.n, this.n), this.transform = function(I, A, C) {
			var B = this.ptr, E = this.n;
			return this.ri.set(new Uint8Array(I.buffer)), this.ii.set(new Uint8Array(A.buffer)), WI(this.size, C, B, B + E, B + E * 2, B + E * 3), {
				real: new Float64Array(KA.HEAPU8.buffer, B + E * 2, this.size),
				imag: new Float64Array(KA.HEAPU8.buffer, B + E * 3, this.size)
			};
		}, this.dispose = function() {
			KA._free(this.ptr);
		};
	}
	var KA, WI, Pg = aA((() => {
		pg(), KA = PI({}), WI = KA.cwrap("fftCross", "void", [
			"number",
			"number",
			"number",
			"number",
			"number",
			"number"
		]);
	})), xI, Wg = aA((() => {
		Pg(), xI = class {
			constructor(g) {
				this.size = g, this.fftcross = new Tg(g), this.real = new Float64Array(this.size), this.imag = new Float64Array(this.size);
			}
			fft(g) {
				for (var I = 0; I < this.size; I++) this.real[I] = g[2 * I], this.imag[I] = g[2 * I + 1];
				const A = this.fftcross.transform(this.real, this.imag, !1), C = new Float32Array(2 * this.size);
				for (var I = 0; I < this.size; I++) C[2 * I] = A.real[I], C[2 * I + 1] = A.imag[I];
				return C;
			}
		};
	}));
	function xg(g) {
		this.n = g, this.levels = -1;
		for (var I = 0; I < 32; I++) 1 << I == g && (this.levels = I);
		if (this.levels == -1) throw "Length is not a power of 2";
		this.cosTable = new Array(g / 2), this.sinTable = new Array(g / 2);
		for (var I = 0; I < g / 2; I++) this.cosTable[I] = Math.cos(2 * Math.PI * I / g), this.sinTable[I] = Math.sin(2 * Math.PI * I / g);
		this.forward = function(A, C) {
			for (var B = this.n, E = 0; E < B; E++) {
				var r = D(E, this.levels);
				if (r > E) {
					var o = A[E];
					A[E] = A[r], A[r] = o, o = C[E], C[E] = C[r], C[r] = o;
				}
			}
			for (var t = 2; t <= B; t *= 2) for (var a = t / 2, c = B / t, E = 0; E < B; E += t) for (var r = E, w = 0; r < E + a; r++, w += c) {
				var n = A[r + a] * this.cosTable[w] + C[r + a] * this.sinTable[w], h = -A[r + a] * this.sinTable[w] + C[r + a] * this.cosTable[w];
				A[r + a] = A[r] - n, C[r + a] = C[r] - h, A[r] += n, C[r] += h;
			}
			function D(l, R) {
				for (var f = 0, N = 0; N < R; N++) f = f << 1 | l & 1, l >>>= 1;
				return f;
			}
		}, this.inverse = function(A, C) {
			forward(C, A);
		};
	}
	var Vg = aA((() => {})), VI, jg = aA((() => {
		Vg(), VI = class {
			constructor(g) {
				this.size = g, this.fftNayuki = new xg(g);
			}
			fft(g) {
				const I = new Float32Array(this.size), A = new Float32Array(this.size), C = new Float32Array(this.size * 2);
				for (var B = 0; B < this.size; ++B) I[B] = g[B * 2], A[B] = g[B * 2 + 1];
				this.fftNayuki.forward(I, A);
				for (var B = 0; B < this.size; ++B) C[B * 2] = I[B], C[B * 2 + 1] = A[B];
				return C;
			}
		};
	})), jI, Xg = aA((() => {
		jI = (() => {
			var g = self.location.href;
			return (function(I = {}) {
				var A = I, C, B;
				A.ready = new Promise((i, Q) => {
					C = i, B = Q;
				});
				var E = Object.assign({}, A), r = !0, o = !1, t = "";
				function a(i) {
					return A.locateFile ? A.locateFile(i, t) : t + i;
				}
				var c;
				(r || o) && (o ? t = self.location.href : typeof document < "u" && document.currentScript && (t = document.currentScript.src), g && (t = g), t.indexOf("blob:") !== 0 ? t = t.substr(0, t.replace(/[?#].*/, "").lastIndexOf("/") + 1) : t = "", o && (c = (i) => {
					var Q = new XMLHttpRequest();
					return Q.open("GET", i, !1), Q.responseType = "arraybuffer", Q.send(null), new Uint8Array(Q.response);
				})), A.print || console.log.bind(console);
				var w = A.printErr || console.error.bind(console);
				Object.assign(A, E), E = null, A.arguments && A.arguments, A.thisProgram && A.thisProgram, A.quit && A.quit;
				var n;
				A.wasmBinary && (n = A.wasmBinary), A.noExitRuntime, typeof WebAssembly != "object" && j("no native wasm support detected");
				var h, D, l = !1, R, f;
				function N() {
					var i = h.buffer;
					A.HEAP8 = R = new Int8Array(i), A.HEAP16 = new Int16Array(i), A.HEAP32 = new Int32Array(i), A.HEAPU8 = f = new Uint8Array(i), A.HEAPU16 = new Uint16Array(i), A.HEAPU32 = new Uint32Array(i), A.HEAPF32 = new Float32Array(i), A.HEAPF64 = new Float64Array(i);
				}
				var M = [], d = [], b = [];
				function K() {
					if (A.preRun) for (typeof A.preRun == "function" && (A.preRun = [A.preRun]); A.preRun.length;) _(A.preRun.shift());
					x(M);
				}
				function W() {
					x(d);
				}
				function T() {
					if (A.postRun) for (typeof A.postRun == "function" && (A.postRun = [A.postRun]); A.postRun.length;) p(A.postRun.shift());
					x(b);
				}
				function _(i) {
					M.unshift(i);
				}
				function q(i) {
					d.unshift(i);
				}
				function p(i) {
					b.unshift(i);
				}
				var Y = 0, H = null, u = null;
				function IA(i) {
					Y++, A.monitorRunDependencies && A.monitorRunDependencies(Y);
				}
				function O(i) {
					if (Y--, A.monitorRunDependencies && A.monitorRunDependencies(Y), Y == 0 && (H !== null && (clearInterval(H), H = null), u)) {
						var Q = u;
						u = null, Q();
					}
				}
				function j(i) {
					A.onAbort && A.onAbort(i), i = "Aborted(" + i + ")", w(i), l = !0, i += ". Build with -sASSERTIONS for more info.";
					var Q = new WebAssembly.RuntimeError(i);
					throw B(Q), Q;
				}
				var $ = "data:application/octet-stream;base64,";
				function AA(i) {
					return i.startsWith($);
				}
				var J = "data:application/octet-stream;base64,";
				AA(J) || (J = a(J));
				function tA(i) {
					if (i == J && n) return new Uint8Array(n);
					var Q = JA(i);
					if (Q) return Q;
					if (c) return c(i);
					throw "sync fetching of the wasm failed: you can preload it to Module['wasmBinary'] manually, or emcc.py will do that for you when generating HTML (but not JS)";
				}
				function QA(i, Q) {
					var e, s = tA(i);
					return e = new WebAssembly.Module(s), [new WebAssembly.Instance(e, Q), e];
				}
				function eA() {
					var i = { a: hA };
					function Q(e, s) {
						var k = e.exports;
						return D = k, h = D.b, N(), D.l, q(D.c), O("wasm-instantiate"), k;
					}
					if (IA("wasm-instantiate"), A.instantiateWasm) try {
						return A.instantiateWasm(i, Q);
					} catch (e) {
						w("Module.instantiateWasm callback failed with error: " + e), B(e);
					}
					return Q(QA(J, i)[0]);
				}
				var x = (i) => {
					for (; i.length > 0;) i.shift()(A);
				}, CA = (i) => {
					j("OOM");
				}, EA = (i) => {
					f.length, i >>>= 0, CA(i);
				};
				function gA(i) {
					return A["_" + i];
				}
				var BA = (i, Q) => {
					R.set(i, Q);
				}, iA = (i) => {
					for (var Q = 0, e = 0; e < i.length; ++e) {
						var s = i.charCodeAt(e);
						s <= 127 ? Q++ : s <= 2047 ? Q += 2 : s >= 55296 && s <= 57343 ? (Q += 4, ++e) : Q += 3;
					}
					return Q;
				}, nA = (i, Q, e, s) => {
					if (!(s > 0)) return 0;
					for (var k = e, S = e + s - 1, G = 0; G < i.length; ++G) {
						var F = i.charCodeAt(G);
						if (F >= 55296 && F <= 57343) {
							var v = i.charCodeAt(++G);
							F = 65536 + ((F & 1023) << 10) | v & 1023;
						}
						if (F <= 127) {
							if (e >= S) break;
							Q[e++] = F;
						} else if (F <= 2047) {
							if (e + 1 >= S) break;
							Q[e++] = 192 | F >> 6, Q[e++] = 128 | F & 63;
						} else if (F <= 65535) {
							if (e + 2 >= S) break;
							Q[e++] = 224 | F >> 12, Q[e++] = 128 | F >> 6 & 63, Q[e++] = 128 | F & 63;
						} else {
							if (e + 3 >= S) break;
							Q[e++] = 240 | F >> 18, Q[e++] = 128 | F >> 12 & 63, Q[e++] = 128 | F >> 6 & 63, Q[e++] = 128 | F & 63;
						}
					}
					return Q[e] = 0, e - k;
				}, oA = (i, Q, e) => nA(i, f, Q, e), sA = (i) => {
					var Q = iA(i) + 1, e = kA(Q);
					return oA(i, e, Q), e;
				}, DA = typeof TextDecoder < "u" ? new TextDecoder("utf8") : void 0, wA = (i, Q, e) => {
					for (var s = Q + e, k = Q; i[k] && !(k >= s);) ++k;
					if (k - Q > 16 && i.buffer && DA) return DA.decode(i.subarray(Q, k));
					for (var S = ""; Q < k;) {
						var G = i[Q++];
						if (!(G & 128)) {
							S += String.fromCharCode(G);
							continue;
						}
						var F = i[Q++] & 63;
						if ((G & 224) == 192) {
							S += String.fromCharCode((G & 31) << 6 | F);
							continue;
						}
						var v = i[Q++] & 63;
						if ((G & 240) == 224 ? G = (G & 15) << 12 | F << 6 | v : G = (G & 7) << 18 | F << 12 | v << 6 | i[Q++] & 63, G < 65536) S += String.fromCharCode(G);
						else {
							var X = G - 65536;
							S += String.fromCharCode(55296 | X >> 10, 56320 | X & 1023);
						}
					}
					return S;
				}, y = (i, Q) => i ? wA(f, i, Q) : "", U = function(i, Q, e, s, k) {
					var S = {
						string: (L) => {
							var z = 0;
							return L != null && L !== 0 && (z = sA(L)), z;
						},
						array: (L) => {
							var z = kA(L.length);
							return BA(L, z), z;
						}
					};
					function G(L) {
						return Q === "string" ? y(L) : Q === "boolean" ? !!L : L;
					}
					var F = gA(i), v = [], X = 0;
					if (s) for (var Z = 0; Z < s.length; Z++) {
						var FA = S[e[Z]];
						FA ? (X === 0 && (X = V()), v[Z] = FA(s[Z])) : v[Z] = s[Z];
					}
					var MA = F.apply(null, v);
					function LA(L) {
						return X !== 0 && bA(X), G(L);
					}
					return MA = LA(MA), MA;
				}, rA = function(i, Q, e, s) {
					var k = !e || e.every((S) => S === "number" || S === "boolean");
					return Q !== "string" && k && !s ? gA(i) : function() {
						return U(i, Q, e, arguments, s);
					};
				}, hA = { a: EA }, m = eA();
				m.c, A._malloc = m.d, A._free = m.e, A._precalc = m.f, A._precalc_f = m.g, A._dispose = m.h, A._dispose_f = m.i, A._transform_radix2_precalc = m.j, A._transform_radix2_precalc_f = m.k, m.__errno_location;
				var V = m.m, bA = m.n, kA = m.o;
				function mA(i) {
					try {
						for (var Q = atob(i), e = new Uint8Array(Q.length), s = 0; s < Q.length; ++s) e[s] = Q.charCodeAt(s);
						return e;
					} catch {
						throw new Error("Converting base64 string to bytes failed.");
					}
				}
				function JA(i) {
					if (AA(i)) return mA(i.slice($.length));
				}
				A.ccall = U, A.cwrap = rA;
				var cA;
				u = function i() {
					cA || RA(), cA || (u = i);
				};
				function RA() {
					if (Y > 0 || (K(), Y > 0)) return;
					function i() {
						cA || (cA = !0, A.calledRun = !0, !l && (W(), C(A), A.onRuntimeInitialized && A.onRuntimeInitialized(), T()));
					}
					A.setStatus ? (A.setStatus("Running..."), setTimeout(function() {
						setTimeout(function() {
							A.setStatus("");
						}, 1), i();
					}, 1)) : i();
				}
				if (A.preInit) for (typeof A.preInit == "function" && (A.preInit = [A.preInit]); A.preInit.length > 0;) A.preInit.pop()();
				return RA(), I;
			});
		})();
	}));
	function Og(g) {
		this.n = g, this.rptr = YA._malloc(g * 4 + g * 4), this.iptr = this.rptr + g * 4, this.rarr = new Float32Array(YA.HEAPU8.buffer, this.rptr, g), this.iarr = new Float32Array(YA.HEAPU8.buffer, this.iptr, g), this.tables = XI(g), this.forward = function(I, A) {
			this.rarr.set(I), this.iarr.set(A), ZI(this.rptr, this.iptr, this.n, this.tables), I.set(this.rarr), A.set(this.iarr);
		}, this.dispose = function() {
			YA._free(this.rptr), OI(this.tables);
		};
	}
	var YA, XI, OI, ZI, Zg = aA((() => {
		Xg(), YA = jI({}), YA.cwrap("precalc", "number", ["number"]), YA.cwrap("dispose", "void", ["number"]), YA.cwrap("transform_radix2_precalc", "void", [
			"number",
			"number",
			"number",
			"number"
		]), XI = YA.cwrap("precalc_f", "number", ["number"]), OI = YA.cwrap("dispose_f", "void", ["number"]), ZI = YA.cwrap("transform_radix2_precalc_f", "void", [
			"number",
			"number",
			"number",
			"number"
		]);
	})), zI, zg = aA((() => {
		Zg(), zI = class {
			constructor(g) {
				this.size = g, this.fftNayuki = new Og(g);
			}
			fft(g) {
				const I = new Float32Array(this.size), A = new Float32Array(this.size), C = new Float32Array(this.size * 2);
				for (var B = 0; B < this.size; ++B) I[B] = g[B * 2], A[B] = g[B * 2 + 1];
				this.fftNayuki.forward(I, A);
				for (var B = 0; B < this.size; ++B) C[B * 2] = I[B], C[B * 2 + 1] = A[B];
				return C;
			}
		};
	})), gI, _g = aA((() => {
		gI || (gI = {}), (function(g) {
			"use strict";
			function I(o, t, a, c, w, n) {
				for (var h = w.twiddle, D = 0; D < n; D++) {
					var l = o[2 * (t + a * D)], R = o[2 * (t + a * D) + 1], f = o[2 * (t + a * (D + n))], N = o[2 * (t + a * (D + n)) + 1], M = h[2 * (0 + c * D)], d = h[2 * (0 + c * D) + 1], b = f * M - N * d, K = f * d + N * M, W = l + b, T = R + K, _ = l - b, q = R - K;
					o[2 * (t + a * D)] = W, o[2 * (t + a * D) + 1] = T, o[2 * (t + a * (D + n))] = _, o[2 * (t + a * (D + n)) + 1] = q;
				}
			}
			function A(o, t, a, c, w, n) {
				for (var h = w.twiddle, D = n, l = 2 * n, R = c, f = 2 * c, N = h[2 * (0 + c * n) + 1], M = 0; M < n; M++) {
					var d = o[2 * (t + a * M)], b = o[2 * (t + a * M) + 1], K = o[2 * (t + a * (M + D))], W = o[2 * (t + a * (M + D)) + 1], T = h[2 * (0 + R * M)], _ = h[2 * (0 + R * M) + 1], q = K * T - W * _, p = K * _ + W * T, Y = o[2 * (t + a * (M + l))], H = o[2 * (t + a * (M + l)) + 1], u = h[2 * (0 + f * M)], IA = h[2 * (0 + f * M) + 1], O = Y * u - H * IA, j = Y * IA + H * u, $ = q + O, AA = p + j, J = d + $, tA = b + AA;
					o[2 * (t + a * M)] = J, o[2 * (t + a * M) + 1] = tA;
					var QA = d - $ * .5, eA = b - AA * .5, x = (q - O) * N, CA = (p - j) * N, EA = QA - CA, gA = eA + x;
					o[2 * (t + a * (M + D))] = EA, o[2 * (t + a * (M + D)) + 1] = gA;
					var BA = QA + CA, iA = eA - x;
					o[2 * (t + a * (M + l))] = BA, o[2 * (t + a * (M + l)) + 1] = iA;
				}
			}
			function C(o, t, a, c, w, n) {
				for (var h = w.twiddle, D = n, l = 2 * n, R = 3 * n, f = c, N = 2 * c, M = 3 * c, d = 0; d < n; d++) {
					var b = o[2 * (t + a * d)], K = o[2 * (t + a * d) + 1], W = o[2 * (t + a * (d + D))], T = o[2 * (t + a * (d + D)) + 1], _ = h[2 * (0 + f * d)], q = h[2 * (0 + f * d) + 1], p = W * _ - T * q, Y = W * q + T * _, H = o[2 * (t + a * (d + l))], u = o[2 * (t + a * (d + l)) + 1], IA = h[2 * (0 + N * d)], O = h[2 * (0 + N * d) + 1], j = H * IA - u * O, $ = H * O + u * IA, AA = o[2 * (t + a * (d + R))], J = o[2 * (t + a * (d + R)) + 1], tA = h[2 * (0 + M * d)], QA = h[2 * (0 + M * d) + 1], eA = AA * tA - J * QA, x = AA * QA + J * tA, CA = b + j, EA = K + $, gA = b - j, BA = K - $, iA = p + eA, nA = Y + x, oA = p - eA, sA = Y - x, DA = CA + iA, wA = EA + nA;
					if (w.inverse) var y = gA - sA, U = BA + oA;
					else var y = gA + sA, U = BA - oA;
					var rA = CA - iA, hA = EA - nA;
					if (w.inverse) var m = gA + sA, V = BA - oA;
					else var m = gA - sA, V = BA + oA;
					o[2 * (t + a * d)] = DA, o[2 * (t + a * d) + 1] = wA, o[2 * (t + a * (d + D))] = y, o[2 * (t + a * (d + D)) + 1] = U, o[2 * (t + a * (d + l))] = rA, o[2 * (t + a * (d + l)) + 1] = hA, o[2 * (t + a * (d + R))] = m, o[2 * (t + a * (d + R)) + 1] = V;
				}
			}
			function B(o, t, a, c, w, n, h) {
				for (var D = w.twiddle, l = w.n, R = new Float64Array(2 * h), f = 0; f < n; f++) {
					for (var N = 0, M = f; N < h; N++, M += n) {
						var d = o[2 * (t + a * M)], b = o[2 * (t + a * M) + 1];
						R[2 * N] = d, R[2 * N + 1] = b;
					}
					for (var N = 0, M = f; N < h; N++, M += n) {
						var K = 0, d = R[0], b = R[1];
						o[2 * (t + a * M)] = d, o[2 * (t + a * M) + 1] = b;
						for (var W = 1; W < h; W++) {
							K = (K + c * M) % l;
							var T = o[2 * (t + a * M)], _ = o[2 * (t + a * M) + 1], q = R[2 * W], p = R[2 * W + 1], Y = D[2 * K], H = D[2 * K + 1], u = q * Y - p * H, IA = q * H + p * Y, O = T + u, j = _ + IA;
							o[2 * (t + a * M)] = O, o[2 * (t + a * M) + 1] = j;
						}
					}
				}
			}
			function E(o, t, a, c, w, n, h, D, l) {
				var R = D.shift(), f = D.shift();
				if (f == 1) for (var N = 0; N < R * f; N++) {
					var M = c[2 * (w + n * h * N)], d = c[2 * (w + n * h * N) + 1];
					o[2 * (t + a * N)] = M, o[2 * (t + a * N) + 1] = d;
				}
				else for (var N = 0; N < R; N++) E(o, t + a * N * f, a, c, w + N * n * h, n * R, h, D.slice(), l);
				switch (R) {
					case 2:
						I(o, t, a, n, l, f);
						break;
					case 3:
						A(o, t, a, n, l, f);
						break;
					case 4:
						C(o, t, a, n, l, f);
						break;
					default:
						B(o, t, a, n, l, f, R);
						break;
				}
			}
			var r = function(a, c) {
				if (arguments.length < 2) throw new RangeError("You didn't pass enough arguments, passed `" + arguments.length + "'");
				var a = ~~a, c = !!c;
				if (a < 1) throw new RangeError("n is outside range, should be positive integer, was `" + a + "'");
				for (var w = {
					n: a,
					inverse: c,
					factors: [],
					twiddle: new Float64Array(2 * a),
					scratch: new Float64Array(2 * a)
				}, n = w.twiddle, h = 2 * Math.PI / a, D = 0; D < a; D++) {
					if (c) var l = h * D;
					else var l = -h * D;
					n[2 * D] = Math.cos(l), n[2 * D + 1] = Math.sin(l);
				}
				for (var R = 4, f = Math.floor(Math.sqrt(a)); a > 1;) {
					for (; a % R;) {
						switch (R) {
							case 4:
								R = 2;
								break;
							case 2:
								R = 3;
								break;
							default:
								R += 2;
								break;
						}
						R > f && (R = a);
					}
					a /= R, w.factors.push(R), w.factors.push(a);
				}
				this.state = w;
			};
			r.prototype.simple = function(o, t, a) {
				this.process(o, 0, 1, t, 0, 1, a);
			}, r.prototype.process = function(o, t, D, c, w, l, h) {
				var D = ~~D, l = ~~l, R = h == "real" ? h : "complex";
				if (D < 1) throw new RangeError("outputStride is outside range, should be positive integer, was `" + D + "'");
				if (l < 1) throw new RangeError("inputStride is outside range, should be positive integer, was `" + l + "'");
				if (R == "real") {
					for (var f = 0; f < this.state.n; f++) {
						var N = c[w + l * f], M = 0;
						this.state.scratch[2 * f] = N, this.state.scratch[2 * f + 1] = M;
					}
					E(o, t, D, this.state.scratch, 0, 1, 1, this.state.factors.slice(), this.state);
				} else if (c == o) {
					E(this.state.scratch, 0, 1, c, w, 1, l, this.state.factors.slice(), this.state);
					for (var f = 0; f < this.state.n; f++) {
						var N = this.state.scratch[2 * f], M = this.state.scratch[2 * f + 1];
						o[2 * (t + D * f)] = N, o[2 * (t + D * f) + 1] = M;
					}
				} else E(o, t, D, c, w, 1, l, this.state.factors.slice(), this.state);
			}, g.complex = r;
		})(gI);
	})), _I, $g = aA((() => {
		_g(), _I = class {
			constructor(g) {
				this.size = g, this.nockertfft = new gI.complex(g, !1);
			}
			fft(g) {
				const I = new Float32Array(2 * this.size);
				return this.nockertfft.simple(I, g, "complex"), I;
			}
		};
	}));
	function AB(g) {
		if (g !== 0 && (g & g - 1) === 0) P = g, CB(), QB(), EB();
		else throw new Error("init: radix-2 required");
	}
	function BI(g, I) {
		lI(g, I, 1);
	}
	function CI(g, I) {
		let A = 1 / P;
		lI(g, I, -1);
		for (let C = 0; C < P; C++) g[C] *= A, I[C] *= A;
	}
	function IB(g, I) {
		lI(g, I, -1);
	}
	function gB(g, I) {
		let A = [], C = [], B = 0;
		for (let E = 0; E < P; E++) {
			B = E * P;
			for (let r = 0; r < P; r++) A[r] = g[r + B], C[r] = I[r + B];
			BI(A, C);
			for (let r = 0; r < P; r++) g[r + B] = A[r], I[r + B] = C[r];
		}
		for (let E = 0; E < P; E++) {
			for (let r = 0; r < P; r++) B = E + r * P, A[r] = g[B], C[r] = I[B];
			BI(A, C);
			for (let r = 0; r < P; r++) B = E + r * P, g[B] = A[r], I[B] = C[r];
		}
	}
	function BB(g, I) {
		let A = [], C = [], B = 0;
		for (let E = 0; E < P; E++) {
			B = E * P;
			for (let r = 0; r < P; r++) A[r] = g[r + B], C[r] = I[r + B];
			CI(A, C);
			for (let r = 0; r < P; r++) g[r + B] = A[r], I[r + B] = C[r];
		}
		for (let E = 0; E < P; E++) {
			for (let r = 0; r < P; r++) B = E + r * P, A[r] = g[B], C[r] = I[B];
			CI(A, C);
			for (let r = 0; r < P; r++) B = E + r * P, g[B] = A[r], I[B] = C[r];
		}
	}
	function lI(g, I, A) {
		let C, B, E, r, o, t, a, c, w, n = P >> 2;
		for (let h = 0; h < P; h++) r = jA[h], h < r && (o = g[h], g[h] = g[r], g[r] = o, o = I[h], I[h] = I[r], I[r] = o);
		for (let h = 1; h < P; h <<= 1) {
			B = 0, C = P / (h << 1);
			for (let D = 0; D < h; D++) {
				t = yA[B + n], a = A * yA[B];
				for (let l = D; l < P; l += h << 1) E = l + h, c = t * g[E] + a * I[E], w = t * I[E] - a * g[E], g[E] = g[l] - c, g[l] += c, I[E] = I[l] - w, I[l] += w;
				B += C;
			}
		}
	}
	function CB() {
		typeof Uint32Array < "u" ? jA = new Uint32Array(P) : jA = [], typeof Float64Array < "u" ? yA = new Float64Array(P * 1.25) : yA = [];
	}
	function QB() {
		let g = 0, I = 0, A = 0;
		for (jA[0] = 0; ++g < P;) {
			for (A = P >> 1; A <= I;) I -= A, A >>= 1;
			I += A, jA[g] = I;
		}
	}
	function EB() {
		let g = P >> 1, I = P >> 2, A = P >> 3, C = g + I, B = Math.sin(Math.PI / P), E = 2 * B * B, r = Math.sqrt(E * (2 - E)), o = yA[I] = 1, t = yA[0] = 0;
		B = 2 * E;
		for (let a = 1; a < A; a++) o -= E, E += B * o, t += r, r -= B * t, yA[a] = t, yA[I - a] = o;
		A !== 0 && (yA[A] = Math.sqrt(.5));
		for (let a = 0; a < I; a++) yA[g - a] = yA[a];
		for (let a = 0; a < C; a++) yA[a + g] = -yA[a];
	}
	var P, jA, yA, $I, iB = aA((() => {
		P = 0, jA = null, yA = null, $I = {
			init: AB,
			fft1d: BI,
			ifft1d: CI,
			fft2d: gB,
			ifft2d: BB,
			fft: BI,
			ifft: CI,
			bt: IB
		};
	})), Ag, rB = aA((() => {
		iB(), Ag = class {
			constructor(g) {
				this.size = g, this.FFT_mljs = $I, this.FFT_mljs.init(g);
			}
			fft(g) {
				const I = new Float32Array(this.size), A = new Float32Array(this.size), C = new Float32Array(2 * this.size);
				for (var B = 0; B < this.size; ++B) I[B] = g[B * 2], A[B] = g[B * 2 + 1];
				this.FFT_mljs.fft(I, A);
				for (var B = 0; B < this.size; ++B) C[B * 2] = I[B], C[B * 2 + 1] = A[B];
				return C;
			}
		};
	}));
	async function tB() {
		return await WebAssembly.validate(new Uint8Array([
			0,
			97,
			115,
			109,
			1,
			0,
			0,
			0,
			1,
			5,
			1,
			96,
			0,
			1,
			123,
			3,
			2,
			1,
			0,
			10,
			15,
			1,
			13,
			0,
			65,
			1,
			253,
			15,
			65,
			2,
			253,
			15,
			253,
			128,
			2,
			11
		]));
	}
	async function eB() {
		return await WebAssembly.validate(new Uint8Array([
			0,
			97,
			115,
			109,
			1,
			0,
			0,
			0,
			1,
			5,
			1,
			96,
			0,
			1,
			123,
			3,
			2,
			1,
			0,
			10,
			10,
			1,
			8,
			0,
			65,
			0,
			253,
			15,
			253,
			98,
			11
		]));
	}
	async function aB() {
		let g = "Other", I = "Unknown", A = "Other", C = "Unknown", B = navigator.userAgentData, E = navigator.userAgent;
		try {
			if (B) {
				const r = await B.getHighEntropyValues([
					"architecture",
					"model",
					"platform",
					"platformVersion",
					"uaFullVersion"
				]), o = B.brands.find((t) => [
					"Microsoft Edge",
					"Google Chrome",
					"Opera"
				].includes(t.brand));
				g = o ? o.brand : "Other", I = o ? `v${o.version}` : "Unknown", A = r.platform ? r.platform : "Other", C = r.platformVersion ? `v${r.platformVersion}` : "Unknown";
			}
			if (g === "Other" || A === "Other") {
				const r = E.split(" "), o = r[r.length - 1], t = /Firefox/.test(o), a = /Safari/.test(o) && !/CriOS/.test(o) && !/Chrome/.test(o), c = /CriOS/.test(o) || /Chrome/.test(o), w = /Edg/.test(o), n = /OPR/.test(o), h = [
					{
						name: "Mozilla Firefox",
						regex: /Firefox\/(\d+\.\d+)/,
						flag: t
					},
					{
						name: "Safari",
						regex: /Version\/(\d+\.\d+)/,
						flag: a
					},
					{
						name: "Google Chrome",
						regex: /CriOS|Chrome\/(\d+\.\d+)/,
						flag: c
					},
					{
						name: "Microsoft Edge",
						regex: /Edg\/(\d+\.\d+)/,
						flag: w
					},
					{
						name: "Opera",
						regex: /OPR\/(\d+\.\d+)/,
						flag: n
					}
				];
				for (const N of h) if (N.flag) {
					g = N.name;
					const M = o.match(N.regex);
					I = M ? M[1] : "Unknown";
					break;
				}
				const D = E.match(/\(([^)]+)\)/), l = D ? D[1].split("; ") : [];
				console.log(D), console.log(l);
				const R = {
					"10.0": "10",
					"6.3": "8.1",
					"6.2": "8",
					"6.1": "7",
					"6.0": "Vista",
					"5.2": "XP 64-bit",
					"5.1": "XP",
					"5.0": "2000"
				}, f = [
					{
						name: "Windows",
						regex: /Windows NT/,
						transform: (N) => R[N.split(" ")[2]],
						index: 0
					},
					{
						name: "Mac OS X",
						regex: /Mac OS X/,
						transform: (N) => N.replace("_", ".").split(" ")[3],
						index: 0
					},
					{
						name: "Linux",
						regex: /Linux/,
						transform: () => "Unknown",
						index: 0
					},
					{
						name: "Android",
						regex: /Android/,
						transform: (N) => N.split(" ")[1],
						index: 0
					},
					{
						name: "iOS",
						regex: /iPhone/,
						transform: (N) => N.split(" ")[1].replace("_", "."),
						index: 0
					}
				];
				for (const N of f) if (N.regex.test(l[0])) {
					A = N.name, console.log(`osDetails: ${l}`), C = N.transform ? N.transform(l[1]) : N.versionMap[l[1].split(" ")[N.index]];
					break;
				}
			}
		} catch (r) {
			console.error("Could not retrieve user agent data", r);
		}
		return {
			browserName: g,
			browserVersion: I,
			osName: A,
			osVersion: C,
			wasm: typeof WebAssembly == "object",
			relaxedSimd: await tB(),
			simd: await eB()
		};
	}
	var oB = aA((() => {})), Ig, nB = aA((() => {
		Ig = (() => {
			var g = self.location.href;
			return (function(I = {}) {
				var A = I, C, B;
				A.ready = new Promise((i, Q) => {
					C = i, B = Q;
				});
				var E = Object.assign({}, A), r = !0, o = !1, t = "";
				function a(i) {
					return A.locateFile ? A.locateFile(i, t) : t + i;
				}
				var c;
				(r || o) && (o ? t = self.location.href : typeof document < "u" && document.currentScript && (t = document.currentScript.src), g && (t = g), t.indexOf("blob:") !== 0 ? t = t.substr(0, t.replace(/[?#].*/, "").lastIndexOf("/") + 1) : t = "", o && (c = (i) => {
					var Q = new XMLHttpRequest();
					return Q.open("GET", i, !1), Q.responseType = "arraybuffer", Q.send(null), new Uint8Array(Q.response);
				})), A.print || console.log.bind(console);
				var w = A.printErr || console.error.bind(console);
				Object.assign(A, E), E = null, A.arguments && A.arguments, A.thisProgram && A.thisProgram, A.quit && A.quit;
				var n;
				A.wasmBinary && (n = A.wasmBinary), A.noExitRuntime, typeof WebAssembly != "object" && j("no native wasm support detected");
				var h, D, l = !1, R, f;
				function N() {
					var i = h.buffer;
					A.HEAP8 = R = new Int8Array(i), A.HEAP16 = new Int16Array(i), A.HEAP32 = new Int32Array(i), A.HEAPU8 = f = new Uint8Array(i), A.HEAPU16 = new Uint16Array(i), A.HEAPU32 = new Uint32Array(i), A.HEAPF32 = new Float32Array(i), A.HEAPF64 = new Float64Array(i);
				}
				var M = [], d = [], b = [];
				function K() {
					if (A.preRun) for (typeof A.preRun == "function" && (A.preRun = [A.preRun]); A.preRun.length;) _(A.preRun.shift());
					x(M);
				}
				function W() {
					x(d);
				}
				function T() {
					if (A.postRun) for (typeof A.postRun == "function" && (A.postRun = [A.postRun]); A.postRun.length;) p(A.postRun.shift());
					x(b);
				}
				function _(i) {
					M.unshift(i);
				}
				function q(i) {
					d.unshift(i);
				}
				function p(i) {
					b.unshift(i);
				}
				var Y = 0, H = null, u = null;
				function IA(i) {
					Y++, A.monitorRunDependencies && A.monitorRunDependencies(Y);
				}
				function O(i) {
					if (Y--, A.monitorRunDependencies && A.monitorRunDependencies(Y), Y == 0 && (H !== null && (clearInterval(H), H = null), u)) {
						var Q = u;
						u = null, Q();
					}
				}
				function j(i) {
					A.onAbort && A.onAbort(i), i = "Aborted(" + i + ")", w(i), l = !0, i += ". Build with -sASSERTIONS for more info.";
					var Q = new WebAssembly.RuntimeError(i);
					throw B(Q), Q;
				}
				var $ = "data:application/octet-stream;base64,";
				function AA(i) {
					return i.startsWith($);
				}
				var J = "data:application/octet-stream;base64,";
				AA(J) || (J = a(J));
				function tA(i) {
					if (i == J && n) return new Uint8Array(n);
					var Q = JA(i);
					if (Q) return Q;
					if (c) return c(i);
					throw "sync fetching of the wasm failed: you can preload it to Module['wasmBinary'] manually, or emcc.py will do that for you when generating HTML (but not JS)";
				}
				function QA(i, Q) {
					var e, s = tA(i);
					return e = new WebAssembly.Module(s), [new WebAssembly.Instance(e, Q), e];
				}
				function eA() {
					var i = { a: hA };
					function Q(e, s) {
						var k = e.exports;
						return D = k, h = D.b, N(), D.i, q(D.c), O("wasm-instantiate"), k;
					}
					if (IA("wasm-instantiate"), A.instantiateWasm) try {
						return A.instantiateWasm(i, Q);
					} catch (e) {
						w("Module.instantiateWasm callback failed with error: " + e), B(e);
					}
					return Q(QA(J, i)[0]);
				}
				var x = (i) => {
					for (; i.length > 0;) i.shift()(A);
				}, CA = (i) => {
					j("OOM");
				}, EA = (i) => {
					f.length, i >>>= 0, CA(i);
				};
				function gA(i) {
					return A["_" + i];
				}
				var BA = (i, Q) => {
					R.set(i, Q);
				}, iA = (i) => {
					for (var Q = 0, e = 0; e < i.length; ++e) {
						var s = i.charCodeAt(e);
						s <= 127 ? Q++ : s <= 2047 ? Q += 2 : s >= 55296 && s <= 57343 ? (Q += 4, ++e) : Q += 3;
					}
					return Q;
				}, nA = (i, Q, e, s) => {
					if (!(s > 0)) return 0;
					for (var k = e, S = e + s - 1, G = 0; G < i.length; ++G) {
						var F = i.charCodeAt(G);
						if (F >= 55296 && F <= 57343) {
							var v = i.charCodeAt(++G);
							F = 65536 + ((F & 1023) << 10) | v & 1023;
						}
						if (F <= 127) {
							if (e >= S) break;
							Q[e++] = F;
						} else if (F <= 2047) {
							if (e + 1 >= S) break;
							Q[e++] = 192 | F >> 6, Q[e++] = 128 | F & 63;
						} else if (F <= 65535) {
							if (e + 2 >= S) break;
							Q[e++] = 224 | F >> 12, Q[e++] = 128 | F >> 6 & 63, Q[e++] = 128 | F & 63;
						} else {
							if (e + 3 >= S) break;
							Q[e++] = 240 | F >> 18, Q[e++] = 128 | F >> 12 & 63, Q[e++] = 128 | F >> 6 & 63, Q[e++] = 128 | F & 63;
						}
					}
					return Q[e] = 0, e - k;
				}, oA = (i, Q, e) => nA(i, f, Q, e), sA = (i) => {
					var Q = iA(i) + 1, e = kA(Q);
					return oA(i, e, Q), e;
				}, DA = typeof TextDecoder < "u" ? new TextDecoder("utf8") : void 0, wA = (i, Q, e) => {
					for (var s = Q + e, k = Q; i[k] && !(k >= s);) ++k;
					if (k - Q > 16 && i.buffer && DA) return DA.decode(i.subarray(Q, k));
					for (var S = ""; Q < k;) {
						var G = i[Q++];
						if (!(G & 128)) {
							S += String.fromCharCode(G);
							continue;
						}
						var F = i[Q++] & 63;
						if ((G & 224) == 192) {
							S += String.fromCharCode((G & 31) << 6 | F);
							continue;
						}
						var v = i[Q++] & 63;
						if ((G & 240) == 224 ? G = (G & 15) << 12 | F << 6 | v : G = (G & 7) << 18 | F << 12 | v << 6 | i[Q++] & 63, G < 65536) S += String.fromCharCode(G);
						else {
							var X = G - 65536;
							S += String.fromCharCode(55296 | X >> 10, 56320 | X & 1023);
						}
					}
					return S;
				}, y = (i, Q) => i ? wA(f, i, Q) : "", U = function(i, Q, e, s, k) {
					var S = {
						string: (L) => {
							var z = 0;
							return L != null && L !== 0 && (z = sA(L)), z;
						},
						array: (L) => {
							var z = kA(L.length);
							return BA(L, z), z;
						}
					};
					function G(L) {
						return Q === "string" ? y(L) : Q === "boolean" ? !!L : L;
					}
					var F = gA(i), v = [], X = 0;
					if (s) for (var Z = 0; Z < s.length; Z++) {
						var FA = S[e[Z]];
						FA ? (X === 0 && (X = V()), v[Z] = FA(s[Z])) : v[Z] = s[Z];
					}
					var MA = F.apply(null, v);
					function LA(L) {
						return X !== 0 && bA(X), G(L);
					}
					return MA = LA(MA), MA;
				}, rA = function(i, Q, e, s) {
					var k = !e || e.every((S) => S === "number" || S === "boolean");
					return Q !== "string" && k && !s ? gA(i) : function() {
						return U(i, Q, e, arguments, s);
					};
				}, hA = { a: EA }, m = eA();
				m.c, A._kiss_fft_free = m.d, A._free = m.e, A._kiss_fft_alloc = m.f, A._malloc = m.g, A._kiss_fft = m.h, m.__errno_location;
				var V = m.j, bA = m.k, kA = m.l;
				function mA(i) {
					try {
						for (var Q = atob(i), e = new Uint8Array(Q.length), s = 0; s < Q.length; ++s) e[s] = Q.charCodeAt(s);
						return e;
					} catch {
						throw new Error("Converting base64 string to bytes failed.");
					}
				}
				function JA(i) {
					if (AA(i)) return mA(i.slice($.length));
				}
				A.ccall = U, A.cwrap = rA;
				var cA;
				u = function i() {
					cA || RA(), cA || (u = i);
				};
				function RA() {
					if (Y > 0 || (K(), Y > 0)) return;
					function i() {
						cA || (cA = !0, A.calledRun = !0, !l && (W(), C(A), A.onRuntimeInitialized && A.onRuntimeInitialized(), T()));
					}
					A.setStatus ? (A.setStatus("Running..."), setTimeout(function() {
						setTimeout(function() {
							A.setStatus("");
						}, 1), i();
					}, 1)) : i();
				}
				if (A.preInit) for (typeof A.preInit == "function" && (A.preInit = [A.preInit]); A.preInit.length > 0;) A.preInit.pop()();
				return RA(), I;
			});
		})();
	})), SA, fI, gg, FI, Bg, sB = aA((() => {
		nB(), SA = Ig({}), fI = SA.cwrap("kiss_fft_alloc", "number", [
			"number",
			"number",
			"number",
			"number"
		]), gg = SA.cwrap("kiss_fft", "void", [
			"number",
			"number",
			"number"
		]), FI = SA.cwrap("kiss_fft_free", "void", ["number"]), Bg = class {
			constructor(g) {
				this.size = g, this.fcfg = fI(g, !1), this.icfg = fI(g, !0), this.inptr = SA._malloc(g * 8 + g * 8), this.cin = new Float32Array(SA.HEAPU8.buffer, this.inptr, g * 2);
			}
			fft = function(g) {
				const I = SA._malloc(this.size * 8), A = new Float32Array(SA.HEAPU8.buffer, I, this.size * 2);
				this.cin.set(g), gg(this.fcfg, this.inptr, I);
				let C = new Float32Array(this.size * 2);
				return C.set(A), SA._free(I), C;
			};
			dispose() {
				FI(this.fcfg), FI(this.icfg), SA._free(this.inptr);
			}
		};
	}));
	function QI(g) {
		this.size = g, this._csize = g << 1;
		for (var I = new Array(this.size * 2), A = 0; A < I.length; A += 2) {
			const t = Math.PI * A / this.size;
			I[A] = Math.cos(t), I[A + 1] = -Math.sin(t);
		}
		this.table = I;
		for (var C = 0, B = 1; this.size > B; B <<= 1) C++;
		this._width = C % 2 === 0 ? C - 1 : C, this._bitrev = new Array(1 << this._width);
		for (var E = 0; E < this._bitrev.length; E++) {
			this._bitrev[E] = 0;
			for (var r = 0; r < this._width; r += 2) {
				var o = this._width - r - 2;
				this._bitrev[E] |= (E >>> r & 3) << o;
			}
		}
		this._data = null;
	}
	var DB = aA((() => {
		QI.prototype.fft = function(I) {
			this._data = I, this._out = new Float32Array(2 * this.size);
			var A = this._csize, C = 1 << this._width, B = A / C << 1, E, r, o = this._bitrev;
			if (B === 4) for (E = 0, r = 0; E < A; E += B, r++) {
				const n = o[r];
				this._singleTransform2(E, n, C);
			}
			else for (E = 0, r = 0; E < A; E += B, r++) {
				const n = o[r];
				this._singleTransform4(E, n, C);
			}
			for (C >>= 2; C >= 2; C >>= 2) {
				B = A / C << 1;
				var t = B >>> 2;
				for (E = 0; E < A; E += B) for (var a = E + t, c = E, w = 0; c < a; c += 2, w += C) {
					const n = c, h = n + t, D = h + t, l = D + t, R = this._out[n], f = this._out[n + 1], N = this._out[h], M = this._out[h + 1], d = this._out[D], b = this._out[D + 1], K = this._out[l], W = this._out[l + 1], T = R, _ = f, q = this.table[w], p = this.table[w + 1], Y = N * q - M * p, H = N * p + M * q, u = this.table[2 * w], IA = this.table[2 * w + 1], O = d * u - b * IA, j = d * IA + b * u, $ = this.table[3 * w], AA = this.table[3 * w + 1], J = K * $ - W * AA, tA = K * AA + W * $, QA = T + O, eA = _ + j, x = T - O, CA = _ - j, EA = Y + J, gA = H + tA, BA = Y - J, iA = H - tA;
					this._out[n] = QA + EA, this._out[n + 1] = eA + gA, this._out[h] = x + iA, this._out[h + 1] = CA - BA, this._out[D] = QA - EA, this._out[D + 1] = eA - gA, this._out[l] = x - iA, this._out[l + 1] = CA + BA;
				}
			}
			return this._out;
		}, QI.prototype._singleTransform2 = function(I, A, C) {
			const B = this._data[A], E = this._data[A + 1], r = this._data[A + C], o = this._data[A + C + 1];
			this._out[I] = B + r, this._out[I + 1] = E + o, this._out[I + 2] = B - r, this._out[I + 3] = E - o;
		}, QI.prototype._singleTransform4 = function(I, A, C) {
			const B = C * 2, E = C * 3, r = this._data[A], o = this._data[A + 1], t = this._data[A + C], a = this._data[A + C + 1], c = this._data[A + B], w = this._data[A + B + 1], n = this._data[A + E], h = this._data[A + E + 1], D = r + c, l = o + w, R = r - c, f = o - w, N = t + n, M = a + h, d = t - n, b = a - h;
			this._out[I] = D + N, this._out[I + 1] = l + M, this._out[I + 2] = R + b, this._out[I + 3] = f - d, this._out[I + 4] = D - N, this._out[I + 5] = l - M, this._out[I + 6] = R - b, this._out[I + 7] = f + d;
		};
	})), hB = cg({ default: () => Cg }), RI, Cg, cB = aA((() => {
		Lg(), qg(), Wg(), jg(), zg(), $g(), rB(), oB(), sB(), DB(), RI = [
			4,
			8,
			16,
			32,
			64,
			128,
			256,
			512,
			1024,
			2048,
			4096,
			8192,
			16384,
			32768,
			16384,
			32768,
			65536,
			131072
		], Cg = class {
			constructor(g = 128, I = "indutnyJavascript", A = !0) {
				if (!RI.includes(g)) throw new Error("Size must be a power of 2 between 4 and 131072");
				this.size = g, this.outputArr = new Float32Array(2 * g), this.subLibrary = I, this.fftLibrary = void 0;
				const C = this.getCurrentProfile();
				C && A ? this.setSubLibrary(C.fastestSubLibrary) : this.setSubLibrary(I);
			}
			availableSubLibraries() {
				return [
					"kissWasm",
					"indutnyModifiedJavascript",
					"indutnyJavascript",
					"crossWasm",
					"mljsJavascript",
					"nockertJavascript",
					"nayuki3Wasm",
					"nayukiJavascript",
					"kissfftmodifiedWasm"
				];
			}
			availableSubLibrariesQuick() {
				return ["kissWasm", "indutnyModifiedJavascript"];
			}
			getCurrentProfile() {
				if (!(typeof localStorage > "u") && localStorage.getItem("webfftProfile")) return JSON.parse(localStorage.getItem("webfftProfile"));
			}
			setSubLibrary(g) {
				switch (g) {
					case "nayukiJavascript":
						this.fftLibrary = new VI(this.size);
						break;
					case "nayuki3Wasm":
						this.fftLibrary = new zI(this.size);
						break;
					case "kissWasm":
						this.fftLibrary = new TI(this.size);
						break;
					case "crossWasm":
						this.fftLibrary = new xI(this.size), this.size > 16384 && (this.fftLibrary = new wI(this.size));
						break;
					case "nockertJavascript":
						this.fftLibrary = new _I(this.size);
						break;
					case "indutnyJavascript":
						this.fftLibrary = new wI(this.size);
						break;
					case "mljsJavascript":
						this.fftLibrary = new Ag(this.size);
						break;
					case "kissfftmodifiedWasm":
						this.fftLibrary = new Bg(this.size);
						break;
					case "indutnyModifiedJavascript":
						this.fftLibrary = new QI(this.size);
						break;
					default: throw new Error("Invalid sublibrary");
				}
			}
			fft(g) {
				if (g.length !== 2 * this.size) throw new Error("Input array length must be == 2 * size");
				return this.outputArr = this.fftLibrary.fft(g), this.outputArr;
			}
			fftr(g) {
				var { outputArr: I, fftLibrary: A, size: C } = this;
				if (g.length !== C) throw new Error("Input array length must be == size");
				const B = new Float32Array(2 * C);
				B.fill(0);
				for (let E = 0; E < C; E++) B[2 * E] = g[E];
				return I = A.fft(B), I.slice(C, C * 2);
			}
			fft2d(g) {
				const I = g[0].length / 2, A = g.length;
				if (I !== this.size) throw new Error("Inner array length must be == 2 * size");
				if (!RI.includes(A)) throw new Error("Outter array length must be a power of 2 between 4 and 131072");
				let C = [];
				for (let r = 0; r < A; r++) this.outputArr = this.fft(g[r]), C.push(this.outputArr);
				this.dispose(), this.size = A, this.setSubLibrary(this.subLibrary);
				let B = [];
				for (let r = 0; r < I; r++) {
					const o = new Float32Array(2 * A);
					o.fill(0);
					for (let a = 0; a < A; a++) o[2 * a] = C[a][2 * r], o[2 * a + 1] = C[a][2 * r + 1];
					let t = new Float32Array(2 * A);
					t = this.fft(o), B.push(t);
				}
				let E = [];
				for (let r = 0; r < A; r++) {
					let o = new Float32Array(2 * I);
					for (let t = 0; t < I; t++) o[2 * t] = B[t][2 * r], o[2 * t + 1] = B[t][2 * r + 1];
					E.push(o);
				}
				return this.dispose(), this.size = I, this.setSubLibrary(this.subLibrary), E;
			}
			profile(g = 1, I = !0, A = !1) {
				if (!I && this.getCurrentProfile()) return this.getCurrentProfile();
				const C = performance.now();
				let B;
				A ? B = this.availableSubLibrariesQuick() : B = this.availableSubLibraries();
				let E = [];
				const r = g / B.length / 2;
				for (let c = 0; c < B.length; c++) {
					this.setSubLibrary(B[c]);
					const w = new Float32Array(2 * this.size);
					for (let D = 0; D < this.size; D++) w[2 * D] = Math.random() - .5, w[2 * D + 1] = Math.random() - .5;
					let n = performance.now();
					for (; (performance.now() - n) / 1e3 < r;) this.fft(w);
					n = performance.now();
					let h = 0;
					for (; (performance.now() - n) / 1e3 < r;) this.fft(w), h++;
					E.push(1e3 * h / (performance.now() - n)), this.dispose();
				}
				const o = (performance.now() - C) / 1e3;
				let t = E.indexOf(Math.max(...E));
				const a = {
					fftsPerSecond: E,
					subLibraries: B,
					totalElapsed: o,
					fastestSubLibrary: B[t]
				};
				return console.log("Setting sublibrary to", a.fastestSubLibrary), this.setSubLibrary(a.fastestSubLibrary), typeof localStorage < "u" && localStorage.setItem("webfftProfile", JSON.stringify(a)), a;
			}
			async checkBrowserCapabilities() {
				return await aB();
			}
			dispose() {
				this.fftLibrary && this.fftLibrary.dispose !== void 0 && this.fftLibrary.dispose();
			}
		};
	}));
	let NI = null, Qg = 0;
	async function wB(g) {
		try {
			const { default: I } = await Promise.resolve().then(() => (cB(), hB));
			NI = new I(g), await NI.profile(), Qg = g;
		} catch (I) {
			console.warn("[dspWorker] WebFFT not available, using Radix-2 fallback:", I), NI = null;
		}
	}
	let UA, HA, qA, pA, lA, fA, Eg, ig, rg, tg, vA, XA, OA, EI, TA, iI, yI, rI, PA, WA, xA;
	const MI = 21;
	let eg = MI, GI = 0, kI = [], YI = [], dI = [], SI = [], UI, HI, tI, eI;
	function ag(g, I) {
		eg = I, GI = 0, kI = Array.from({ length: I }, () => new Float32Array(g)), YI = Array.from({ length: I }, () => new Float32Array(g)), dI = Array.from({ length: I }, () => new Float32Array(g)), SI = Array.from({ length: I }, () => new Float32Array(g)), UI = new Float32Array(g), HI = new Float32Array(g), tI = new Float32Array(g), eI = new Float32Array(g);
	}
	function lB(g, I, A, C, B) {
		const E = GI;
		for (let r = 0; r < B; r++) {
			const o = g[r] * g[r] + I[r] * I[r], t = A[r] * A[r] + C[r] * C[r], a = g[r] * A[r] + I[r] * C[r], c = g[r] * C[r] - I[r] * A[r];
			UI[r] += o - kI[E][r], HI[r] += t - YI[E][r], tI[r] += a - dI[E][r], eI[r] += c - SI[E][r], kI[E][r] = o, YI[E][r] = t, dI[E][r] = a, SI[E][r] = c;
		}
		GI = (E + 1) % eg;
	}
	function fB(g, I) {
		for (let A = 0; A < I; A++) {
			const C = tI[A] * tI[A] + eI[A] * eI[A], B = UI[A] * HI[A] + 1e-12;
			g[A] = Math.min(1, Math.max(0, Math.sqrt(C) / Math.sqrt(B)));
		}
	}
	let GA = 0, aI = 0, uA = null;
	const og = new Yg();
	let ZA = null, oI = null, vI = "None", ng = 0, VA = null, nI = null, mI = null, sg = null, Dg = null, hg = 0;
	function FB(g, I) {
		const A = g.length, C = (I % A + A) % A;
		if (C === 0) return;
		const B = new Float32Array(C);
		B.set(g.subarray(0, C)), g.copyWithin(0, C), g.set(B, A - C);
	}
	self.onmessage = (g) => {
		if (g.data && g.data.type === "run-dsp") try {
			const { measTimeDomain: I, refTimeDomain: A, BINS: C, FFT_SIZE: B, metrics: E, windowType: r, weightingType: o, averagingType: t, averagingDepth: a, averagingAlpha: c, averagingThresholdDb: w, enableSourceWindow: n, sourceWindowWidthMs: h, sourceWindowOffsetMs: D, sampleRate: l, compensationDelaySamples: R, autoDelayCompensation: f, inputGain: N, displayOffset: M, polarity: d, calibrationGain: b, inputFilter: K, besselSpeed: W, ppoSmoothing: T, fftOverlap: _ } = g.data, q = l || 48e3;
			if (!I || !A) return;
			B && B !== Qg && wB(B), (C !== GA || B !== aI) && (GA = C, aI = B, UA = new Float32Array(B), HA = new Float32Array(B), qA = new Float32Array(B), pA = new Float32Array(B), lA = new Float32Array(C), fA = new Float32Array(C), Eg = new Float32Array(B), ig = new Float32Array(B), rg = new Float32Array(B), tg = new Float32Array(B), vA = new Float32Array(C), XA = new Float32Array(C), OA = new Float32Array(C), EI = new Float32Array(C), TA = new Float32Array(B), iI = new Float32Array(B), yI = new Float32Array(C), rI = new Float32Array(C), PA = new Float32Array(C), WA = new Float32Array(C), xA = new Float32Array(C), ag(C, a || MI), uA = new Mg(C, a || 16), VA = null), uA && uA.setDepth(a || 16);
			const p = new Set(E);
			let Y = new Float32Array(I), H = new Float32Array(A);
			const u = _ || 0;
			if (u > 0 && B > 0) {
				(!nI || hg !== B) && (nI = new Float32Array(B), mI = new Float32Array(B), sg = new Float32Array(B), Dg = new Float32Array(B), hg = B);
				const y = u / 100, U = Math.round(B * y), rA = B - U, hA = sg, m = Dg;
				hA.set(nI.subarray(rA), 0), m.set(mI.subarray(rA), 0), hA.set(Y.subarray(0, rA), U), m.set(H.subarray(0, rA), U), nI.set(Y), mI.set(H), Y = hA, H = m;
			}
			const IA = LI(H), O = LI(Y);
			if (R && R > 0 && FB(H, R), N && N !== 0) {
				const y = Math.pow(10, N / 20);
				for (let U = 0; U < B; U++) Y[U] *= y;
			}
			if (d) for (let y = 0; y < B; y++) Y[y] = -Y[y];
			K && K !== "None" ? ((!ZA || vI !== K || ng !== q) && (vI = K, ng = q, ZA = KI(K, q), oI = KI(K, q)), ZA && ZA.process(Y), oI && oI.process(H)) : ZA && (ZA = null, oI = null, vI = "None");
			const j = r || "Hann";
			j !== "Rectangular" && (og.apply(Y, j), og.apply(H, j));
			let $ = 0, AA = 0;
			for (let y = 0; y < B; y++) $ += Y[y], AA += H[y];
			$ /= B, AA /= B;
			for (let y = 0; y < B; y++) Y[y] -= $, H[y] -= AA;
			if (JI(H, qA, pA), JI(Y, UA, HA), p.has("Spectrum")) {
				for (let y = 0; y < C; y++) {
					const U = Math.sqrt(UA[y] * UA[y] + HA[y] * HA[y]);
					PA[y] = 20 * Math.log10(U / B * Math.SQRT2 + 1e-12);
				}
				if (M && M !== 0) for (let y = 0; y < C; y++) PA[y] += M;
			}
			const J = p.has("Magnitude") || p.has("Impulse") || p.has("Step"), tA = p.has("Phase") || p.has("Group Delay"), QA = p.has("Impulse") || p.has("Step");
			if (J && Fg(UA, HA, qA, pA, vA, lA, fA), t !== "None" && J) {
				if (t === "FIFO" && uA) {
					uA.processFIFO(lA, fA, WA, xA, w), lA.set(WA), fA.set(xA);
					for (let y = 0; y < C; y++) {
						const U = Math.sqrt(lA[y] * lA[y] + fA[y] * fA[y]);
						vA[y] = 20 * Math.log10(U + 1e-8);
					}
				} else if (t === "EMA" && uA) {
					uA.processLPF(lA, fA, WA, xA, c || .1), lA.set(WA), fA.set(xA);
					for (let y = 0; y < C; y++) {
						const U = Math.sqrt(lA[y] * lA[y] + fA[y] * fA[y]);
						vA[y] = 20 * Math.log10(U + 1e-8);
					}
				} else if (t === "LPF") try {
					VA || (VA = new vg(C, W || "Medium")), VA.setFrequency(W || "Medium"), VA.process(lA, fA, WA, xA), lA.set(WA), fA.set(xA);
					for (let y = 0; y < C; y++) {
						const U = Math.sqrt(lA[y] * lA[y] + fA[y] * fA[y]);
						vA[y] = 20 * Math.log10(U + 1e-8);
					}
				} catch {}
			}
			if (M && M !== 0 && J) for (let y = 0; y < C; y++) vA[y] += M;
			if (b) {
				const y = new Float32Array(b);
				if (J) for (let U = 0; U < C; U++) vA[U] += y[U];
				if (p.has("Spectrum")) for (let U = 0; U < C; U++) PA[U] += y[U];
			}
			if (tA && Rg(UA, HA, qA, pA, XA), lB(qA, pA, UA, HA, C), fB(OA, C), Math.random() < .05) {
				let y = 0, U = 0, rA = 0;
				for (let V = 0; V < C; V++) y += qA[V] * qA[V] + pA[V] * pA[V], U += UA[V] * UA[V] + HA[V] * HA[V], rA += OA[V];
				const hA = H[0] !== void 0 ? H.slice(0, 5).join(",") : "N/A", m = Y[0] !== void 0 ? Y.slice(0, 5).join(",") : "N/A";
				console.log(`[DSP-DIAG] refFFTrms=${Math.sqrt(y / C).toFixed(2)} measFFTrms=${Math.sqrt(U / C).toFixed(2)} cohMean=${(rA / C).toFixed(3)} ref[0:5]=${hA} meas[0:5]=${m} sameData=${H[0] === Y[0] && H[100] === Y[100]}`);
			}
			if (QA && (Gg(UA, HA, qA, pA, TA, Eg, ig, rg, tg), n && kg(TA, h, D, q)), p.has("Step") && Ng(TA, iI, q), p.has("Group Delay")) {
				for (let y = 0; y < C; y++) yI[y] = XA[y] * Math.PI / 180;
				yg(yI, q / 2 / C, EI);
			}
			const eA = O.peakDb - O.rmsDb;
			rI.fill(Math.max(0, Math.min(30, eA)));
			let x = 0;
			if (f && QA) {
				let y = 0;
				for (let U = 0; U < TA.length; U++) {
					const rA = Math.abs(TA[U]);
					rA > y && (y = rA, x = U);
				}
			}
			T && T > 0 && (J && DI(vA, C, q, T), tA && bg(XA, C, q, T), DI(OA, C, q, T), p.has("Spectrum") && DI(PA, C, q, T));
			const CA = vA.buffer, EA = XA.buffer, gA = OA.buffer, BA = EI.buffer, iA = TA.buffer, nA = iI.buffer, oA = rI.buffer, sA = lA.buffer, DA = fA.buffer, wA = PA.buffer;
			self.postMessage({
				type: "dsp-results",
				outputMagnitude: CA,
				outputPhase: EA,
				outputCoherence: gA,
				outputGroupDelay: BA,
				outputImpulse: iA,
				outputStep: nA,
				outputCrestFactor: oA,
				outputSpectrum: wA,
				hReal: sA,
				hImag: DA,
				refPeakDb: IA.peakDb,
				refRmsDb: IA.rmsDb,
				measPeakDb: O.peakDb,
				measRmsDb: O.rmsDb,
				detectedDelaySamples: x
			}, [
				CA,
				EA,
				gA,
				BA,
				iA,
				nA,
				oA,
				wA,
				sA,
				DA
			]), vA = new Float32Array(GA), XA = new Float32Array(GA), OA = new Float32Array(GA), EI = new Float32Array(GA), TA = new Float32Array(aI), iI = new Float32Array(aI), rI = new Float32Array(GA), PA = new Float32Array(GA), lA = new Float32Array(GA), fA = new Float32Array(GA);
		} catch (I) {
			console.error("[dspWorker] Error in run-dsp:", I);
		}
		g.data && g.data.type === "reset-averaging" && (GA > 0 && ag(GA, MI), uA && uA.reset(), VA && VA.reset());
	};
})();
