(function() {
	var uI = Object.defineProperty, eA = (g, I) => () => (g && (I = g(g = 0)), I), cg = (g, I) => {
		let A = {};
	...
	var pI, Jg = eA((() => {
		pI = (() => {
			...
			return (function(I = {}) {
				...
				function T() {
					if (A.postRun) for (typeof A.postRun == "function" && (A.postRun = [A.postRun]); A.postRun.length;) q(A.postRun.shift());
					x(b);
				}
				function z(Q) {
					M.unshift(Q);
				...
				function rA() {
					...
					function e(s, k) {
						var d = s.exports;
						return D = d, h = D.c, N(), D.j, p(D.d), X("wasm-instantiate"), d;
					}
	...
	})), dA, hI, qI, cI, TI, Lg = eA((() => {
		Jg(), dA = pI({}), hI = dA.cwrap("kiss_fft_alloc", "number", [
			"number",
			"number",
			"number",
			"number"
		]), qI = dA.cwrap("kiss_fft", "void", [
			"number",
			"number",
			"number"
			...
			fft = function(g) {
				...
				this.cin.set(g), qI(this.fcfg, this.inptr, I);
		...
		NA.prototype.fromComplexArray = function(I, A) {
			...
			for (C >>= 2; C >= 2; C >>= 2) {
				...
				for (E = 0; E < A; E += B) for (var w = E + c, n = E, h = 0; n < w; n += 2, h += C) {
					const D = n, l = D + c, R = l + c, f = R + c, N = I[D], M = I[D + 1], Y = I[l], b = I[l + 1], K = I[R], W = I[R + 1], T = I[f], z = I[f + 1], p = N, q = M, S = a[h], v = t * a[h + 1], u = Y * S - b * v, AA = Y * v + b * S, X = a[2 * h], V = t * a[2 * h + 1], _ = K * X - W * V, $ = K * V + W * X, J = a[3 * h], iA = t * a[3 * h + 1], CA = T * J - z * iA, rA = T * iA + z * J, x = p + _, BA = q + $, QA = p - _, IA = q - $, gA = u + CA, EA = AA + rA, nA = t * (u - CA), aA = t * (AA - rA), sA = x + gA, DA = BA + EA, cA = x - gA, y = BA - EA, H = QA + aA, tA = IA - nA, wA = QA - aA, m = IA + nA;
			...
			const B = this._out, E = this._data, r = this._inv ? -1 : 1, o = C * 2, t = C * 3, a = E[A], c = E[A + 1], w = E[A + C], n = E[A + C + 1], h = E[A + o], D = E[A + o + 1], l = E[A + t], R = E[A + t + 1], f = a + h, N = c + D, M = a - h, Y = c - D, b = w + l, K = n + R, W = r * (w - l), T = r * (n - R), z = f + b, p = N + K, q = M + T, S = Y - W, v = f - b, u = N - K, AA = M - T, X = Y + W;
			B[I] = z, B[I + 1] = p, B[I + 2] = q, B[I + 3] = S, B[I + 4] = v, B[I + 5] = u, B[I + 6] = AA, B[I + 7] = X;
			...
			for (C >>= 2; C >= 2; C >>= 2) {
				...
				for (E = 0; E < A; E += B) for (var h = 0, D = 0; h <= n; h += 2, D += C) {
					var l = E + h, R = l + w, f = R + w, N = f + w, M = I[l], Y = I[l + 1], b = I[R], K = I[R + 1], W = I[f], T = I[f + 1], z = I[N], p = I[N + 1], q = M, S = Y, v = a[D], u = t * a[D + 1], AA = b * v - K * u, X = b * u + K * v, V = a[2 * D], _ = t * a[2 * D + 1], $ = W * V - T * _, J = W * _ + T * V, iA = a[3 * D], CA = t * a[3 * D + 1], rA = z * iA - p * CA, x = z * CA + p * iA, BA = q + $, QA = S + J, IA = q - $, gA = S - J, EA = AA + rA, nA = X + x, aA = t * (AA - rA), sA = t * (X - x), DA = BA + EA, cA = QA + nA, y = IA + sA, H = gA - aA;
		...
		PI = (() => {
			...
			return (function(I = {}) {
				...
				function rA() {
					...
					function Q(e, s) {
						var k = e.exports;
						return D = k, h = D.b, N(), D.e, p(D.c), X("wasm-instantiate"), k;
					}
					if (AA("wasm-instantiate"), A.instantiateWasm) try {
	...
	var JA, WI, Pg = eA((() => {
		qg(), JA = PI({}), WI = JA.cwrap("fftCross", "void", [
			"number",
			"number",
			"number",
			"number",
		...
		jI = (() => {
			...
			return (function(I = {}) {
				...
				function rA() {
					...
					function Q(e, s) {
						var k = e.exports;
						return D = k, h = D.b, N(), D.l, p(D.c), X("wasm-instantiate"), k;
					}
					if (AA("wasm-instantiate"), A.instantiateWasm) try {
						return A.instantiateWasm(i, Q);
					} catch (e) {
		...
		gI || (gI = {}), (function(g) {
			...
			function I(o, t, a, c, w, n) {
				for (var h = w.twiddle, D = 0; D < n; D++) {
					var l = o[2 * (t + a * D)], R = o[2 * (t + a * D) + 1], f = o[2 * (t + a * (D + n))], N = o[2 * (t + a * (D + n)) + 1], M = h[2 * (0 + c * D)], Y = h[2 * (0 + c * D) + 1], b = f * M - N * Y, K = f * Y + N * M, W = l + b, T = R + K, z = l - b, p = R - K;
					o[2 * (t + a * D)] = W, o[2 * (t + a * D) + 1] = T, o[2 * (t + a * (D + n))] = z, o[2 * (t + a * (D + n)) + 1] = p;
				}
			}
			function A(o, t, a, c, w, n) {
				for (var h = w.twiddle, D = n, l = 2 * n, R = c, f = 2 * c, N = h[2 * (0 + c * n) + 1], M = 0; M < n; M++) {
					var Y = o[2 * (t + a * M)], b = o[2 * (t + a * M) + 1], K = o[2 * (t + a * (M + D))], W = o[2 * (t + a * (M + D)) + 1], T = h[2 * (0 + R * M)], z = h[2 * (0 + R * M) + 1], p = K * T - W * z, q = K * z + W * T, S = o[2 * (t + a * (M + l))], v = o[2 * (t + a * (M + l)) + 1], u = h[2 * (0 + f * M)], AA = h[2 * (0 + f * M) + 1], X = S * u - v * AA, V = S * AA + v * u, _ = p + X, $ = q + V, J = Y + _, iA = b + $;
					o[2 * (t + a * M)] = J, o[2 * (t + a * M) + 1] = iA;
					var CA = Y - _ * .5, rA = b - $ * .5, x = (p - X) * N, BA = (q - V) * N, QA = CA - BA, IA = rA + x;
			...
			function C(o, t, a, c, w, n) {
				for (var h = w.twiddle, D = n, l = 2 * n, R = 3 * n, f = c, N = 2 * c, M = 3 * c, Y = 0; Y < n; Y++) {
					var b = o[2 * (t + a * Y)], K = o[2 * (t + a * Y) + 1], W = o[2 * (t + a * (Y + D))], T = o[2 * (t + a * (Y + D)) + 1], z = h[2 * (0 + f * Y)], p = h[2 * (0 + f * Y) + 1], q = W * z - T * p, S = W * p + T * z, v = o[2 * (t + a * (Y + l))], u = o[2 * (t + a * (Y + l)) + 1], AA = h[2 * (0 + N * Y)], X = h[2 * (0 + N * Y) + 1], V = v * AA - u * X, _ = v * X + u * AA, $ = o[2 * (t + a * (Y + R))], J = o[2 * (t + a * (Y + R)) + 1], iA = h[2 * (0 + M * Y)], CA = h[2 * (0 + M * Y) + 1], rA = $ * iA - J * CA, x = $ * CA + J * iA, BA = b + V, QA = K + _, IA = b - V, gA = K - _, EA = q + rA, nA = S + x, aA = q - rA, sA = S - x, DA = BA + EA, cA = QA + nA;
			...
			function B(o, t, a, c, w, n, h) {
				for (var D = w.twiddle, l = w.n, R = new Float64Array(2 * h), f = 0; f < n; f++) {
					...
					for (var N = 0, M = f; N < h; N++, M += n) {
						...
						for (var W = 1; W < h; W++) {
							K = (K + c * M) % l;
							var T = o[2 * (t + a * M)], z = o[2 * (t + a * M) + 1], p = R[2 * W], q = R[2 * W + 1], S = D[2 * K], v = D[2 * K + 1], u = p * S - q * v, AA = p * v + q * S, X = T + u, V = z + AA;
		...
		Ig = (() => {
			...
			return (function(I = {}) {
				...
				function rA() {
					...
					function Q(e, s) {
						var k = e.exports;
						return D = k, h = D.b, N(), D.i, p(D.c), X("wasm-instantiate"), k;
					}
					if (AA("wasm-instantiate"), A.instantiateWasm) try {
						return A.instantiateWasm(i, Q);
					} catch (e) {
		...
		QI.prototype.fft = function(I) {
			...
			for (C >>= 2; C >= 2; C >>= 2) {
				...
				for (E = 0; E < A; E += B) for (var a = E + t, c = E, w = 0; c < a; c += 2, w += C) {
					const n = c, h = n + t, D = h + t, l = D + t, R = this._out[n], f = this._out[n + 1], N = this._out[h], M = this._out[h + 1], Y = this._out[D], b = this._out[D + 1], K = this._out[l], W = this._out[l + 1], T = R, z = f, p = this.table[w], q = this.table[w + 1], S = N * p - M * q, v = N * q + M * p, u = this.table[2 * w], AA = this.table[2 * w + 1], X = Y * u - b * AA, V = Y * AA + b * u, _ = this.table[3 * w], $ = this.table[3 * w + 1], J = K * _ - W * $, iA = K * $ + W * _, CA = T + X, rA = z + V, x = T - X, BA = z - V, QA = S + J, IA = v + iA, gA = S - J, EA = v - iA;
	...
	})), hB = cg({ default: () => Cg }), RI, Cg, cB = eA((() => {
		Lg(), pg(), Wg(), jg(), zg(), $g(), rB(), oB(), sB(), DB(), RI = [
			4,
			8,
			16,
			32,
	...
	let LA, KA, VA, jA, lA, fA, Eg, ig, rg, tg, UA, XA, ZA, EI, pA, iI, yI, rI, qA, TA, PA;
	const MI = 21;
	...
	let GA = 0, aI = 0, vA = null;
	const og = new Yg();
	let OA = null, oI = null, vI = "None", ng = 0, WA = null, nI = null, mI = null, sg = null, Dg = null, hg = 0;
	function FB(g, I) {
	...
	self.onmessage = (g) => {
		if (g.data && g.data.type === "run-dsp") try {
			const { measTimeDomain: I, refTimeDomain: A, BINS: C, FFT_SIZE: B, metrics: E, windowType: r, weightingType: o, averagingType: t, averagingDepth: a, averagingAlpha: c, averagingThresholdDb: w, enableSourceWindow: n, sourceWindowWidthMs: h, sourceWindowOffsetMs: D, sampleRate: l, compensationDelaySamples: R, autoDelayCompensation: f, inputGain: N, displayOffset: M, polarity: Y, calibrationGain: b, inputFilter: K, besselSpeed: W, ppoSmoothing: T, fftOverlap: z } = g.data, p = l || 48e3;
			if (!I || !A) return;
			B && B !== Qg && wB(B), (C !== GA || B !== aI) && (GA = C, aI = B, LA = new Float32Array(B), KA = new Float32Array(B), VA = new Float32Array(B), jA = new Float32Array(B), lA = new Float32Array(C), fA = new Float32Array(C), Eg = new Float32Array(B), ig = new Float32Array(B), rg = new Float32Array(B), tg = new Float32Array(B), UA = new Float32Array(C), XA = new Float32Array(C), ZA = new Float32Array(C), EI = new Float32Array(C), pA = new Float32Array(B), iI = new Float32Array(B), yI = new Float32Array(C), rI = new Float32Array(C), qA = new Float32Array(C), TA = new Float32Array(C), PA = new Float32Array(C), ag(C, a || MI), vA = new Mg(C, a || 16), WA = null), vA && vA.setDepth(a || 16);
			const q = new Set(E);
			...
			if (u > 0 && B > 0) {
				...
				const y = u / 100, H = Math.round(B * y), tA = B - H, wA = sg, m = Dg;
				wA.set(nI.subarray(tA), 0), m.set(mI.subarray(tA), 0), wA.set(S.subarray(0, tA), H), m.set(v.subarray(0, tA), H), nI.set(S), mI.set(v), S = wA, v = m;
			}
			const AA = LI(v), X = LI(S);
			if (R && R > 0 && FB(v, R), N && N !== 0) {
				const y = Math.pow(10, N / 20);
				for (let H = 0; H < B; H++) S[H] *= y;
			}
			if (Y) for (let y = 0; y < B; y++) S[y] = -S[y];
			K && K !== "None" ? ((!OA || vI !== K || ng !== p) && (vI = K, ng = p, OA = KI(K, p), oI = KI(K, p)), OA && OA.process(S), oI && oI.process(v)) : OA && (OA = null, oI = null, vI = "None");
			const V = r || "Hann";
			V !== "Rectangular" && (og.apply(S, V), og.apply(v, V));
			let _ = 0, $ = 0;
			for (let y = 0; y < B; y++) _ += S[y], $ += v[y];
			_ /= B, $ /= B;
			for (let y = 0; y < B; y++) S[y] -= _, v[y] -= $;
			if (JI(v, VA, jA), JI(S, LA, KA), q.has("Spectrum")) {
				for (let y = 0; y < C; y++) {
					const H = Math.sqrt(LA[y] * LA[y] + KA[y] * KA[y]);
					qA[y] = 20 * Math.log10(H / B * Math.SQRT2 + 1e-12);
				}
				if (M && M !== 0) for (let y = 0; y < C; y++) qA[y] += M;
			}
			const J = q.has("Magnitude") || q.has("Impulse") || q.has("Step"), iA = q.has("Phase") || q.has("Group Delay"), CA = q.has("Impulse") || q.has("Step");
			if (J && Fg(LA, KA, VA, jA, UA, lA, fA), t !== "None" && J) {
				if (t === "FIFO" && vA) {
					...
					WA || (WA = new vg(C, W || "Medium")), WA.setFrequency(W || "Medium"), WA.process(lA, fA, TA, PA), lA.set(TA), fA.set(PA);
					for (let y = 0; y < C; y++) {
						const H = Math.sqrt(lA[y] * lA[y] + fA[y] * fA[y]);
						UA[y] = 20 * Math.log10(H + 1e-8);
					}
			...
			if (b) {
				...
				if (q.has("Spectrum")) for (let H = 0; H < C; H++) qA[H] += y[H];
			}
			if (iA && Rg(LA, KA, VA, jA, XA), lB(VA, jA, LA, KA, C), fB(ZA, C), CA && (Gg(LA, KA, VA, jA, pA, Eg, ig, rg, tg), n && kg(pA, h, D, p)), q.has("Step") && Ng(pA, iI, p), q.has("Group Delay")) {
				for (let y = 0; y < C; y++) yI[y] = XA[y] * Math.PI / 180;
				yg(yI, p / 2 / C, EI);
			}
			const rA = X.peakDb - X.rmsDb;
			rI.fill(Math.max(0, Math.min(30, rA)));
			let x = 0;
			if (f && CA) {
				let y = 0;
				for (let H = 0; H < pA.length; H++) {
					const tA = Math.abs(pA[H]);
					tA > y && (y = tA, x = H);
				}
			}
			T && T > 0 && (J && DI(UA, C, p, T), iA && bg(XA, C, p, T), DI(ZA, C, p, T), q.has("Spectrum") && DI(qA, C, p, T));
			const BA = UA.buffer, QA = XA.buffer, IA = ZA.buffer, gA = EI.buffer, EA = pA.buffer, nA = iI.buffer, aA = rI.buffer, sA = lA.buffer, DA = fA.buffer, cA = qA.buffer;
			self.postMessage({
				type: "dsp-results",
				outputMagnitude: BA,
				outputPhase: QA,
				...
				sA,
				DA
			]), UA = new Float32Array(GA), XA = new Float32Array(GA), ZA = new Float32Array(GA), EI = new Float32Array(GA), pA = new Float32Array(aI), iI = new Float32Array(aI), rI = new Float32Array(GA), qA = new Float32Array(GA), lA = new Float32Array(GA), fA = new Float32Array(GA);
		} catch (I) {
			console.error("[dspWorker] Error in run-dsp:", I);
		}
		g.data && g.data.type === "reset-averaging" && (GA > 0 && ag(GA, MI), vA && vA.reset(), WA && WA.reset());
	};
})();
