(function() {
	...
	function wI(E, I, A) {
		const i = E.length, B = Math.log2(i);
		for (let Q = 0; Q < i; Q++) {
			const r = XI(Q, B);
			if (r > Q) {
				const o = E[Q], t = I[Q];
	...
	function fI(E, I, A) {
		const i = E.length, B = I || new Float32Array(i), Q = A || new Float32Array(i);
		return B.set(E), Q.fill(0), wI(B, Q, !1), {
			real: B,
			imag: Q
		};
	}
	function ZI(E, I, A, i) {
		const B = E.length, Q = A || new Float32Array(B), r = i || new Float32Array(B);
		return Q.set(E), r.set(I), wI(Q, r, !0), Q;
	}
	function OI(E, I, A, i, B, Q, r) {
		const o = B.length;
		for (let t = 0; t < o; t++) {
	...
	function zI(E, I, A, i, B) {
		const Q = B.length;
		for (let r = 0; r < Q; r++) {
			const o = A[r] * A[r] + i[r] * i[r] + 1e-12, t = (E[r] * A[r] + I[r] * i[r]) / o, e = (I[r] * A[r] - E[r] * i[r]) / o;
			B[r] = Math.atan2(e, t) * (180 / Math.PI);
		}
	}
	function _I(E, I, A = 48e3) {
		let i = 0;
		const B = E.length, Q = 1 / A;
		for (let r = 0; r < B; r++) i += E[r] * Q * 1e3, I[r] = i;
	}
	function $I(E, I, A) {
		const i = A.length;
		A[0] = 0;
	...
	var Ag = class {
		...
		processFIFO(E, I, A, i) {
			...
			for (let B = 0; B < this.count; B++) for (let Q = 0; Q < this.bins; Q++) A[Q] += this.bufferReal[B][Q], i[Q] += this.bufferImag[B][Q];
			for (let B = 0; B < this.bins; B++) A[B] /= this.count, i[B] /= this.count;
		}
		processLPF(E, I, A, i, B) {
			for (let Q = 0; Q < this.bins; Q++) this.lpfReal[Q] += (E[Q] - this.lpfReal[Q]) * B, this.lpfImag[Q] += (I[Q] - this.lpfImag[Q]) * B, A[Q] = this.lpfReal[Q], i[Q] = this.lpfImag[Q];
		}
		setDepth(E) {
	...
	function gg(E, I, A, i = 48e3) {
		const B = E.length, Q = Math.round(A / 1e3 * i), r = Math.round(I / 2 / 1e3 * i), o = Math.max(0, Q - r), t = Math.min(B - 1, Q + r), e = Math.round(r * .2);
		for (let c = 0; c < B; c++) if (c < o || c > t) E[c] = 0;
		else if (c < o + e) {
	...
	function cA(E) {
		...
		this.table = I;
		for (var i = 0, B = 1; this.size > B; B <<= 1) i++;
	...
	var Eg = iA((() => {
		cA.prototype.fromComplexArray = function(I, A) {
			for (var i = A || new Array(I.length >>> 1), B = 0; B < I.length; B += 2) i[B >>> 1] = I[B];
			return i;
			...
			for (var i = A || this.createComplexArray(), B = 0; B < i.length; B += 2) i[B] = I[B >>> 1], i[B + 1] = 0;
			return i;
		}, cA.prototype.completeSpectrum = function(I) {
			for (var A = this._csize, i = A >>> 1, B = 2; B < i; B += 2) I[A - B] = I[B], I[A - B + 1] = -I[B + 1];
			...
			var I = this._out, A = this._csize, i = 1 << this._width, B = A / i << 1, Q, r, o = this._bitrev;
			if (B === 4) for (Q = 0, r = 0; Q < A; Q += B, r++) {
				const D = o[r];
				this._singleTransform2(Q, D, i);
			}
			else for (Q = 0, r = 0; Q < A; Q += B, r++) {
				const D = o[r];
			...
			for (i >>= 2; i >= 2; i >>= 2) {
				B = A / i << 1;
				var c = B >>> 2;
				for (Q = 0; Q < A; Q += B) for (var w = Q + c, s = Q, h = 0; s < w; s += 2, h += i) {
			...
			const B = this._out, Q = this._data, r = Q[A], o = Q[A + 1], t = Q[A + i], e = Q[A + i + 1], c = r + t, w = o + e, s = r - t, h = o - e;
			B[I] = c, B[I + 1] = w, B[I + 2] = s, B[I + 3] = h;
		}, cA.prototype._singleTransform4 = function(I, A, i) {
			const B = this._out, Q = this._data, r = this._inv ? -1 : 1, o = i * 2, t = i * 3, e = Q[A], c = Q[A + 1], w = Q[A + i], s = Q[A + i + 1], h = Q[A + o], D = Q[A + o + 1], f = Q[A + t], N = Q[A + t + 1], F = e + h, l = c + D, y = e - h, Y = c - D, v = w + f, b = s + N, K = r * (w - f), T = r * (s - N), Z = F + v, W = l + b, O = y + T, d = Y - K, L = F - v, H = l - b, z = y - T, _ = Y + K;
			B[I] = Z, B[I + 1] = W, B[I + 2] = O, B[I + 3] = d, B[I + 4] = L, B[I + 5] = H, B[I + 6] = z, B[I + 7] = _;
			...
			for (i >>= 2; i >= 2; i >>= 2) {
				...
				for (Q = 0; Q < A; Q += B) for (var h = 0, D = 0; h <= s; h += 2, D += i) {
					...
					if (h !== s) {
						var u = IA, rA = -BA, HA = CA, GA = -QA, UA = -t * oA, vA = -t * tA, aA = -t * eA, hA = -t * EA, C = u + UA, g = rA + vA, a = HA + hA, n = GA - aA, G = Q + w - h, k = Q + c - h;
						I[G] = C, I[G + 1] = g, I[k] = a, I[k + 1] = n;
					}
				}
			}
		}, cA.prototype._singleRealTransform2 = function(I, A, i) {
			const B = this._out, Q = this._data, r = Q[A], o = Q[A + i], t = r + o, e = r - o;
			B[I] = t, B[I + 1] = 0, B[I + 2] = e, B[I + 3] = 0;
		}, cA.prototype._singleRealTransform4 = function(I, A, i) {
			const B = this._out, Q = this._data, r = this._inv ? -1 : 1, o = i * 2, t = i * 3, e = Q[A], c = Q[A + i], w = Q[A + o], s = Q[A + t], h = e + w, D = e - w, f = c + s, N = r * (c - s), F = h + f, l = D, y = -N, Y = h - f, v = D, b = N;
			B[I] = F, B[I + 1] = 0, B[I + 2] = l, B[I + 3] = y, B[I + 4] = Y, B[I + 5] = 0, B[I + 6] = v, B[I + 7] = b;
		};
	})), aI, ig = iA((() => {
		Eg(), aI = class {
			constructor(E) {
		...
		yI = (() => {
			...
			return (function(I = {}) {
				...
				AA(J) || (J = e(J));
				function gA(C) {
					if (C == J && s) return new Uint8Array(s);
					var g = vA(C);
					if (g) return g;
					if (c) return c(C);
					throw "sync fetching of the wasm failed: you can preload it to Module['wasmBinary'] manually, or emcc.py will do that for you when generating HTML (but not JS)";
				...
				function V() {
					...
					if (z("wasm-instantiate"), A.instantiateWasm) try {
						...
						w("Module.instantiateWasm callback failed with error: " + a), B(a);
					}
					return g(S(J, C)[0]);
				}
				var p = (C) => {
				...
				var BA = (C, g) => {
					...
					for (var k = ""; g < G;) {
						...
						var U = C[g++] & 63;
						if ((M & 240) == 224 ? M = (M & 15) << 12 | R << 6 | U : M = (M & 7) << 18 | R << 12 | U << 6 | C[g++] & 63, M < 65536) k += String.fromCharCode(M);
						else {
							var P = M - 65536;
					...
					return k;
				}, FA = (C, g) => C ? fA(F, C, g) : "", nA = function(C, g, a, n, G) {
					var k = {
						string: (m) => {
							var X = 0;
				...
				function vA(C) {
					if (AA(C)) return UA(C.slice($.length));
				}
	...
	function tg(E) {
		this.size = E, this.n = E * 8, this.ptr = uA._malloc(this.n * 4), this.ri = new Uint8Array(uA.HEAPU8.buffer, this.ptr, this.n), this.ii = new Uint8Array(uA.HEAPU8.buffer, this.ptr + this.n, this.n), this.transform = function(I, A, i) {
			var B = this.ptr, Q = this.n;
			return this.ri.set(new Uint8Array(I.buffer)), this.ii.set(new Uint8Array(A.buffer)), MI(this.size, i, B, B + Q, B + Q * 2, B + Q * 3), {
				real: new Float64Array(uA.HEAPU8.buffer, B + Q * 2, this.size),
				imag: new Float64Array(uA.HEAPU8.buffer, B + Q * 3, this.size)
			};
	...
	function og(E) {
		...
		this.forward = function(A, i) {
			...
			for (var t = 2; t <= B; t *= 2) for (var e = t / 2, c = B / t, Q = 0; Q < B; Q += t) for (var r = Q, w = 0; r < Q + e; r++, w += c) {
	...
	var ng = iA((() => {})), YI, sg = iA((() => {
		ng(), YI = class {
			...
			fft(E) {
				...
				for (var B = 0; B < this.size; ++B) I[B] = E[B * 2], A[B] = E[B * 2 + 1];
				this.fftNayuki.forward(I, A);
				for (var B = 0; B < this.size; ++B) i[B * 2] = I[B], i[B * 2 + 1] = A[B];
				return i;
			}
		};
	})), kI, Dg = iA((() => {
	...
	function Ng(E, I) {
		...
		for (let Q = 0; Q < q; Q++) {
			B = Q * q;
			for (let r = 0; r < q; r++) A[r] = E[r + B], i[r] = I[r + B];
			_A(A, i);
			for (let r = 0; r < q; r++) E[r + B] = A[r], I[r + B] = i[r];
		}
		for (let Q = 0; Q < q; Q++) {
			for (let r = 0; r < q; r++) B = Q + r * q, A[r] = E[B], i[r] = I[B];
			_A(A, i);
			for (let r = 0; r < q; r++) B = Q + r * q, E[B] = A[r], I[B] = i[r];
		}
	}
	function yg(E, I) {
		let A = [], i = [], B = 0;
	...
	function Yg() {
		let E = q >> 1, I = q >> 2, A = q >> 3, i = E + I, B = Math.sin(Math.PI / q), Q = 2 * B * B, r = Math.sqrt(Q * (2 - Q)), o = wA[I] = 1, t = wA[0] = 0;
		B = 2 * Q;
		for (let e = 1; e < A; e++) o -= Q, Q += B * o, t += r, r -= B * t, wA[e] = t, wA[I - e] = o;
	...
	async function Hg() {
		let E = "Other", I = "Unknown", A = "Other", i = "Unknown", B = navigator.userAgentData, Q = navigator.userAgent;
		try {
			if (B) {
	...
	var bg = iA((() => {
		AI.prototype.fft = function(I) {
			...
			var A = this._csize, i = 1 << this._width, B = A / i << 1, Q, r, o = this._bitrev;
			if (B === 4) for (Q = 0, r = 0; Q < A; Q += B, r++) {
				const s = o[r];
			...
			for (i >>= 2; i >= 2; i >>= 2) {
				B = A / i << 1;
				var t = B >>> 2;
				for (Q = 0; Q < A; Q += B) for (var e = Q + t, c = Q, w = 0; c < e; c += 2, w += i) {
			...
			const B = this._data[A], Q = this._data[A + 1], r = this._data[A + i], o = this._data[A + i + 1];
			this._out[I] = B + r, this._out[I + 1] = Q + o, this._out[I + 2] = B - r, this._out[I + 3] = Q - o;
		}, AI.prototype._singleTransform4 = function(I, A, i) {
			const B = i * 2, Q = i * 3, r = this._data[A], o = this._data[A + 1], t = this._data[A + i], e = this._data[A + i + 1], c = this._data[A + B], w = this._data[A + B + 1], s = this._data[A + Q], h = this._data[A + Q + 1], D = r + c, f = o + w, N = r - c, F = o - w, l = t + s, y = e + h, Y = t - s, v = e - h;
		...
		Qg(), ig(), eg(), sg(), wg(), Fg(), dg(), vg(), ug(), bg(), sI = [
			...
			fftr(E) {
				...
				const B = new Float32Array(2 * i);
				B.fill(0);
				for (let Q = 0; Q < i; Q++) B[2 * Q] = E[Q];
				return I = A.fft(B), I.slice(i, i * 2);
			}
			fft2d(E) {
	...
	function pg(E, I) {
		for (let A = 0; A < I; A++) {
			const i = jA[A] * jA[A] + XA[A] * XA[A], B = QI[A] * EI[A] + 1e-12;
			E[A] = Math.min(1, Math.max(0, i / B));
		}
	}
	function Tg(E, I) {
		...
		B.set(E.subarray(0, i)), E.copyWithin(0, i), E.set(B, A - i);
	}
	self.onmessage = (E) => {
		if (E.data && E.data.type === "run-dsp") {
			const { measTimeDomain: I, refTimeDomain: A, BINS: i, FFT_SIZE: B, metrics: Q, windowType: r, weightingType: o, averagingType: t, averagingDepth: e, averagingAlpha: c, enableSourceWindow: w, sourceWindowWidthMs: s, sourceWindowOffsetMs: h, sampleRate: D, compensationDelaySamples: f, autoDelayCompensation: N } = E.data, F = D || 48e3;
			if (!I || !A) return;
			B && B !== qI && Kg(B), (i !== SA || B !== iI) && (SA = i, iI = B, LA = new Float32Array(B), KA = new Float32Array(B), qA = new Float32Array(B), pA = new Float32Array(B), yA = new Float32Array(i), MA = new Float32Array(i), pI = new Float32Array(B), TI = new Float32Array(B), WI = new Float32Array(B), PI = new Float32Array(B), TA = new Float32Array(i), PA = new Float32Array(i), II = new Float32Array(i), gI = new Float32Array(i), bA = new Float32Array(B), BI = new Float32Array(B), hI = new Float32Array(i), CI = new Float32Array(i), xA = new Float32Array(i), VA = new Float32Array(i), QI = new Float32Array(i), EI = new Float32Array(i), jA = new Float32Array(i), XA = new Float32Array(i), WA = new Ag(i, e || 16)), WA && WA.setDepth(e || 16), ZA = c || .1;
