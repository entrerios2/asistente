(function() {
	...
	function fI(E, I, A) {
		const i = E.length, C = I || new Float32Array(i), B = A || new Float32Array(i);
		return C.set(E), B.fill(0), wI(C, B, !1), {
			real: C,
			imag: B
		};
	}
	function ZI(E, I, A, i) {
		const C = E.length, B = A || new Float32Array(C), r = i || new Float32Array(C);
		return B.set(E), r.set(I), wI(B, r, !0), B;
	}
	function OI(E, I, A, i, C, B, r) {
		const o = C.length;
		for (let t = 0; t < o; t++) {
	...
	function _I(E, I, A = 48e3) {
		let i = 0;
		const C = E.length, B = 1 / A;
		for (let r = 0; r < C; r++) i += E[r] * B * 1e3, I[r] = i;
	}
	function $I(E, I, A) {
		const i = A.length;
		A[0] = 0;
	...
	var Ag = class {
		...
		processFIFO(E, I, A, i, C) {
			if (C !== void 0 && C > -120) for (let B = 0; B < this.bins; B++) {
				const r = Math.sqrt(E[B] * E[B] + I[B] * I[B]);
				20 * Math.log10(r + 1e-12) < C ? (E[B] = this.lastValidReal[B], I[B] = this.lastValidImag[B]) : (this.lastValidReal[B] = E[B], this.lastValidImag[B] = I[B]);
			}
		...
		processLPF(E, I, A, i, C) {
			for (let B = 0; B < this.bins; B++) this.lpfReal[B] += (E[B] - this.lpfReal[B]) * C, this.lpfImag[B] += (I[B] - this.lpfImag[B]) * C, A[B] = this.lpfReal[B], i[B] = this.lpfImag[B];
		}
		setDepth(E) {
	...
	function gg(E, I, A, i = 48e3) {
		const C = E.length, B = Math.round(A / 1e3 * i), r = Math.round(I / 2 / 1e3 * i), o = Math.max(0, B - r), t = Math.min(C - 1, B + r), e = Math.round(r * .2);
	...
	var Bg = class {
		...
		FI = (() => {
			...
			return (function(I = {}) {
				...
				var U = 0, L = null, v = null;
				function z(g) {
					U++, A.monitorRunDependencies && A.monitorRunDependencies(U);
				}
				function _(g) {
					if (U--, A.monitorRunDependencies && A.monitorRunDependencies(U), U == 0 && (L !== null && (clearInterval(L), L = null), v)) {
						var a = v;
						v = null, a();
					}
				}
				...
				function BA() {
					...
					function a(n, G) {
						var k = n.exports;
						return D = k, h = D.c, N(), D.j, T(D.d), _("wasm-instantiate"), k;
					}
				...
				var EA = (g, a) => {
					...
					for (var k = n, M = n + G - 1, l = 0; l < g.length; ++l) {
						...
						if (S <= 127) {
							...
							if (n + 1 >= M) break;
							a[n++] = 192 | S >> 6, a[n++] = 128 | S & 63;
						} else if (S <= 65535) {
							if (n + 2 >= M) break;
							a[n++] = 224 | S >> 12, a[n++] = 128 | S >> 6 & 63, a[n++] = 128 | S & 63;
						} else {
							if (n + 3 >= M) break;
							a[n++] = 240 | S >> 18, a[n++] = 128 | S >> 12 & 63, a[n++] = 128 | S >> 6 & 63, a[n++] = 128 | S & 63;
						}
					}
					return a[n] = 0, n - k;
					...
					for (var M = ""; a < k;) {
						...
						var S = g[a++] & 63;
						if ((l & 224) == 192) {
							M += String.fromCharCode((l & 31) << 6 | S);
							continue;
						}
						var x = g[a++] & 63;
						if ((l & 240) == 224 ? l = (l & 15) << 12 | S << 6 | x : l = (l & 7) << 18 | S << 12 | x << 6 | g[a++] & 63, l < 65536) M += String.fromCharCode(l);
						else {
							var X = l - 65536;
				...
				function Q() {
					if (U > 0 || (J(), U > 0)) return;
					function g() {
						hA || (hA = !0, A.calledRun = !0, !f && (K(), i(A), A.onRuntimeInitialized && A.onRuntimeInitialized(), p()));
					}
	...
	var Eg = iA((() => {
		cA.prototype.fromComplexArray = function(I, A) {
			...
			var I = this._out, A = this._csize, i = 1 << this._width, C = A / i << 1, B, r, o = this._bitrev;
			if (C === 4) for (B = 0, r = 0; B < A; B += C, r++) {
				const D = o[r];
				this._singleTransform2(B, D, i);
			}
			else for (B = 0, r = 0; B < A; B += C, r++) {
				const D = o[r];
			...
			for (i >>= 2; i >= 2; i >>= 2) {
				C = A / i << 1;
				var c = C >>> 2;
				for (B = 0; B < A; B += C) for (var w = B + c, s = B, h = 0; s < w; s += 2, h += i) {
					const D = s, f = D + c, R = f + c, F = R + c, N = I[D], y = I[D + 1], Y = I[f], H = I[f + 1], J = I[R], K = I[R + 1], p = I[F], P = I[F + 1], T = N, O = y, U = e[h], L = t * e[h + 1], v = Y * U - H * L, z = Y * L + H * U, _ = e[2 * h], V = t * e[2 * h + 1], $ = J * _ - K * V, AA = J * V + K * _, b = e[3 * h], CA = t * e[3 * h + 1], gA = p * b - P * CA, BA = p * CA + P * b, W = T + $, d = O + AA, j = T - $, IA = O - AA, QA = v + gA, EA = z + BA, eA = t * (v - gA), tA = t * (z - BA), oA = W + QA, sA = d + EA, fA = W - QA, lA = d - EA, nA = j + tA, FA = IA - eA, NA = j - tA, u = IA + eA;
					I[D] = oA, I[D + 1] = sA, I[f] = nA, I[f + 1] = FA, I[R] = fA, I[R + 1] = lA, I[F] = NA, I[F + 1] = u;
				}
			}
		}, cA.prototype._singleTransform2 = function(I, A, i) {
			const C = this._out, B = this._data, r = B[A], o = B[A + 1], t = B[A + i], e = B[A + i + 1], c = r + t, w = o + e, s = r - t, h = o - e;
			C[I] = c, C[I + 1] = w, C[I + 2] = s, C[I + 3] = h;
		}, cA.prototype._singleTransform4 = function(I, A, i) {
			const C = this._out, B = this._data, r = this._inv ? -1 : 1, o = i * 2, t = i * 3, e = B[A], c = B[A + 1], w = B[A + i], s = B[A + i + 1], h = B[A + o], D = B[A + o + 1], f = B[A + t], R = B[A + t + 1], F = e + h, N = c + D, y = e - h, Y = c - D, H = w + f, J = s + R, K = r * (w - f), p = r * (s - R), P = F + H, T = N + J, O = y + p, U = Y - K, L = F - H, v = N - J, z = y - p, _ = Y + K;
			C[I] = P, C[I + 1] = T, C[I + 2] = O, C[I + 3] = U, C[I + 4] = L, C[I + 5] = v, C[I + 6] = z, C[I + 7] = _;
			...
			for (i >>= 2; i >= 2; i >>= 2) {
				...
				for (B = 0; B < A; B += C) for (var h = 0, D = 0; h <= s; h += 2, D += i) {
					var f = B + h, R = f + w, F = R + w, N = F + w, y = I[f], Y = I[f + 1], H = I[R], J = I[R + 1], K = I[F], p = I[F + 1], P = I[N], T = I[N + 1], O = y, U = Y, L = e[D], v = t * e[D + 1], z = H * L - J * v, _ = H * v + J * L, V = e[2 * D], $ = t * e[2 * D + 1], AA = K * V - p * $, b = K * $ + p * V, CA = e[3 * D], gA = t * e[3 * D + 1], BA = P * CA - T * gA, W = P * gA + T * CA, d = O + AA, j = U + b, IA = O - AA, QA = U - b, EA = z + BA, eA = _ + W, tA = t * (z - BA), oA = t * (_ - W), sA = d + EA, fA = j + eA, lA = IA + oA, nA = QA - tA;
					...
					if (h !== s) {
						var u = IA, rA = -QA, HA = d, GA = -j, UA = -t * oA, vA = -t * tA, aA = -t * eA, hA = -t * EA, Q = u + UA, g = rA + vA, a = HA + hA, n = GA - aA, G = B + w - h, k = B + c - h;
						I[G] = Q, I[G + 1] = g, I[k] = a, I[k + 1] = n;
					}
				}
			}
		}, cA.prototype._singleRealTransform2 = function(I, A, i) {
			const C = this._out, B = this._data, r = B[A], o = B[A + i], t = r + o, e = r - o;
			C[I] = t, C[I + 1] = 0, C[I + 2] = e, C[I + 3] = 0;
		}, cA.prototype._singleRealTransform4 = function(I, A, i) {
			const C = this._out, B = this._data, r = this._inv ? -1 : 1, o = i * 2, t = i * 3, e = B[A], c = B[A + i], w = B[A + o], s = B[A + t], h = e + w, D = e - w, f = c + s, R = r * (c - s), F = h + f, N = D, y = -R, Y = h - f, H = D, J = R;
			C[I] = F, C[I + 1] = 0, C[I + 2] = N, C[I + 3] = y, C[I + 4] = Y, C[I + 5] = 0, C[I + 6] = H, C[I + 7] = J;
		};
	})), aI, ig = iA((() => {
		Eg(), aI = class {
			constructor(E) {
		...
		yI = (() => {
			...
			return (function(I = {}) {
				...
				var c;
				(r || o) && (o ? t = self.location.href : typeof document < "u" && document.currentScript && (t = document.currentScript.src), E && (t = E), t.indexOf("blob:") !== 0 ? t = t.substr(0, t.replace(/[?#].*/, "").lastIndexOf("/") + 1) : t = "", o && (c = (Q) => {
				...
				AA(b) || (b = e(b));
				function CA(Q) {
					if (Q == b && s) return new Uint8Array(s);
					var g = vA(Q);
					if (g) return g;
					if (c) return c(Q);
					throw "sync fetching of the wasm failed: you can preload it to Module['wasmBinary'] manually, or emcc.py will do that for you when generating HTML (but not JS)";
				...
				function BA() {
					...
					function g(a, n) {
						var G = a.exports;
						return D = G, h = D.b, N(), D.e, T(D.c), _("wasm-instantiate"), G;
					}
					if (z("wasm-instantiate"), A.instantiateWasm) try {
				...
				var QA = (Q, g) => {
					...
					for (var k = ""; g < G;) {
						...
						var S = Q[g++] & 63;
						if ((M & 240) == 224 ? M = (M & 15) << 12 | l << 6 | S : M = (M & 7) << 18 | l << 12 | S << 6 | Q[g++] & 63, M < 65536) k += String.fromCharCode(M);
						else {
							var x = M - 65536;
					...
					if (n) for (var X = 0; X < n.length; X++) {
						var DA = k[a[X]];
						DA ? (x === 0 && (x = rA()), S[X] = DA(n[X])) : S[X] = n[X];
					}
					var RA = l.apply(null, S);
					function mA(m) {
						return x !== 0 && HA(x), M(m);
					}
					return RA = mA(RA), RA;
				...
				function vA(Q) {
					if (AA(Q)) return UA(Q.slice($.length));
				}
				...
				function hA() {
					if (U > 0 || (J(), U > 0)) return;
					function Q() {
						aA || (aA = !0, A.calledRun = !0, !f && (K(), i(A), A.onRuntimeInitialized && A.onRuntimeInitialized(), p()));
					}
					A.setStatus ? (A.setStatus("Running..."), setTimeout(function() {
						setTimeout(function() {
							A.setStatus("");
	...
	function tg(E) {
		this.size = E, this.n = E * 8, this.ptr = uA._malloc(this.n * 4), this.ri = new Uint8Array(uA.HEAPU8.buffer, this.ptr, this.n), this.ii = new Uint8Array(uA.HEAPU8.buffer, this.ptr + this.n, this.n), this.transform = function(I, A, i) {
			var C = this.ptr, B = this.n;
			return this.ri.set(new Uint8Array(I.buffer)), this.ii.set(new Uint8Array(A.buffer)), MI(this.size, i, C, C + B, C + B * 2, C + B * 3), {
				real: new Float64Array(uA.HEAPU8.buffer, C + B * 2, this.size),
				imag: new Float64Array(uA.HEAPU8.buffer, C + B * 3, this.size)
			};
	...
	function og(E) {
		...
		this.forward = function(A, i) {
			...
			for (var t = 2; t <= C; t *= 2) for (var e = t / 2, c = C / t, B = 0; B < C; B += t) for (var r = B, w = 0; r < B + e; r++, w += c) {
		...
		kI = (() => {
			...
			return (function(I = {}) {
				...
				function BA() {
					...
					function g(a, n) {
						var G = a.exports;
						return D = G, h = D.b, N(), D.l, T(D.c), _("wasm-instantiate"), G;
					}
					if (z("wasm-instantiate"), A.instantiateWasm) try {
						return A.instantiateWasm(Q, g);
					} catch (a) {
		...
		zA || (zA = {}), (function(E) {
			...
			function I(o, t, e, c, w, s) {
				for (var h = w.twiddle, D = 0; D < s; D++) {
					var f = o[2 * (t + e * D)], R = o[2 * (t + e * D) + 1], F = o[2 * (t + e * (D + s))], N = o[2 * (t + e * (D + s)) + 1], y = h[2 * (0 + c * D)], Y = h[2 * (0 + c * D) + 1], H = F * y - N * Y, J = F * Y + N * y, K = f + H, p = R + J, P = f - H, T = R - J;
					o[2 * (t + e * D)] = K, o[2 * (t + e * D) + 1] = p, o[2 * (t + e * (D + s))] = P, o[2 * (t + e * (D + s)) + 1] = T;
				}
			}
			function A(o, t, e, c, w, s) {
				for (var h = w.twiddle, D = s, f = 2 * s, R = c, F = 2 * c, N = h[2 * (0 + c * s) + 1], y = 0; y < s; y++) {
					var Y = o[2 * (t + e * y)], H = o[2 * (t + e * y) + 1], J = o[2 * (t + e * (y + D))], K = o[2 * (t + e * (y + D)) + 1], p = h[2 * (0 + R * y)], P = h[2 * (0 + R * y) + 1], T = J * p - K * P, O = J * P + K * p, U = o[2 * (t + e * (y + f))], L = o[2 * (t + e * (y + f)) + 1], v = h[2 * (0 + F * y)], z = h[2 * (0 + F * y) + 1], _ = U * v - L * z, V = U * z + L * v, $ = T + _, AA = O + V, b = Y + $, CA = H + AA;
					o[2 * (t + e * y)] = b, o[2 * (t + e * y) + 1] = CA;
					var gA = Y - $ * .5, BA = H - AA * .5, W = (T - _) * N, d = (O - V) * N, j = gA - d, IA = BA + W;
			...
			function i(o, t, e, c, w, s) {
				for (var h = w.twiddle, D = s, f = 2 * s, R = 3 * s, F = c, N = 2 * c, y = 3 * c, Y = 0; Y < s; Y++) {
					var H = o[2 * (t + e * Y)], J = o[2 * (t + e * Y) + 1], K = o[2 * (t + e * (Y + D))], p = o[2 * (t + e * (Y + D)) + 1], P = h[2 * (0 + F * Y)], T = h[2 * (0 + F * Y) + 1], O = K * P - p * T, U = K * T + p * P, L = o[2 * (t + e * (Y + f))], v = o[2 * (t + e * (Y + f)) + 1], z = h[2 * (0 + N * Y)], _ = h[2 * (0 + N * Y) + 1], V = L * z - v * _, $ = L * _ + v * z, AA = o[2 * (t + e * (Y + R))], b = o[2 * (t + e * (Y + R)) + 1], CA = h[2 * (0 + y * Y)], gA = h[2 * (0 + y * Y) + 1], BA = AA * CA - b * gA, W = AA * gA + b * CA, d = H + V, j = J + $, IA = H - V, QA = J - $, EA = O + BA, eA = U + W, tA = O - BA, oA = U - W, sA = d + EA, fA = j + eA;
			...
			function C(o, t, e, c, w, s, h) {
				for (var D = w.twiddle, f = w.n, R = new Float64Array(2 * h), F = 0; F < s; F++) {
					...
					for (var N = 0, y = F; N < h; N++, y += s) {
						...
						for (var K = 1; K < h; K++) {
							J = (J + c * y) % f;
							var p = o[2 * (t + e * y)], P = o[2 * (t + e * y) + 1], T = R[2 * K], O = R[2 * K + 1], U = D[2 * J], L = D[2 * J + 1], v = T * U - O * L, z = T * L + O * U, _ = p + v, V = P + z;
	...
	function Ng(E, I) {
		...
		for (let B = 0; B < q; B++) {
			for (let r = 0; r < q; r++) C = B + r * q, A[r] = E[C], i[r] = I[C];
			_A(A, i);
			for (let r = 0; r < q; r++) C = B + r * q, E[C] = A[r], I[C] = i[r];
		}
	}
	function yg(E, I) {
		let A = [], i = [], C = 0;
	...
	function eI(E, I, A) {
		let i, C, B, r, o, t, e, c, w, s = q >> 2;
		for (let h = 0; h < q; h++) r = JA[h], h < r && (o = E[h], E[h] = E[r], E[r] = o, o = I[h], I[h] = I[r], I[r] = o);
		for (let h = 1; h < q; h <<= 1) {
			C = 0, i = q / (h << 1);
			for (let D = 0; D < h; D++) {
				t = wA[C + s], e = A * wA[C];
				for (let f = D; f < q; f += h << 1) B = f + h, c = t * E[B] + e * I[B], w = t * I[B] - e * E[B], E[B] = E[f] - c, E[f] += c, I[B] = I[f] - w, I[f] += w;
				C += i;
			}
		}
	}
	...
	function Yg() {
		let E = q >> 1, I = q >> 2, A = q >> 3, i = E + I, C = Math.sin(Math.PI / q), B = 2 * C * C, r = Math.sqrt(B * (2 - B)), o = wA[I] = 1, t = wA[0] = 0;
		C = 2 * B;
		for (let e = 1; e < A; e++) o -= B, B += C * o, t += r, r -= C * t, wA[e] = t, wA[I - e] = o;
	...
	async function Hg() {
		let E = "Other", I = "Unknown", A = "Other", i = "Unknown", C = navigator.userAgentData, B = navigator.userAgent;
		try {
			if (C) {
		...
		bI = (() => {
			...
			return (function(I = {}) {
				...
				function BA() {
					...
					function g(a, n) {
						var G = a.exports;
						return D = G, h = D.b, N(), D.i, T(D.c), _("wasm-instantiate"), G;
					}
					if (z("wasm-instantiate"), A.instantiateWasm) try {
						return A.instantiateWasm(Q, g);
					} catch (a) {
	...
	var bg = iA((() => {
		AI.prototype.fft = function(I) {
			...
			var A = this._csize, i = 1 << this._width, C = A / i << 1, B, r, o = this._bitrev;
			if (C === 4) for (B = 0, r = 0; B < A; B += C, r++) {
				const s = o[r];
			...
			for (i >>= 2; i >= 2; i >>= 2) {
				C = A / i << 1;
				var t = C >>> 2;
				for (B = 0; B < A; B += C) for (var e = B + t, c = B, w = 0; c < e; c += 2, w += i) {
					const s = c, h = s + t, D = h + t, f = D + t, R = this._out[s], F = this._out[s + 1], N = this._out[h], y = this._out[h + 1], Y = this._out[D], H = this._out[D + 1], J = this._out[f], K = this._out[f + 1], p = R, P = F, T = this.table[w], O = this.table[w + 1], U = N * T - y * O, L = N * O + y * T, v = this.table[2 * w], z = this.table[2 * w + 1], _ = Y * v - H * z, V = Y * z + H * v, $ = this.table[3 * w], AA = this.table[3 * w + 1], b = J * $ - K * AA, CA = J * AA + K * $, gA = p + _, BA = P + V, W = p - _, d = P - V, j = U + b, IA = L + CA, QA = U - b, EA = L - CA;
			...
			const C = this._data[A], B = this._data[A + 1], r = this._data[A + i], o = this._data[A + i + 1];
			this._out[I] = C + r, this._out[I + 1] = B + o, this._out[I + 2] = C - r, this._out[I + 3] = B - o;
		}, AI.prototype._singleTransform4 = function(I, A, i) {
			const C = i * 2, B = i * 3, r = this._data[A], o = this._data[A + 1], t = this._data[A + i], e = this._data[A + i + 1], c = this._data[A + C], w = this._data[A + C + 1], s = this._data[A + B], h = this._data[A + B + 1], D = r + c, f = o + w, R = r - c, F = o - w, N = t + s, y = e + h, Y = t - s, H = e - h;
	...
	function pg(E, I) {
		for (let A = 0; A < I; A++) {
			...
			E[A] = Math.min(1, Math.max(0, Math.sqrt(i) / Math.sqrt(C)));
		}
	}
	function Tg(E, I) {
	...
	self.onmessage = (E) => {
		if (E.data && E.data.type === "run-dsp") {
			const { measTimeDomain: I, refTimeDomain: A, BINS: i, FFT_SIZE: C, metrics: B, windowType: r, weightingType: o, averagingType: t, averagingDepth: e, averagingAlpha: c, averagingThresholdDb: w, enableSourceWindow: s, sourceWindowWidthMs: h, sourceWindowOffsetMs: D, sampleRate: f, compensationDelaySamples: R, autoDelayCompensation: F } = E.data, N = f || 48e3;
			if (!I || !A) return;
			C && C !== qI && Kg(C), (i !== SA || C !== iI) && (SA = i, iI = C, LA = new Float32Array(C), KA = new Float32Array(C), qA = new Float32Array(C), pA = new Float32Array(C), yA = new Float32Array(i), MA = new Float32Array(i), pI = new Float32Array(C), TI = new Float32Array(C), WI = new Float32Array(C), PI = new Float32Array(C), TA = new Float32Array(i), PA = new Float32Array(i), II = new Float32Array(i), gI = new Float32Array(i), bA = new Float32Array(C), BI = new Float32Array(C), hI = new Float32Array(i), CI = new Float32Array(i), xA = new Float32Array(i), VA = new Float32Array(i), QI = new Float32Array(i), EI = new Float32Array(i), jA = new Float32Array(i), XA = new Float32Array(i), WA = new Ag(i, e || 16)), WA && WA.setDepth(e || 16), ZA = c || .1;
			const y = new Set(B), Y = new Float32Array(I), H = new Float32Array(A), J = lI(H), K = lI(Y);
			R && R > 0 && Tg(H, R);
			const p = r || "Hann";
			p !== "Rectangular" && (xI.apply(Y, p), xI.apply(H, p));
			let P = 0, T = 0;
			for (let d = 0; d < C; d++) P += Y[d], T += H[d];
			P /= C, T /= C;
			for (let d = 0; d < C; d++) Y[d] -= P, H[d] -= T;
			fI(H, qA, pA), fI(Y, LA, KA);
			const O = y.has("Magnitude") || y.has("Impulse") || y.has("Step"), U = y.has("Phase") || y.has("Group Delay"), L = y.has("Impulse") || y.has("Step");
			if (O && OI(LA, KA, qA, pA, TA, yA, MA), WA && t !== "None" && O) {
				if (t === "FIFO") {
					WA.processFIFO(yA, MA, xA, VA, w), yA.set(xA), MA.set(VA);
					for (let d = 0; d < i; d++) {
						const j = Math.sqrt(yA[d] * yA[d] + MA[d] * MA[d]);
						TA[d] = 20 * Math.log10(j + 1e-8);
					}
				} else if (t === "LPF") {
			...
			if (U && zI(LA, KA, qA, pA, PA), qg(qA, pA, LA, KA, i), pg(II, i), L && (Ig(LA, KA, qA, pA, bA, pI, TI, WI, PI), s && gg(bA, h, D, N)), y.has("Step") && _I(bA, BI, N), y.has("Group Delay")) {
				for (let d = 0; d < i; d++) hI[d] = PA[d] * Math.PI / 180;
				$I(hI, N / 2 / i, gI);
			}
			const v = K.peakDb - K.rmsDb;
			CI.fill(Math.max(0, Math.min(30, v)));
			let z = 0;
			if (F && L) {
				let d = 0;
				for (let j = 0; j < bA.length; j++) {
					const IA = Math.abs(bA[j]);
					IA > d && (d = IA, z = j);
				}
			}
			const _ = TA.buffer, V = PA.buffer, $ = II.buffer, AA = gI.buffer, b = bA.buffer, CA = BI.buffer, gA = CI.buffer, BA = yA.buffer, W = MA.buffer;
			self.postMessage({
				type: "dsp-results",
				outputMagnitude: _,
				outputPhase: V,
