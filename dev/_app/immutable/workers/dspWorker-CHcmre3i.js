(function() {
	var SI = Object.defineProperty, tA = (g, I) => () => (g && (I = g(g = 0)), I), tg = (g, I) => {
		let A = {};
		for (var C in g) SI(A, C, {
			get: g[C],
			enumerable: !0
		});
		return I || SI(A, Symbol.toStringTag, { value: "Module" }), A;
	};
	typeof window < "u" && import("webfft").then((g) => {
		g && g.default && new g.default(8192);
	}).catch(() => {});
	function ag(g, I) {
		let A = 0;
		for (let C = 0; C < I; C++) A = A << 1 | g & 1, g >>= 1;
		return A;
	}
	function UI(g, I, A) {
		const C = g.length, B = Math.log2(C);
		for (let E = 0; E < C; E++) {
			const r = ag(E, B);
			if (r > E) {
				const o = g[E], t = I[E];
				g[E] = g[r], I[E] = I[r], g[r] = o, I[r] = t;
			}
		}
		for (let E = 2; E <= C; E <<= 1) {
			const r = E >> 1, o = (A ? 2 : -2) * Math.PI / E, t = Math.cos(o), a = Math.sin(o);
			for (let c = 0; c < C; c += E) {
				let w = 1, n = 0;
				for (let h = 0; h < r; h++) {
					const D = g[c + h], l = I[c + h], R = c + h + r, f = w * g[R] - n * I[R], N = w * I[R] + n * g[R];
					g[c + h] = D + f, I[c + h] = l + N, g[R] = D - f, I[R] = l - N;
					const M = w * t - n * a;
					n = w * a + n * t, w = M;
				}
			}
		}
		if (A) for (let E = 0; E < C; E++) g[E] /= C, I[E] /= C;
	}
	function HI(g, I, A) {
		const C = g.length, B = I || new Float32Array(C), E = A || new Float32Array(C);
		return B.set(g), E.fill(0), UI(B, E, !1), {
			real: B,
			imag: E
		};
	}
	function og(g, I, A, C) {
		const B = g.length, E = A || new Float32Array(B), r = C || new Float32Array(B);
		return E.set(g), r.set(I), UI(E, r, !0), E;
	}
	function ng(g, I, A, C, B, E, r) {
		const o = B.length;
		for (let t = 0; t < o; t++) {
			const a = A[t] * A[t] + C[t] * C[t] + 1e-12, c = (g[t] * A[t] + I[t] * C[t]) / a, w = (I[t] * A[t] - g[t] * C[t]) / a;
			E && (E[t] = c), r && (r[t] = w);
			const n = Math.sqrt(c * c + w * w);
			B[t] = 20 * Math.log10(n + 1e-8);
		}
	}
	function sg(g, I, A, C, B) {
		const E = B.length;
		for (let r = 0; r < E; r++) {
			const o = A[r] * A[r] + C[r] * C[r] + 1e-12, t = (g[r] * A[r] + I[r] * C[r]) / o, a = (I[r] * A[r] - g[r] * C[r]) / o;
			B[r] = Math.atan2(a, t) * (180 / Math.PI);
		}
	}
	function Dg(g, I, A = 48e3) {
		let C = 0;
		const B = g.length;
		for (let E = 0; E < B; E++) C += g[E], I[E] = C;
	}
	function hg(g, I, A) {
		const C = A.length;
		A[0] = 0;
		const B = 2 * Math.PI * I;
		for (let E = 1; E < C; E++) {
			let r = g[E] - g[E - 1];
			for (; r > Math.PI;) r -= 2 * Math.PI;
			for (; r < -Math.PI;) r += 2 * Math.PI;
			A[E] = -r / B * 1e3;
		}
	}
	function vI(g) {
		let I = 0, A = 0;
		const C = g.length;
		for (let B = 0; B < C; B++) {
			const E = Math.abs(g[B]);
			E > I && (I = E), A += g[B] * g[B];
		}
		return {
			peakDb: 20 * Math.log10(I + 1e-9),
			rmsDb: 20 * Math.log10(Math.sqrt(A / Math.max(1, C)) + 1e-9)
		};
	}
	var cg = class {
		depth;
		bins;
		bufferReal;
		bufferImag;
		writeIdx = 0;
		count = 0;
		lpfReal;
		lpfImag;
		lastValidReal;
		lastValidImag;
		constructor(g, I = 16) {
			this.bins = g, this.depth = I, this.bufferReal = Array.from({ length: I }, () => new Float32Array(g)), this.bufferImag = Array.from({ length: I }, () => new Float32Array(g)), this.lpfReal = new Float32Array(g), this.lpfImag = new Float32Array(g), this.lastValidReal = new Float32Array(g), this.lastValidImag = new Float32Array(g);
		}
		processFIFO(g, I, A, C, B) {
			if (B !== void 0 && B > -120) for (let E = 0; E < this.bins; E++) {
				const r = Math.sqrt(g[E] * g[E] + I[E] * I[E]);
				20 * Math.log10(r + 1e-12) < B ? (g[E] = this.lastValidReal[E], I[E] = this.lastValidImag[E]) : (this.lastValidReal[E] = g[E], this.lastValidImag[E] = I[E]);
			}
			this.bufferReal[this.writeIdx].set(g), this.bufferImag[this.writeIdx].set(I), this.writeIdx = (this.writeIdx + 1) % this.depth, this.count < this.depth && this.count++, A.fill(0), C.fill(0);
			for (let E = 0; E < this.count; E++) for (let r = 0; r < this.bins; r++) A[r] += this.bufferReal[E][r], C[r] += this.bufferImag[E][r];
			for (let E = 0; E < this.bins; E++) A[E] /= this.count, C[E] /= this.count;
		}
		processLPF(g, I, A, C, B) {
			for (let E = 0; E < this.bins; E++) this.lpfReal[E] += (g[E] - this.lpfReal[E]) * B, this.lpfImag[E] += (I[E] - this.lpfImag[E]) * B, A[E] = this.lpfReal[E], C[E] = this.lpfImag[E];
		}
		setDepth(g) {
			g !== this.depth && (this.depth = Math.max(1, Math.min(64, g)), this.bufferReal = Array.from({ length: this.depth }, () => new Float32Array(this.bins)), this.bufferImag = Array.from({ length: this.depth }, () => new Float32Array(this.bins)), this.lastValidReal = new Float32Array(this.bins), this.lastValidImag = new Float32Array(this.bins), this.writeIdx = 0, this.count = 0);
		}
		reset() {
			this.writeIdx = 0, this.count = 0, this.lpfReal.fill(0), this.lpfImag.fill(0), this.lastValidReal.fill(0), this.lastValidImag.fill(0);
		}
	};
	function wg(g, I, A, C, B, E, r, o, t) {
		const a = g.length, c = a * 2, w = 1e-10;
		for (let n = 0; n < a; n++) {
			const h = A[n] * A[n] + C[n] * C[n] + w, D = (g[n] * A[n] + I[n] * C[n]) / h, l = (I[n] * A[n] - g[n] * C[n]) / h;
			E[n] = D, r[n] = l;
		}
		for (let n = 1; n < a; n++) E[c - n] = E[n], r[c - n] = -r[n];
		og(E, r, o, t), B.set(o);
	}
	function lg(g, I, A, C = 48e3) {
		const B = g.length, E = Math.round(A / 1e3 * C), r = Math.round(I / 2 / 1e3 * C), o = Math.max(0, E - r), t = Math.min(B - 1, E + r), a = Math.round(r * .2);
		for (let c = 0; c < B; c++) if (c < o || c > t) g[c] = 0;
		else if (c < o + a) {
			const w = (c - o) / a, n = .5 * (1 - Math.cos(w * Math.PI));
			g[c] *= n;
		} else if (c > t - a) {
			const w = (t - c) / a, n = .5 * (1 - Math.cos(w * Math.PI));
			g[c] *= n;
		}
	}
	var fg = class {
		cache = {};
		getWindow(g, I) {
			const A = `${g}_${I}`;
			if (!this.cache[A]) {
				const C = new Float32Array(g);
				let B = 0, E = 0;
				for (let o = 0; o < g; o++) {
					let t = 1;
					const a = 2 * Math.PI * o / (g - 1);
					if (I === "Hann") t = .5 * (1 - Math.cos(a));
					else if (I === "Hamming") t = .54 - .46 * Math.cos(a);
					else if (I === "FlatTop") t = 1 - 1.93 * Math.cos(a) + 1.29 * Math.cos(2 * a) - .388 * Math.cos(3 * a) + .0322 * Math.cos(4 * a);
					else if (I === "BlackmanHarris") t = .35875 - .48829 * Math.cos(a) + .14128 * Math.cos(2 * a) - .01168 * Math.cos(3 * a);
					else if (I === "HFT223D") t = 1 - 1.98298997309 * Math.cos(a) + 1.75556083063 * Math.cos(2 * a) - 1.19037717712 * Math.cos(3 * a) + .56155440797 * Math.cos(4 * a) - .17296769663 * Math.cos(5 * a) + .03233247087 * Math.cos(6 * a) - .00324954578 * Math.cos(7 * a) + .0001380104 * Math.cos(8 * a) - 132725e-11 * Math.cos(9 * a);
					else if (I === "Exponential") {
						const c = g / 5;
						t = Math.exp(-o / c);
					}
					C[o] = t, B += t, E += t * t;
				}
				const r = B / g;
				for (let o = 0; o < g; o++) C[o] /= r;
				this.cache[A] = C;
			}
			return this.cache[A];
		}
		apply(g, I) {
			if (I === "Rectangular") return;
			const A = g.length, C = this.getWindow(A, I);
			for (let B = 0; B < A; B++) g[B] *= C[B];
		}
	}, eI = class {
		b0;
		b1;
		b2;
		a1;
		a2;
		z1 = 0;
		z2 = 0;
		constructor(g, I, A, C, B, E) {
			this.b0 = g / C, this.b1 = I / C, this.b2 = A / C, this.a1 = B / C, this.a2 = E / C;
		}
		process(g) {
			for (let I = 0; I < g.length; I++) {
				const A = g[I], C = this.b0 * A + this.z1;
				this.z1 = this.b1 * A - this.a1 * C + this.z2, this.z2 = this.b2 * A - this.a2 * C, g[I] = C;
			}
		}
		reset() {
			this.z1 = 0, this.z2 = 0;
		}
	};
	function Fg(g, I, A) {
		const C = 2 * Math.PI * g / A, B = Math.sin(C) / (2 * I);
		return new eI(1, -2 * Math.cos(C), 1, 1 + B, -2 * Math.cos(C), 1 - B);
	}
	function Rg(g, I, A) {
		const C = 2 * Math.PI * g / A, B = Math.sin(C) / (2 * I);
		return new eI(B, 0, -B, 1 + B, -2 * Math.cos(C), 1 - B);
	}
	function Ng(g, I, A) {
		const C = 2 * Math.PI * g / A, B = Math.sin(C) / (2 * I), E = Math.cos(C);
		return new eI((1 - E) / 2, 1 - E, (1 - E) / 2, 1 + B, -2 * E, 1 - B);
	}
	function mI(g, I) {
		switch (g) {
			case "Notch1k": return Fg(1e3, 10, I);
			case "BP100": return Rg(100, 1, I);
			case "LP200": return Ng(200, .7071, I);
			default: return null;
		}
	}
	const yg = {
		Slow: {
			gain: 132731.3202,
			k: [
				.4600089841,
				-2.6653917847,
				6.200654795,
				-7.2408808951,
				4.2453678122
			]
		},
		Medium: {
			gain: 5908.173436,
			k: [
				.2116396822,
				-1.3993115731,
				3.752522757,
				-5.1097576527,
				3.5394905611
			]
		},
		Fast: {
			gain: 350.8023803,
			k: [
				.0448577871,
				-.3690099172,
				1.271946008,
				-2.321921842,
				2.2829085146
			]
		}
	};
	var _A = class {
		gain;
		k;
		x = [
			0,
			0,
			0,
			0,
			0,
			0
		];
		y = [
			0,
			0,
			0,
			0,
			0,
			0
		];
		p = 3;
		constructor(g) {
			const I = yg[g];
			this.gain = I.gain, this.k = I.k;
		}
		ptr(g) {
			let I = this.p + g;
			return I > 5 && (I -= 6), I;
		}
		process(g) {
			if (g !== g) return this.y[this.ptr(5)];
			this.p = this.ptr(1);
			const I = this.ptr(5);
			return this.x[I] = g / this.gain, this.y[I] = this.x[this.ptr(0)] * 1 + this.x[this.ptr(1)] * 5 + this.x[this.ptr(2)] * 10 + this.x[this.ptr(3)] * 10 + this.x[this.ptr(4)] * 5 + this.x[I] * 1 + this.y[this.ptr(0)] * this.k[0] + this.y[this.ptr(1)] * this.k[1] + this.y[this.ptr(2)] * this.k[2] + this.y[this.ptr(3)] * this.k[3] + this.y[this.ptr(4)] * this.k[4], this.y[I];
		}
		reset() {
			this.x.fill(0), this.y.fill(0);
		}
	}, Mg = class {
		filtersReal;
		filtersImag;
		bins;
		currentFreq;
		constructor(g, I = "Medium") {
			this.bins = g, this.currentFreq = I, this.filtersReal = Array.from({ length: g }, () => new _A(I)), this.filtersImag = Array.from({ length: g }, () => new _A(I));
		}
		setFrequency(g) {
			g !== this.currentFreq && (this.currentFreq = g, this.filtersReal = Array.from({ length: this.bins }, () => new _A(g)), this.filtersImag = Array.from({ length: this.bins }, () => new _A(g)));
		}
		process(g, I, A, C) {
			for (let B = 0; B < this.bins; B++) A[B] = this.filtersReal[B].process(g[B]), C[B] = this.filtersImag[B].process(I[B]);
		}
		reset() {
			for (let g = 0; g < this.bins; g++) this.filtersReal[g].reset(), this.filtersImag[g].reset();
		}
	};
	function aI(g, I, A, C) {
		if (C <= 0 || I <= 0) return;
		const B = A / 2 / I, E = Math.pow(2, 1 / (2 * C)), r = new Float32Array(I);
		for (let o = 1; o < I; o++) {
			const t = o * B, a = t / E, c = t * E, w = Math.max(1, Math.floor(a / B)), n = Math.min(I - 1, Math.ceil(c / B));
			let h = 0;
			const D = n - w + 1;
			for (let l = w; l <= n; l++) h += g[l];
			r[o] = h / D;
		}
		r[0] = g[0], g.set(r);
	}
	function Gg(g, I, A, C) {
		if (C <= 0 || I <= 0) return;
		const B = A / 2 / I, E = Math.pow(2, 1 / (2 * C)), r = Math.PI / 180, o = 180 / Math.PI, t = new Float32Array(I);
		for (let a = 1; a < I; a++) {
			const c = a * B, w = c / E, n = c * E, h = Math.max(1, Math.floor(w / B)), D = Math.min(I - 1, Math.ceil(n / B));
			let l = 0, R = 0;
			for (let f = h; f <= D; f++) {
				const N = g[f] * r;
				l += Math.sin(N), R += Math.cos(N);
			}
			t[a] = Math.atan2(l, R) * o;
		}
		t[0] = g[0], g.set(t);
	}
	var uI, Yg = tA((() => {
		uI = (() => {
			var g = self.location.href;
			return (function(I = {}) {
				var A = I, C, B;
				A.ready = new Promise((Q, e) => {
					C = Q, B = e;
				});
				var E = Object.assign({}, A), r = !0, o = !1, t = "";
				function a(Q) {
					return A.locateFile ? A.locateFile(Q, t) : t + Q;
				}
				var c;
				(r || o) && (o ? t = self.location.href : typeof document < "u" && document.currentScript && (t = document.currentScript.src), g && (t = g), t.indexOf("blob:") !== 0 ? t = t.substr(0, t.replace(/[?#].*/, "").lastIndexOf("/") + 1) : t = "", o && (c = (Q) => {
					var e = new XMLHttpRequest();
					return e.open("GET", Q, !1), e.responseType = "arraybuffer", e.send(null), new Uint8Array(e.response);
				})), A.print || console.log.bind(console);
				var w = A.printErr || console.error.bind(console);
				Object.assign(A, E), E = null, A.arguments && A.arguments, A.thisProgram && A.thisProgram, A.quit && A.quit;
				var n;
				A.wasmBinary && (n = A.wasmBinary), A.noExitRuntime, typeof WebAssembly != "object" && j("no native wasm support detected");
				var h, D, l = !1, R, f;
				function N() {
					var Q = h.buffer;
					A.HEAP8 = R = new Int8Array(Q), A.HEAP16 = new Int16Array(Q), A.HEAP32 = new Int32Array(Q), A.HEAPU8 = f = new Uint8Array(Q), A.HEAPU16 = new Uint16Array(Q), A.HEAPU32 = new Uint32Array(Q), A.HEAPF32 = new Float32Array(Q), A.HEAPF64 = new Float64Array(Q);
				}
				var M = [], k = [], v = [];
				function u() {
					if (A.preRun) for (typeof A.preRun == "function" && (A.preRun = [A.preRun]); A.preRun.length;) K(A.preRun.shift());
					V(M);
				}
				function x() {
					V(k);
				}
				function P() {
					if (A.postRun) for (typeof A.postRun == "function" && (A.postRun = [A.postRun]); A.postRun.length;) T(A.postRun.shift());
					V(v);
				}
				function K(Q) {
					M.unshift(Q);
				}
				function b(Q) {
					k.unshift(Q);
				}
				function T(Q) {
					v.unshift(Q);
				}
				var S = 0, p = null, H = null;
				function _(Q) {
					S++, A.monitorRunDependencies && A.monitorRunDependencies(S);
				}
				function $(Q) {
					if (S--, A.monitorRunDependencies && A.monitorRunDependencies(S), S == 0 && (p !== null && (clearInterval(p), p = null), H)) {
						var e = H;
						H = null, e();
					}
				}
				function j(Q) {
					A.onAbort && A.onAbort(Q), Q = "Aborted(" + Q + ")", w(Q), l = !0, Q += ". Build with -sASSERTIONS for more info.";
					var e = new WebAssembly.RuntimeError(Q);
					throw B(e), e;
				}
				var X = "data:application/octet-stream;base64,";
				function AA(Q) {
					return Q.startsWith(X);
				}
				var q = "data:application/octet-stream;base64,";
				AA(q) || (q = a(q));
				function rA(Q) {
					if (Q == q && n) return new Uint8Array(n);
					var e = sA(Q);
					if (e) return e;
					if (c) return c(Q);
					throw "sync fetching of the wasm failed: you can preload it to Module['wasmBinary'] manually, or emcc.py will do that for you when generating HTML (but not JS)";
				}
				function CA(Q, e) {
					var s, Y = rA(Q);
					return s = new WebAssembly.Module(Y), [new WebAssembly.Instance(s, e), s];
				}
				function QA() {
					var Q = { a: J };
					function e(s, Y) {
						var d = s.exports;
						return D = d, h = D.c, N(), D.j, b(D.d), $("wasm-instantiate"), d;
					}
					if (_("wasm-instantiate"), A.instantiateWasm) try {
						return A.instantiateWasm(Q, e);
					} catch (s) {
						w("Module.instantiateWasm callback failed with error: " + s), B(s);
					}
					return e(CA(q, Q)[0]);
				}
				var V = (Q) => {
					for (; Q.length > 0;) Q.shift()(A);
				}, BA = (Q, e, s) => f.copyWithin(Q, e, e + s), EA = (Q) => {
					j("OOM");
				}, IA = (Q) => {
					f.length, Q >>>= 0, EA(Q);
				};
				function gA(Q) {
					return A["_" + Q];
				}
				var iA = (Q, e) => {
					R.set(Q, e);
				}, oA = (Q) => {
					for (var e = 0, s = 0; s < Q.length; ++s) {
						var Y = Q.charCodeAt(s);
						Y <= 127 ? e++ : Y <= 2047 ? e += 2 : Y >= 55296 && Y <= 57343 ? (e += 4, ++s) : e += 3;
					}
					return e;
				}, eA = (Q, e, s, Y) => {
					if (!(Y > 0)) return 0;
					for (var d = s, G = s + Y - 1, F = 0; F < Q.length; ++F) {
						var U = Q.charCodeAt(F);
						if (U >= 55296 && U <= 57343) {
							var O = Q.charCodeAt(++F);
							U = 65536 + ((U & 1023) << 10) | O & 1023;
						}
						if (U <= 127) {
							if (s >= G) break;
							e[s++] = U;
						} else if (U <= 2047) {
							if (s + 1 >= G) break;
							e[s++] = 192 | U >> 6, e[s++] = 128 | U & 63;
						} else if (U <= 65535) {
							if (s + 2 >= G) break;
							e[s++] = 224 | U >> 12, e[s++] = 128 | U >> 6 & 63, e[s++] = 128 | U & 63;
						} else {
							if (s + 3 >= G) break;
							e[s++] = 240 | U >> 18, e[s++] = 128 | U >> 12 & 63, e[s++] = 128 | U >> 6 & 63, e[s++] = 128 | U & 63;
						}
					}
					return e[s] = 0, s - d;
				}, nA = (Q, e, s) => eA(Q, f, e, s), y = (Q) => {
					var e = oA(Q) + 1, s = HA(e);
					return nA(Q, s, e), s;
				}, L = typeof TextDecoder < "u" ? new TextDecoder("utf8") : void 0, DA = (Q, e, s) => {
					for (var Y = e + s, d = e; Q[d] && !(d >= Y);) ++d;
					if (d - e > 16 && Q.buffer && L) return L.decode(Q.subarray(e, d));
					for (var G = ""; e < d;) {
						var F = Q[e++];
						if (!(F & 128)) {
							G += String.fromCharCode(F);
							continue;
						}
						var U = Q[e++] & 63;
						if ((F & 224) == 192) {
							G += String.fromCharCode((F & 31) << 6 | U);
							continue;
						}
						var O = Q[e++] & 63;
						if ((F & 240) == 224 ? F = (F & 15) << 12 | U << 6 | O : F = (F & 7) << 18 | U << 12 | O << 6 | Q[e++] & 63, F < 65536) G += String.fromCharCode(F);
						else {
							var Z = F - 65536;
							G += String.fromCharCode(55296 | Z >> 10, 56320 | Z & 1023);
						}
					}
					return G;
				}, hA = (Q, e) => Q ? DA(f, Q, e) : "", NA = function(Q, e, s, Y, d) {
					var G = {
						string: (z) => {
							var zA = 0;
							return z != null && z !== 0 && (zA = y(z)), zA;
						},
						array: (z) => {
							var zA = HA(z.length);
							return iA(z, zA), zA;
						}
					};
					function F(z) {
						return e === "string" ? hA(z) : e === "boolean" ? !!z : z;
					}
					var U = gA(Q), O = [], Z = 0;
					if (Y) for (var lA = 0; lA < Y.length; lA++) {
						var yA = G[s[lA]];
						yA ? (Z === 0 && (Z = mA()), O[lA] = yA(Y[lA])) : O[lA] = Y[lA];
					}
					var bA = U.apply(null, O);
					function m(z) {
						return Z !== 0 && YA(Z), F(z);
					}
					return bA = m(bA), bA;
				}, MA = function(Q, e, s, Y) {
					var d = !s || s.every((G) => G === "number" || G === "boolean");
					return e !== "string" && d && !Y ? gA(Q) : function() {
						return NA(Q, e, s, arguments, Y);
					};
				}, J = {
					b: BA,
					a: IA
				}, aA = QA();
				aA.d, A._kiss_fft_free = aA.e, A._free = aA.f, A._kiss_fft_alloc = aA.g, A._malloc = aA.h, A._kiss_fft = aA.i, aA.__errno_location;
				var mA = aA.k, YA = aA.l, HA = aA.m;
				function uA(Q) {
					try {
						for (var e = atob(Q), s = new Uint8Array(e.length), Y = 0; Y < e.length; ++Y) s[Y] = e.charCodeAt(Y);
						return s;
					} catch {
						throw new Error("Converting base64 string to bytes failed.");
					}
				}
				function sA(Q) {
					if (AA(Q)) return uA(Q.slice(X.length));
				}
				A.ccall = NA, A.cwrap = MA;
				var fA;
				H = function Q() {
					fA || i(), fA || (H = Q);
				};
				function i() {
					if (S > 0 || (u(), S > 0)) return;
					function Q() {
						fA || (fA = !0, A.calledRun = !0, !l && (x(), C(A), A.onRuntimeInitialized && A.onRuntimeInitialized(), P()));
					}
					A.setStatus ? (A.setStatus("Running..."), setTimeout(function() {
						setTimeout(function() {
							A.setStatus("");
						}, 1), Q();
					}, 1)) : Q();
				}
				if (A.preInit) for (typeof A.preInit == "function" && (A.preInit = [A.preInit]); A.preInit.length > 0;) A.preInit.pop()();
				return i(), I;
			});
		})();
	})), dA, oI, bI, nI, JI, kg = tA((() => {
		Yg(), dA = uI({}), oI = dA.cwrap("kiss_fft_alloc", "number", [
			"number",
			"number",
			"number",
			"number"
		]), bI = dA.cwrap("kiss_fft", "void", [
			"number",
			"number",
			"number"
		]), nI = dA.cwrap("kiss_fft_free", "void", ["number"]), JI = class {
			constructor(g) {
				this.size = g, this.fcfg = oI(this.size, !1), this.icfg = oI(this.size, !0), this.inptr = dA._malloc(this.size * 8), this.cin = new Float32Array(dA.HEAPU8.buffer, this.inptr, this.size * 2);
			}
			fft = function(g) {
				const I = dA._malloc(this.size * 8), A = new Float32Array(dA.HEAPU8.buffer, I, this.size * 2);
				this.cin.set(g), bI(this.fcfg, this.inptr, I);
				let C = new Float32Array(this.size * 2);
				return C.set(A), dA._free(I), C;
			};
			dispose() {
				nI(this.fcfg), nI(this.icfg), dA._free(this.inptr);
			}
		};
	}));
	function FA(g) {
		if (this.size = g | 0, this.size <= 1 || (this.size & this.size - 1) !== 0) throw new Error("FFT size must be a power of two and bigger than 1");
		this._csize = g << 1;
		for (var I = new Array(this.size * 2), A = 0; A < I.length; A += 2) {
			const t = Math.PI * A / this.size;
			I[A] = Math.cos(t), I[A + 1] = -Math.sin(t);
		}
		this.table = I;
		for (var C = 0, B = 1; this.size > B; B <<= 1) C++;
		this._width = C % 2 === 0 ? C - 1 : C, this._bitrev = new Array(1 << this._width);
		for (var E = 0; E < this._bitrev.length; E++) {
			this._bitrev[E] = 0;
			for (var r = 0; r < this._width; r += 2) {
				var o = this._width - r - 2;
				this._bitrev[E] |= (E >>> r & 3) << o;
			}
		}
		this._out = null, this._data = null, this._inv = 0;
	}
	var dg = tA((() => {
		FA.prototype.fromComplexArray = function(I, A) {
			for (var C = A || new Array(I.length >>> 1), B = 0; B < I.length; B += 2) C[B >>> 1] = I[B];
			return C;
		}, FA.prototype.createComplexArray = function() {
			const I = new Array(this._csize);
			for (var A = 0; A < I.length; A++) I[A] = 0;
			return I;
		}, FA.prototype.toComplexArray = function(I, A) {
			for (var C = A || this.createComplexArray(), B = 0; B < C.length; B += 2) C[B] = I[B >>> 1], C[B + 1] = 0;
			return C;
		}, FA.prototype.completeSpectrum = function(I) {
			for (var A = this._csize, C = A >>> 1, B = 2; B < C; B += 2) I[A - B] = I[B], I[A - B + 1] = -I[B + 1];
		}, FA.prototype.transform = function(I, A) {
			if (I === A) throw new Error("Input and output buffers must be different");
			this._out = I, this._data = A, this._inv = 0, this._transform4(), this._out = null, this._data = null;
		}, FA.prototype.realTransform = function(I, A) {
			if (I === A) throw new Error("Input and output buffers must be different");
			this._out = I, this._data = A, this._inv = 0, this._realTransform4(), this._out = null, this._data = null;
		}, FA.prototype.inverseTransform = function(I, A) {
			if (I === A) throw new Error("Input and output buffers must be different");
			this._out = I, this._data = A, this._inv = 1, this._transform4();
			for (var C = 0; C < I.length; C++) I[C] /= this.size;
			this._out = null, this._data = null;
		}, FA.prototype._transform4 = function() {
			var I = this._out, A = this._csize, C = 1 << this._width, B = A / C << 1, E, r, o = this._bitrev;
			if (B === 4) for (E = 0, r = 0; E < A; E += B, r++) {
				const D = o[r];
				this._singleTransform2(E, D, C);
			}
			else for (E = 0, r = 0; E < A; E += B, r++) {
				const D = o[r];
				this._singleTransform4(E, D, C);
			}
			var t = this._inv ? -1 : 1, a = this.table;
			for (C >>= 2; C >= 2; C >>= 2) {
				B = A / C << 1;
				var c = B >>> 2;
				for (E = 0; E < A; E += B) for (var w = E + c, n = E, h = 0; n < w; n += 2, h += C) {
					const D = n, l = D + c, R = l + c, f = R + c, N = I[D], M = I[D + 1], k = I[l], v = I[l + 1], u = I[R], x = I[R + 1], P = I[f], K = I[f + 1], b = N, T = M, S = a[h], p = t * a[h + 1], H = k * S - v * p, _ = k * p + v * S, $ = a[2 * h], j = t * a[2 * h + 1], X = u * $ - x * j, AA = u * j + x * $, q = a[3 * h], rA = t * a[3 * h + 1], CA = P * q - K * rA, QA = P * rA + K * q, V = b + X, BA = T + AA, EA = b - X, IA = T - AA, gA = H + CA, iA = _ + QA, oA = t * (H - CA), eA = t * (_ - QA), nA = V + gA, y = BA + iA, L = V - gA, DA = BA - iA, hA = EA + eA, NA = IA - oA, MA = EA - eA, J = IA + oA;
					I[D] = nA, I[D + 1] = y, I[l] = hA, I[l + 1] = NA, I[R] = L, I[R + 1] = DA, I[f] = MA, I[f + 1] = J;
				}
			}
		}, FA.prototype._singleTransform2 = function(I, A, C) {
			const B = this._out, E = this._data, r = E[A], o = E[A + 1], t = E[A + C], a = E[A + C + 1], c = r + t, w = o + a, n = r - t, h = o - a;
			B[I] = c, B[I + 1] = w, B[I + 2] = n, B[I + 3] = h;
		}, FA.prototype._singleTransform4 = function(I, A, C) {
			const B = this._out, E = this._data, r = this._inv ? -1 : 1, o = C * 2, t = C * 3, a = E[A], c = E[A + 1], w = E[A + C], n = E[A + C + 1], h = E[A + o], D = E[A + o + 1], l = E[A + t], R = E[A + t + 1], f = a + h, N = c + D, M = a - h, k = c - D, v = w + l, u = n + R, x = r * (w - l), P = r * (n - R), K = f + v, b = N + u, T = M + P, S = k - x, p = f - v, H = N - u, _ = M - P, $ = k + x;
			B[I] = K, B[I + 1] = b, B[I + 2] = T, B[I + 3] = S, B[I + 4] = p, B[I + 5] = H, B[I + 6] = _, B[I + 7] = $;
		}, FA.prototype._realTransform4 = function() {
			var I = this._out, A = this._csize, C = 1 << this._width, B = A / C << 1, E, r, o = this._bitrev;
			if (B === 4) for (E = 0, r = 0; E < A; E += B, r++) {
				const G = o[r];
				this._singleRealTransform2(E, G >>> 1, C >>> 1);
			}
			else for (E = 0, r = 0; E < A; E += B, r++) {
				const G = o[r];
				this._singleRealTransform4(E, G >>> 1, C >>> 1);
			}
			var t = this._inv ? -1 : 1, a = this.table;
			for (C >>= 2; C >= 2; C >>= 2) {
				B = A / C << 1;
				var c = B >>> 1, w = c >>> 1, n = w >>> 1;
				for (E = 0; E < A; E += B) for (var h = 0, D = 0; h <= n; h += 2, D += C) {
					var l = E + h, R = l + w, f = R + w, N = f + w, M = I[l], k = I[l + 1], v = I[R], u = I[R + 1], x = I[f], P = I[f + 1], K = I[N], b = I[N + 1], T = M, S = k, p = a[D], H = t * a[D + 1], _ = v * p - u * H, $ = v * H + u * p, j = a[2 * D], X = t * a[2 * D + 1], AA = x * j - P * X, q = x * X + P * j, rA = a[3 * D], CA = t * a[3 * D + 1], QA = K * rA - b * CA, V = K * CA + b * rA, BA = T + AA, EA = S + q, IA = T - AA, gA = S - q, iA = _ + QA, oA = $ + V, eA = t * (_ - QA), nA = t * ($ - V), y = BA + iA, L = EA + oA, DA = IA + nA, hA = gA - eA;
					if (I[l] = y, I[l + 1] = L, I[R] = DA, I[R + 1] = hA, h === 0) {
						var NA = BA - iA, MA = EA - oA;
						I[f] = NA, I[f + 1] = MA;
						continue;
					}
					if (h !== n) {
						var J = IA, aA = -gA, mA = BA, YA = -EA, HA = -t * nA, uA = -t * eA, sA = -t * oA, fA = -t * iA, i = J + HA, Q = aA + uA, e = mA + fA, s = YA - sA, Y = E + w - h, d = E + c - h;
						I[Y] = i, I[Y + 1] = Q, I[d] = e, I[d + 1] = s;
					}
				}
			}
		}, FA.prototype._singleRealTransform2 = function(I, A, C) {
			const B = this._out, E = this._data, r = E[A], o = E[A + C], t = r + o, a = r - o;
			B[I] = t, B[I + 1] = 0, B[I + 2] = a, B[I + 3] = 0;
		}, FA.prototype._singleRealTransform4 = function(I, A, C) {
			const B = this._out, E = this._data, r = this._inv ? -1 : 1, o = C * 2, t = C * 3, a = E[A], c = E[A + C], w = E[A + o], n = E[A + t], h = a + w, D = a - w, l = c + n, R = r * (c - n), f = h + l, N = D, M = -R, k = h - l, v = D, u = R;
			B[I] = f, B[I + 1] = 0, B[I + 2] = N, B[I + 3] = M, B[I + 4] = k, B[I + 5] = 0, B[I + 6] = v, B[I + 7] = u;
		};
	})), sI, Sg = tA((() => {
		dg(), sI = class {
			constructor(g) {
				this.size = g, this.indutnyFft = new FA(g);
			}
			fft(g) {
				const I = new Float32Array(2 * this.size);
				return this.indutnyFft.transform(I, g), I;
			}
		};
	})), LI, Ug = tA((() => {
		LI = (() => {
			var g = self.location.href;
			return (function(I = {}) {
				var A = I, C, B;
				A.ready = new Promise((i, Q) => {
					C = i, B = Q;
				});
				var E = Object.assign({}, A), r = !0, o = !1, t = "";
				function a(i) {
					return A.locateFile ? A.locateFile(i, t) : t + i;
				}
				var c;
				(r || o) && (o ? t = self.location.href : typeof document < "u" && document.currentScript && (t = document.currentScript.src), g && (t = g), t.indexOf("blob:") !== 0 ? t = t.substr(0, t.replace(/[?#].*/, "").lastIndexOf("/") + 1) : t = "", o && (c = (i) => {
					var Q = new XMLHttpRequest();
					return Q.open("GET", i, !1), Q.responseType = "arraybuffer", Q.send(null), new Uint8Array(Q.response);
				})), A.print || console.log.bind(console);
				var w = A.printErr || console.error.bind(console);
				Object.assign(A, E), E = null, A.arguments && A.arguments, A.thisProgram && A.thisProgram, A.quit && A.quit;
				var n;
				A.wasmBinary && (n = A.wasmBinary), A.noExitRuntime, typeof WebAssembly != "object" && j("no native wasm support detected");
				var h, D, l = !1, R, f;
				function N() {
					var i = h.buffer;
					A.HEAP8 = R = new Int8Array(i), A.HEAP16 = new Int16Array(i), A.HEAP32 = new Int32Array(i), A.HEAPU8 = f = new Uint8Array(i), A.HEAPU16 = new Uint16Array(i), A.HEAPU32 = new Uint32Array(i), A.HEAPF32 = new Float32Array(i), A.HEAPF64 = new Float64Array(i);
				}
				var M = [], k = [], v = [];
				function u() {
					if (A.preRun) for (typeof A.preRun == "function" && (A.preRun = [A.preRun]); A.preRun.length;) K(A.preRun.shift());
					V(M);
				}
				function x() {
					V(k);
				}
				function P() {
					if (A.postRun) for (typeof A.postRun == "function" && (A.postRun = [A.postRun]); A.postRun.length;) T(A.postRun.shift());
					V(v);
				}
				function K(i) {
					M.unshift(i);
				}
				function b(i) {
					k.unshift(i);
				}
				function T(i) {
					v.unshift(i);
				}
				var S = 0, p = null, H = null;
				function _(i) {
					S++, A.monitorRunDependencies && A.monitorRunDependencies(S);
				}
				function $(i) {
					if (S--, A.monitorRunDependencies && A.monitorRunDependencies(S), S == 0 && (p !== null && (clearInterval(p), p = null), H)) {
						var Q = H;
						H = null, Q();
					}
				}
				function j(i) {
					A.onAbort && A.onAbort(i), i = "Aborted(" + i + ")", w(i), l = !0, i += ". Build with -sASSERTIONS for more info.";
					var Q = new WebAssembly.RuntimeError(i);
					throw B(Q), Q;
				}
				var X = "data:application/octet-stream;base64,";
				function AA(i) {
					return i.startsWith(X);
				}
				var q = "data:application/octet-stream;base64,";
				AA(q) || (q = a(q));
				function rA(i) {
					if (i == q && n) return new Uint8Array(n);
					var Q = uA(i);
					if (Q) return Q;
					if (c) return c(i);
					throw "sync fetching of the wasm failed: you can preload it to Module['wasmBinary'] manually, or emcc.py will do that for you when generating HTML (but not JS)";
				}
				function CA(i, Q) {
					var e, s = rA(i);
					return e = new WebAssembly.Module(s), [new WebAssembly.Instance(e, Q), e];
				}
				function QA() {
					var i = { a: MA };
					function Q(e, s) {
						var Y = e.exports;
						return D = Y, h = D.b, N(), D.e, b(D.c), $("wasm-instantiate"), Y;
					}
					if (_("wasm-instantiate"), A.instantiateWasm) try {
						return A.instantiateWasm(i, Q);
					} catch (e) {
						w("Module.instantiateWasm callback failed with error: " + e), B(e);
					}
					return Q(CA(q, i)[0]);
				}
				var V = (i) => {
					for (; i.length > 0;) i.shift()(A);
				}, BA = (i) => {
					j("OOM");
				}, EA = (i) => {
					f.length, i >>>= 0, BA(i);
				};
				function IA(i) {
					return A["_" + i];
				}
				var gA = (i, Q) => {
					R.set(i, Q);
				}, iA = (i) => {
					for (var Q = 0, e = 0; e < i.length; ++e) {
						var s = i.charCodeAt(e);
						s <= 127 ? Q++ : s <= 2047 ? Q += 2 : s >= 55296 && s <= 57343 ? (Q += 4, ++e) : Q += 3;
					}
					return Q;
				}, oA = (i, Q, e, s) => {
					if (!(s > 0)) return 0;
					for (var Y = e, d = e + s - 1, G = 0; G < i.length; ++G) {
						var F = i.charCodeAt(G);
						if (F >= 55296 && F <= 57343) {
							var U = i.charCodeAt(++G);
							F = 65536 + ((F & 1023) << 10) | U & 1023;
						}
						if (F <= 127) {
							if (e >= d) break;
							Q[e++] = F;
						} else if (F <= 2047) {
							if (e + 1 >= d) break;
							Q[e++] = 192 | F >> 6, Q[e++] = 128 | F & 63;
						} else if (F <= 65535) {
							if (e + 2 >= d) break;
							Q[e++] = 224 | F >> 12, Q[e++] = 128 | F >> 6 & 63, Q[e++] = 128 | F & 63;
						} else {
							if (e + 3 >= d) break;
							Q[e++] = 240 | F >> 18, Q[e++] = 128 | F >> 12 & 63, Q[e++] = 128 | F >> 6 & 63, Q[e++] = 128 | F & 63;
						}
					}
					return Q[e] = 0, e - Y;
				}, eA = (i, Q, e) => oA(i, f, Q, e), nA = (i) => {
					var Q = iA(i) + 1, e = YA(Q);
					return eA(i, e, Q), e;
				}, y = typeof TextDecoder < "u" ? new TextDecoder("utf8") : void 0, L = (i, Q, e) => {
					for (var s = Q + e, Y = Q; i[Y] && !(Y >= s);) ++Y;
					if (Y - Q > 16 && i.buffer && y) return y.decode(i.subarray(Q, Y));
					for (var d = ""; Q < Y;) {
						var G = i[Q++];
						if (!(G & 128)) {
							d += String.fromCharCode(G);
							continue;
						}
						var F = i[Q++] & 63;
						if ((G & 224) == 192) {
							d += String.fromCharCode((G & 31) << 6 | F);
							continue;
						}
						var U = i[Q++] & 63;
						if ((G & 240) == 224 ? G = (G & 15) << 12 | F << 6 | U : G = (G & 7) << 18 | F << 12 | U << 6 | i[Q++] & 63, G < 65536) d += String.fromCharCode(G);
						else {
							var O = G - 65536;
							d += String.fromCharCode(55296 | O >> 10, 56320 | O & 1023);
						}
					}
					return d;
				}, DA = (i, Q) => i ? L(f, i, Q) : "", hA = function(i, Q, e, s, Y) {
					var d = {
						string: (m) => {
							var z = 0;
							return m != null && m !== 0 && (z = nA(m)), z;
						},
						array: (m) => {
							var z = YA(m.length);
							return gA(m, z), z;
						}
					};
					function G(m) {
						return Q === "string" ? DA(m) : Q === "boolean" ? !!m : m;
					}
					var F = IA(i), U = [], O = 0;
					if (s) for (var Z = 0; Z < s.length; Z++) {
						var lA = d[e[Z]];
						lA ? (O === 0 && (O = aA()), U[Z] = lA(s[Z])) : U[Z] = s[Z];
					}
					var yA = F.apply(null, U);
					function bA(m) {
						return O !== 0 && mA(O), G(m);
					}
					return yA = bA(yA), yA;
				}, NA = function(i, Q, e, s) {
					var Y = !e || e.every((d) => d === "number" || d === "boolean");
					return Q !== "string" && Y && !s ? IA(i) : function() {
						return hA(i, Q, e, arguments, s);
					};
				}, MA = { a: EA }, J = QA();
				J.c, A._fftCross = J.d, J.__errno_location, A._malloc = J.f, A._free = J.g;
				var aA = J.h, mA = J.i, YA = J.j;
				function HA(i) {
					try {
						for (var Q = atob(i), e = new Uint8Array(Q.length), s = 0; s < Q.length; ++s) e[s] = Q.charCodeAt(s);
						return e;
					} catch {
						throw new Error("Converting base64 string to bytes failed.");
					}
				}
				function uA(i) {
					if (AA(i)) return HA(i.slice(X.length));
				}
				A.ccall = hA, A.cwrap = NA;
				var sA;
				H = function i() {
					sA || fA(), sA || (H = i);
				};
				function fA() {
					if (S > 0 || (u(), S > 0)) return;
					function i() {
						sA || (sA = !0, A.calledRun = !0, !l && (x(), C(A), A.onRuntimeInitialized && A.onRuntimeInitialized(), P()));
					}
					A.setStatus ? (A.setStatus("Running..."), setTimeout(function() {
						setTimeout(function() {
							A.setStatus("");
						}, 1), i();
					}, 1)) : i();
				}
				if (A.preInit) for (typeof A.preInit == "function" && (A.preInit = [A.preInit]); A.preInit.length > 0;) A.preInit.pop()();
				return fA(), I;
			});
		})();
	}));
	function Hg(g) {
		this.size = g, this.n = g * 8, this.ptr = JA._malloc(this.n * 4), this.ri = new Uint8Array(JA.HEAPU8.buffer, this.ptr, this.n), this.ii = new Uint8Array(JA.HEAPU8.buffer, this.ptr + this.n, this.n), this.transform = function(I, A, C) {
			var B = this.ptr, E = this.n;
			return this.ri.set(new Uint8Array(I.buffer)), this.ii.set(new Uint8Array(A.buffer)), KI(this.size, C, B, B + E, B + E * 2, B + E * 3), {
				real: new Float64Array(JA.HEAPU8.buffer, B + E * 2, this.size),
				imag: new Float64Array(JA.HEAPU8.buffer, B + E * 3, this.size)
			};
		}, this.dispose = function() {
			JA._free(this.ptr);
		};
	}
	var JA, KI, vg = tA((() => {
		Ug(), JA = LI({}), KI = JA.cwrap("fftCross", "void", [
			"number",
			"number",
			"number",
			"number",
			"number",
			"number"
		]);
	})), qI, mg = tA((() => {
		vg(), qI = class {
			constructor(g) {
				this.size = g, this.fftcross = new Hg(g), this.real = new Float64Array(this.size), this.imag = new Float64Array(this.size);
			}
			fft(g) {
				for (var I = 0; I < this.size; I++) this.real[I] = g[2 * I], this.imag[I] = g[2 * I + 1];
				const A = this.fftcross.transform(this.real, this.imag, !1), C = new Float32Array(2 * this.size);
				for (var I = 0; I < this.size; I++) C[2 * I] = A.real[I], C[2 * I + 1] = A.imag[I];
				return C;
			}
		};
	}));
	function ug(g) {
		this.n = g, this.levels = -1;
		for (var I = 0; I < 32; I++) 1 << I == g && (this.levels = I);
		if (this.levels == -1) throw "Length is not a power of 2";
		this.cosTable = new Array(g / 2), this.sinTable = new Array(g / 2);
		for (var I = 0; I < g / 2; I++) this.cosTable[I] = Math.cos(2 * Math.PI * I / g), this.sinTable[I] = Math.sin(2 * Math.PI * I / g);
		this.forward = function(A, C) {
			for (var B = this.n, E = 0; E < B; E++) {
				var r = D(E, this.levels);
				if (r > E) {
					var o = A[E];
					A[E] = A[r], A[r] = o, o = C[E], C[E] = C[r], C[r] = o;
				}
			}
			for (var t = 2; t <= B; t *= 2) for (var a = t / 2, c = B / t, E = 0; E < B; E += t) for (var r = E, w = 0; r < E + a; r++, w += c) {
				var n = A[r + a] * this.cosTable[w] + C[r + a] * this.sinTable[w], h = -A[r + a] * this.sinTable[w] + C[r + a] * this.cosTable[w];
				A[r + a] = A[r] - n, C[r + a] = C[r] - h, A[r] += n, C[r] += h;
			}
			function D(l, R) {
				for (var f = 0, N = 0; N < R; N++) f = f << 1 | l & 1, l >>>= 1;
				return f;
			}
		}, this.inverse = function(A, C) {
			forward(C, A);
		};
	}
	var bg = tA((() => {})), pI, Jg = tA((() => {
		bg(), pI = class {
			constructor(g) {
				this.size = g, this.fftNayuki = new ug(g);
			}
			fft(g) {
				const I = new Float32Array(this.size), A = new Float32Array(this.size), C = new Float32Array(this.size * 2);
				for (var B = 0; B < this.size; ++B) I[B] = g[B * 2], A[B] = g[B * 2 + 1];
				this.fftNayuki.forward(I, A);
				for (var B = 0; B < this.size; ++B) C[B * 2] = I[B], C[B * 2 + 1] = A[B];
				return C;
			}
		};
	})), TI, Lg = tA((() => {
		TI = (() => {
			var g = self.location.href;
			return (function(I = {}) {
				var A = I, C, B;
				A.ready = new Promise((i, Q) => {
					C = i, B = Q;
				});
				var E = Object.assign({}, A), r = !0, o = !1, t = "";
				function a(i) {
					return A.locateFile ? A.locateFile(i, t) : t + i;
				}
				var c;
				(r || o) && (o ? t = self.location.href : typeof document < "u" && document.currentScript && (t = document.currentScript.src), g && (t = g), t.indexOf("blob:") !== 0 ? t = t.substr(0, t.replace(/[?#].*/, "").lastIndexOf("/") + 1) : t = "", o && (c = (i) => {
					var Q = new XMLHttpRequest();
					return Q.open("GET", i, !1), Q.responseType = "arraybuffer", Q.send(null), new Uint8Array(Q.response);
				})), A.print || console.log.bind(console);
				var w = A.printErr || console.error.bind(console);
				Object.assign(A, E), E = null, A.arguments && A.arguments, A.thisProgram && A.thisProgram, A.quit && A.quit;
				var n;
				A.wasmBinary && (n = A.wasmBinary), A.noExitRuntime, typeof WebAssembly != "object" && j("no native wasm support detected");
				var h, D, l = !1, R, f;
				function N() {
					var i = h.buffer;
					A.HEAP8 = R = new Int8Array(i), A.HEAP16 = new Int16Array(i), A.HEAP32 = new Int32Array(i), A.HEAPU8 = f = new Uint8Array(i), A.HEAPU16 = new Uint16Array(i), A.HEAPU32 = new Uint32Array(i), A.HEAPF32 = new Float32Array(i), A.HEAPF64 = new Float64Array(i);
				}
				var M = [], k = [], v = [];
				function u() {
					if (A.preRun) for (typeof A.preRun == "function" && (A.preRun = [A.preRun]); A.preRun.length;) K(A.preRun.shift());
					V(M);
				}
				function x() {
					V(k);
				}
				function P() {
					if (A.postRun) for (typeof A.postRun == "function" && (A.postRun = [A.postRun]); A.postRun.length;) T(A.postRun.shift());
					V(v);
				}
				function K(i) {
					M.unshift(i);
				}
				function b(i) {
					k.unshift(i);
				}
				function T(i) {
					v.unshift(i);
				}
				var S = 0, p = null, H = null;
				function _(i) {
					S++, A.monitorRunDependencies && A.monitorRunDependencies(S);
				}
				function $(i) {
					if (S--, A.monitorRunDependencies && A.monitorRunDependencies(S), S == 0 && (p !== null && (clearInterval(p), p = null), H)) {
						var Q = H;
						H = null, Q();
					}
				}
				function j(i) {
					A.onAbort && A.onAbort(i), i = "Aborted(" + i + ")", w(i), l = !0, i += ". Build with -sASSERTIONS for more info.";
					var Q = new WebAssembly.RuntimeError(i);
					throw B(Q), Q;
				}
				var X = "data:application/octet-stream;base64,";
				function AA(i) {
					return i.startsWith(X);
				}
				var q = "data:application/octet-stream;base64,";
				AA(q) || (q = a(q));
				function rA(i) {
					if (i == q && n) return new Uint8Array(n);
					var Q = uA(i);
					if (Q) return Q;
					if (c) return c(i);
					throw "sync fetching of the wasm failed: you can preload it to Module['wasmBinary'] manually, or emcc.py will do that for you when generating HTML (but not JS)";
				}
				function CA(i, Q) {
					var e, s = rA(i);
					return e = new WebAssembly.Module(s), [new WebAssembly.Instance(e, Q), e];
				}
				function QA() {
					var i = { a: MA };
					function Q(e, s) {
						var Y = e.exports;
						return D = Y, h = D.b, N(), D.l, b(D.c), $("wasm-instantiate"), Y;
					}
					if (_("wasm-instantiate"), A.instantiateWasm) try {
						return A.instantiateWasm(i, Q);
					} catch (e) {
						w("Module.instantiateWasm callback failed with error: " + e), B(e);
					}
					return Q(CA(q, i)[0]);
				}
				var V = (i) => {
					for (; i.length > 0;) i.shift()(A);
				}, BA = (i) => {
					j("OOM");
				}, EA = (i) => {
					f.length, i >>>= 0, BA(i);
				};
				function IA(i) {
					return A["_" + i];
				}
				var gA = (i, Q) => {
					R.set(i, Q);
				}, iA = (i) => {
					for (var Q = 0, e = 0; e < i.length; ++e) {
						var s = i.charCodeAt(e);
						s <= 127 ? Q++ : s <= 2047 ? Q += 2 : s >= 55296 && s <= 57343 ? (Q += 4, ++e) : Q += 3;
					}
					return Q;
				}, oA = (i, Q, e, s) => {
					if (!(s > 0)) return 0;
					for (var Y = e, d = e + s - 1, G = 0; G < i.length; ++G) {
						var F = i.charCodeAt(G);
						if (F >= 55296 && F <= 57343) {
							var U = i.charCodeAt(++G);
							F = 65536 + ((F & 1023) << 10) | U & 1023;
						}
						if (F <= 127) {
							if (e >= d) break;
							Q[e++] = F;
						} else if (F <= 2047) {
							if (e + 1 >= d) break;
							Q[e++] = 192 | F >> 6, Q[e++] = 128 | F & 63;
						} else if (F <= 65535) {
							if (e + 2 >= d) break;
							Q[e++] = 224 | F >> 12, Q[e++] = 128 | F >> 6 & 63, Q[e++] = 128 | F & 63;
						} else {
							if (e + 3 >= d) break;
							Q[e++] = 240 | F >> 18, Q[e++] = 128 | F >> 12 & 63, Q[e++] = 128 | F >> 6 & 63, Q[e++] = 128 | F & 63;
						}
					}
					return Q[e] = 0, e - Y;
				}, eA = (i, Q, e) => oA(i, f, Q, e), nA = (i) => {
					var Q = iA(i) + 1, e = YA(Q);
					return eA(i, e, Q), e;
				}, y = typeof TextDecoder < "u" ? new TextDecoder("utf8") : void 0, L = (i, Q, e) => {
					for (var s = Q + e, Y = Q; i[Y] && !(Y >= s);) ++Y;
					if (Y - Q > 16 && i.buffer && y) return y.decode(i.subarray(Q, Y));
					for (var d = ""; Q < Y;) {
						var G = i[Q++];
						if (!(G & 128)) {
							d += String.fromCharCode(G);
							continue;
						}
						var F = i[Q++] & 63;
						if ((G & 224) == 192) {
							d += String.fromCharCode((G & 31) << 6 | F);
							continue;
						}
						var U = i[Q++] & 63;
						if ((G & 240) == 224 ? G = (G & 15) << 12 | F << 6 | U : G = (G & 7) << 18 | F << 12 | U << 6 | i[Q++] & 63, G < 65536) d += String.fromCharCode(G);
						else {
							var O = G - 65536;
							d += String.fromCharCode(55296 | O >> 10, 56320 | O & 1023);
						}
					}
					return d;
				}, DA = (i, Q) => i ? L(f, i, Q) : "", hA = function(i, Q, e, s, Y) {
					var d = {
						string: (m) => {
							var z = 0;
							return m != null && m !== 0 && (z = nA(m)), z;
						},
						array: (m) => {
							var z = YA(m.length);
							return gA(m, z), z;
						}
					};
					function G(m) {
						return Q === "string" ? DA(m) : Q === "boolean" ? !!m : m;
					}
					var F = IA(i), U = [], O = 0;
					if (s) for (var Z = 0; Z < s.length; Z++) {
						var lA = d[e[Z]];
						lA ? (O === 0 && (O = aA()), U[Z] = lA(s[Z])) : U[Z] = s[Z];
					}
					var yA = F.apply(null, U);
					function bA(m) {
						return O !== 0 && mA(O), G(m);
					}
					return yA = bA(yA), yA;
				}, NA = function(i, Q, e, s) {
					var Y = !e || e.every((d) => d === "number" || d === "boolean");
					return Q !== "string" && Y && !s ? IA(i) : function() {
						return hA(i, Q, e, arguments, s);
					};
				}, MA = { a: EA }, J = QA();
				J.c, A._malloc = J.d, A._free = J.e, A._precalc = J.f, A._precalc_f = J.g, A._dispose = J.h, A._dispose_f = J.i, A._transform_radix2_precalc = J.j, A._transform_radix2_precalc_f = J.k, J.__errno_location;
				var aA = J.m, mA = J.n, YA = J.o;
				function HA(i) {
					try {
						for (var Q = atob(i), e = new Uint8Array(Q.length), s = 0; s < Q.length; ++s) e[s] = Q.charCodeAt(s);
						return e;
					} catch {
						throw new Error("Converting base64 string to bytes failed.");
					}
				}
				function uA(i) {
					if (AA(i)) return HA(i.slice(X.length));
				}
				A.ccall = hA, A.cwrap = NA;
				var sA;
				H = function i() {
					sA || fA(), sA || (H = i);
				};
				function fA() {
					if (S > 0 || (u(), S > 0)) return;
					function i() {
						sA || (sA = !0, A.calledRun = !0, !l && (x(), C(A), A.onRuntimeInitialized && A.onRuntimeInitialized(), P()));
					}
					A.setStatus ? (A.setStatus("Running..."), setTimeout(function() {
						setTimeout(function() {
							A.setStatus("");
						}, 1), i();
					}, 1)) : i();
				}
				if (A.preInit) for (typeof A.preInit == "function" && (A.preInit = [A.preInit]); A.preInit.length > 0;) A.preInit.pop()();
				return fA(), I;
			});
		})();
	}));
	function Kg(g) {
		this.n = g, this.rptr = kA._malloc(g * 4 + g * 4), this.iptr = this.rptr + g * 4, this.rarr = new Float32Array(kA.HEAPU8.buffer, this.rptr, g), this.iarr = new Float32Array(kA.HEAPU8.buffer, this.iptr, g), this.tables = PI(g), this.forward = function(I, A) {
			this.rarr.set(I), this.iarr.set(A), xI(this.rptr, this.iptr, this.n, this.tables), I.set(this.rarr), A.set(this.iarr);
		}, this.dispose = function() {
			kA._free(this.rptr), WI(this.tables);
		};
	}
	var kA, PI, WI, xI, qg = tA((() => {
		Lg(), kA = TI({}), kA.cwrap("precalc", "number", ["number"]), kA.cwrap("dispose", "void", ["number"]), kA.cwrap("transform_radix2_precalc", "void", [
			"number",
			"number",
			"number",
			"number"
		]), PI = kA.cwrap("precalc_f", "number", ["number"]), WI = kA.cwrap("dispose_f", "void", ["number"]), xI = kA.cwrap("transform_radix2_precalc_f", "void", [
			"number",
			"number",
			"number",
			"number"
		]);
	})), VI, pg = tA((() => {
		qg(), VI = class {
			constructor(g) {
				this.size = g, this.fftNayuki = new Kg(g);
			}
			fft(g) {
				const I = new Float32Array(this.size), A = new Float32Array(this.size), C = new Float32Array(this.size * 2);
				for (var B = 0; B < this.size; ++B) I[B] = g[B * 2], A[B] = g[B * 2 + 1];
				this.fftNayuki.forward(I, A);
				for (var B = 0; B < this.size; ++B) C[B * 2] = I[B], C[B * 2 + 1] = A[B];
				return C;
			}
		};
	})), $A, Tg = tA((() => {
		$A || ($A = {}), (function(g) {
			"use strict";
			function I(o, t, a, c, w, n) {
				for (var h = w.twiddle, D = 0; D < n; D++) {
					var l = o[2 * (t + a * D)], R = o[2 * (t + a * D) + 1], f = o[2 * (t + a * (D + n))], N = o[2 * (t + a * (D + n)) + 1], M = h[2 * (0 + c * D)], k = h[2 * (0 + c * D) + 1], v = f * M - N * k, u = f * k + N * M, x = l + v, P = R + u, K = l - v, b = R - u;
					o[2 * (t + a * D)] = x, o[2 * (t + a * D) + 1] = P, o[2 * (t + a * (D + n))] = K, o[2 * (t + a * (D + n)) + 1] = b;
				}
			}
			function A(o, t, a, c, w, n) {
				for (var h = w.twiddle, D = n, l = 2 * n, R = c, f = 2 * c, N = h[2 * (0 + c * n) + 1], M = 0; M < n; M++) {
					var k = o[2 * (t + a * M)], v = o[2 * (t + a * M) + 1], u = o[2 * (t + a * (M + D))], x = o[2 * (t + a * (M + D)) + 1], P = h[2 * (0 + R * M)], K = h[2 * (0 + R * M) + 1], b = u * P - x * K, T = u * K + x * P, S = o[2 * (t + a * (M + l))], p = o[2 * (t + a * (M + l)) + 1], H = h[2 * (0 + f * M)], _ = h[2 * (0 + f * M) + 1], $ = S * H - p * _, j = S * _ + p * H, X = b + $, AA = T + j, q = k + X, rA = v + AA;
					o[2 * (t + a * M)] = q, o[2 * (t + a * M) + 1] = rA;
					var CA = k - X * .5, QA = v - AA * .5, V = (b - $) * N, BA = (T - j) * N, EA = CA - BA, IA = QA + V;
					o[2 * (t + a * (M + D))] = EA, o[2 * (t + a * (M + D)) + 1] = IA;
					var gA = CA + BA, iA = QA - V;
					o[2 * (t + a * (M + l))] = gA, o[2 * (t + a * (M + l)) + 1] = iA;
				}
			}
			function C(o, t, a, c, w, n) {
				for (var h = w.twiddle, D = n, l = 2 * n, R = 3 * n, f = c, N = 2 * c, M = 3 * c, k = 0; k < n; k++) {
					var v = o[2 * (t + a * k)], u = o[2 * (t + a * k) + 1], x = o[2 * (t + a * (k + D))], P = o[2 * (t + a * (k + D)) + 1], K = h[2 * (0 + f * k)], b = h[2 * (0 + f * k) + 1], T = x * K - P * b, S = x * b + P * K, p = o[2 * (t + a * (k + l))], H = o[2 * (t + a * (k + l)) + 1], _ = h[2 * (0 + N * k)], $ = h[2 * (0 + N * k) + 1], j = p * _ - H * $, X = p * $ + H * _, AA = o[2 * (t + a * (k + R))], q = o[2 * (t + a * (k + R)) + 1], rA = h[2 * (0 + M * k)], CA = h[2 * (0 + M * k) + 1], QA = AA * rA - q * CA, V = AA * CA + q * rA, BA = v + j, EA = u + X, IA = v - j, gA = u - X, iA = T + QA, oA = S + V, eA = T - QA, nA = S - V, y = BA + iA, L = EA + oA;
					if (w.inverse) var DA = IA - nA, hA = gA + eA;
					else var DA = IA + nA, hA = gA - eA;
					var NA = BA - iA, MA = EA - oA;
					if (w.inverse) var J = IA + nA, aA = gA - eA;
					else var J = IA - nA, aA = gA + eA;
					o[2 * (t + a * k)] = y, o[2 * (t + a * k) + 1] = L, o[2 * (t + a * (k + D))] = DA, o[2 * (t + a * (k + D)) + 1] = hA, o[2 * (t + a * (k + l))] = NA, o[2 * (t + a * (k + l)) + 1] = MA, o[2 * (t + a * (k + R))] = J, o[2 * (t + a * (k + R)) + 1] = aA;
				}
			}
			function B(o, t, a, c, w, n, h) {
				for (var D = w.twiddle, l = w.n, R = new Float64Array(2 * h), f = 0; f < n; f++) {
					for (var N = 0, M = f; N < h; N++, M += n) {
						var k = o[2 * (t + a * M)], v = o[2 * (t + a * M) + 1];
						R[2 * N] = k, R[2 * N + 1] = v;
					}
					for (var N = 0, M = f; N < h; N++, M += n) {
						var u = 0, k = R[0], v = R[1];
						o[2 * (t + a * M)] = k, o[2 * (t + a * M) + 1] = v;
						for (var x = 1; x < h; x++) {
							u = (u + c * M) % l;
							var P = o[2 * (t + a * M)], K = o[2 * (t + a * M) + 1], b = R[2 * x], T = R[2 * x + 1], S = D[2 * u], p = D[2 * u + 1], H = b * S - T * p, _ = b * p + T * S, $ = P + H, j = K + _;
							o[2 * (t + a * M)] = $, o[2 * (t + a * M) + 1] = j;
						}
					}
				}
			}
			function E(o, t, a, c, w, n, h, D, l) {
				var R = D.shift(), f = D.shift();
				if (f == 1) for (var N = 0; N < R * f; N++) {
					var M = c[2 * (w + n * h * N)], k = c[2 * (w + n * h * N) + 1];
					o[2 * (t + a * N)] = M, o[2 * (t + a * N) + 1] = k;
				}
				else for (var N = 0; N < R; N++) E(o, t + a * N * f, a, c, w + N * n * h, n * R, h, D.slice(), l);
				switch (R) {
					case 2:
						I(o, t, a, n, l, f);
						break;
					case 3:
						A(o, t, a, n, l, f);
						break;
					case 4:
						C(o, t, a, n, l, f);
						break;
					default:
						B(o, t, a, n, l, f, R);
						break;
				}
			}
			var r = function(a, c) {
				if (arguments.length < 2) throw new RangeError("You didn't pass enough arguments, passed `" + arguments.length + "'");
				var a = ~~a, c = !!c;
				if (a < 1) throw new RangeError("n is outside range, should be positive integer, was `" + a + "'");
				for (var w = {
					n: a,
					inverse: c,
					factors: [],
					twiddle: new Float64Array(2 * a),
					scratch: new Float64Array(2 * a)
				}, n = w.twiddle, h = 2 * Math.PI / a, D = 0; D < a; D++) {
					if (c) var l = h * D;
					else var l = -h * D;
					n[2 * D] = Math.cos(l), n[2 * D + 1] = Math.sin(l);
				}
				for (var R = 4, f = Math.floor(Math.sqrt(a)); a > 1;) {
					for (; a % R;) {
						switch (R) {
							case 4:
								R = 2;
								break;
							case 2:
								R = 3;
								break;
							default:
								R += 2;
								break;
						}
						R > f && (R = a);
					}
					a /= R, w.factors.push(R), w.factors.push(a);
				}
				this.state = w;
			};
			r.prototype.simple = function(o, t, a) {
				this.process(o, 0, 1, t, 0, 1, a);
			}, r.prototype.process = function(o, t, D, c, w, l, h) {
				var D = ~~D, l = ~~l, R = h == "real" ? h : "complex";
				if (D < 1) throw new RangeError("outputStride is outside range, should be positive integer, was `" + D + "'");
				if (l < 1) throw new RangeError("inputStride is outside range, should be positive integer, was `" + l + "'");
				if (R == "real") {
					for (var f = 0; f < this.state.n; f++) {
						var N = c[w + l * f], M = 0;
						this.state.scratch[2 * f] = N, this.state.scratch[2 * f + 1] = M;
					}
					E(o, t, D, this.state.scratch, 0, 1, 1, this.state.factors.slice(), this.state);
				} else if (c == o) {
					E(this.state.scratch, 0, 1, c, w, 1, l, this.state.factors.slice(), this.state);
					for (var f = 0; f < this.state.n; f++) {
						var N = this.state.scratch[2 * f], M = this.state.scratch[2 * f + 1];
						o[2 * (t + D * f)] = N, o[2 * (t + D * f) + 1] = M;
					}
				} else E(o, t, D, c, w, 1, l, this.state.factors.slice(), this.state);
			}, g.complex = r;
		})($A);
	})), jI, Pg = tA((() => {
		Tg(), jI = class {
			constructor(g) {
				this.size = g, this.nockertfft = new $A.complex(g, !1);
			}
			fft(g) {
				const I = new Float32Array(2 * this.size);
				return this.nockertfft.simple(I, g, "complex"), I;
			}
		};
	}));
	function Wg(g) {
		if (g !== 0 && (g & g - 1) === 0) W = g, Xg(), Og(), Zg();
		else throw new Error("init: radix-2 required");
	}
	function AI(g, I) {
		DI(g, I, 1);
	}
	function II(g, I) {
		let A = 1 / W;
		DI(g, I, -1);
		for (let C = 0; C < W; C++) g[C] *= A, I[C] *= A;
	}
	function xg(g, I) {
		DI(g, I, -1);
	}
	function Vg(g, I) {
		let A = [], C = [], B = 0;
		for (let E = 0; E < W; E++) {
			B = E * W;
			for (let r = 0; r < W; r++) A[r] = g[r + B], C[r] = I[r + B];
			AI(A, C);
			for (let r = 0; r < W; r++) g[r + B] = A[r], I[r + B] = C[r];
		}
		for (let E = 0; E < W; E++) {
			for (let r = 0; r < W; r++) B = E + r * W, A[r] = g[B], C[r] = I[B];
			AI(A, C);
			for (let r = 0; r < W; r++) B = E + r * W, g[B] = A[r], I[B] = C[r];
		}
	}
	function jg(g, I) {
		let A = [], C = [], B = 0;
		for (let E = 0; E < W; E++) {
			B = E * W;
			for (let r = 0; r < W; r++) A[r] = g[r + B], C[r] = I[r + B];
			II(A, C);
			for (let r = 0; r < W; r++) g[r + B] = A[r], I[r + B] = C[r];
		}
		for (let E = 0; E < W; E++) {
			for (let r = 0; r < W; r++) B = E + r * W, A[r] = g[B], C[r] = I[B];
			II(A, C);
			for (let r = 0; r < W; r++) B = E + r * W, g[B] = A[r], I[B] = C[r];
		}
	}
	function DI(g, I, A) {
		let C, B, E, r, o, t, a, c, w, n = W >> 2;
		for (let h = 0; h < W; h++) r = xA[h], h < r && (o = g[h], g[h] = g[r], g[r] = o, o = I[h], I[h] = I[r], I[r] = o);
		for (let h = 1; h < W; h <<= 1) {
			B = 0, C = W / (h << 1);
			for (let D = 0; D < h; D++) {
				t = RA[B + n], a = A * RA[B];
				for (let l = D; l < W; l += h << 1) E = l + h, c = t * g[E] + a * I[E], w = t * I[E] - a * g[E], g[E] = g[l] - c, g[l] += c, I[E] = I[l] - w, I[l] += w;
				B += C;
			}
		}
	}
	function Xg() {
		typeof Uint32Array < "u" ? xA = new Uint32Array(W) : xA = [], typeof Float64Array < "u" ? RA = new Float64Array(W * 1.25) : RA = [];
	}
	function Og() {
		let g = 0, I = 0, A = 0;
		for (xA[0] = 0; ++g < W;) {
			for (A = W >> 1; A <= I;) I -= A, A >>= 1;
			I += A, xA[g] = I;
		}
	}
	function Zg() {
		let g = W >> 1, I = W >> 2, A = W >> 3, C = g + I, B = Math.sin(Math.PI / W), E = 2 * B * B, r = Math.sqrt(E * (2 - E)), o = RA[I] = 1, t = RA[0] = 0;
		B = 2 * E;
		for (let a = 1; a < A; a++) o -= E, E += B * o, t += r, r -= B * t, RA[a] = t, RA[I - a] = o;
		A !== 0 && (RA[A] = Math.sqrt(.5));
		for (let a = 0; a < I; a++) RA[g - a] = RA[a];
		for (let a = 0; a < C; a++) RA[a + g] = -RA[a];
	}
	var W, xA, RA, XI, zg = tA((() => {
		W = 0, xA = null, RA = null, XI = {
			init: Wg,
			fft1d: AI,
			ifft1d: II,
			fft2d: Vg,
			ifft2d: jg,
			fft: AI,
			ifft: II,
			bt: xg
		};
	})), OI, _g = tA((() => {
		zg(), OI = class {
			constructor(g) {
				this.size = g, this.FFT_mljs = XI, this.FFT_mljs.init(g);
			}
			fft(g) {
				const I = new Float32Array(this.size), A = new Float32Array(this.size), C = new Float32Array(2 * this.size);
				for (var B = 0; B < this.size; ++B) I[B] = g[B * 2], A[B] = g[B * 2 + 1];
				this.FFT_mljs.fft(I, A);
				for (var B = 0; B < this.size; ++B) C[B * 2] = I[B], C[B * 2 + 1] = A[B];
				return C;
			}
		};
	}));
	async function $g() {
		return await WebAssembly.validate(new Uint8Array([
			0,
			97,
			115,
			109,
			1,
			0,
			0,
			0,
			1,
			5,
			1,
			96,
			0,
			1,
			123,
			3,
			2,
			1,
			0,
			10,
			15,
			1,
			13,
			0,
			65,
			1,
			253,
			15,
			65,
			2,
			253,
			15,
			253,
			128,
			2,
			11
		]));
	}
	async function AB() {
		return await WebAssembly.validate(new Uint8Array([
			0,
			97,
			115,
			109,
			1,
			0,
			0,
			0,
			1,
			5,
			1,
			96,
			0,
			1,
			123,
			3,
			2,
			1,
			0,
			10,
			10,
			1,
			8,
			0,
			65,
			0,
			253,
			15,
			253,
			98,
			11
		]));
	}
	async function IB() {
		let g = "Other", I = "Unknown", A = "Other", C = "Unknown", B = navigator.userAgentData, E = navigator.userAgent;
		try {
			if (B) {
				const r = await B.getHighEntropyValues([
					"architecture",
					"model",
					"platform",
					"platformVersion",
					"uaFullVersion"
				]), o = B.brands.find((t) => [
					"Microsoft Edge",
					"Google Chrome",
					"Opera"
				].includes(t.brand));
				g = o ? o.brand : "Other", I = o ? `v${o.version}` : "Unknown", A = r.platform ? r.platform : "Other", C = r.platformVersion ? `v${r.platformVersion}` : "Unknown";
			}
			if (g === "Other" || A === "Other") {
				const r = E.split(" "), o = r[r.length - 1], t = /Firefox/.test(o), a = /Safari/.test(o) && !/CriOS/.test(o) && !/Chrome/.test(o), c = /CriOS/.test(o) || /Chrome/.test(o), w = /Edg/.test(o), n = /OPR/.test(o), h = [
					{
						name: "Mozilla Firefox",
						regex: /Firefox\/(\d+\.\d+)/,
						flag: t
					},
					{
						name: "Safari",
						regex: /Version\/(\d+\.\d+)/,
						flag: a
					},
					{
						name: "Google Chrome",
						regex: /CriOS|Chrome\/(\d+\.\d+)/,
						flag: c
					},
					{
						name: "Microsoft Edge",
						regex: /Edg\/(\d+\.\d+)/,
						flag: w
					},
					{
						name: "Opera",
						regex: /OPR\/(\d+\.\d+)/,
						flag: n
					}
				];
				for (const N of h) if (N.flag) {
					g = N.name;
					const M = o.match(N.regex);
					I = M ? M[1] : "Unknown";
					break;
				}
				const D = E.match(/\(([^)]+)\)/), l = D ? D[1].split("; ") : [];
				console.log(D), console.log(l);
				const R = {
					"10.0": "10",
					"6.3": "8.1",
					"6.2": "8",
					"6.1": "7",
					"6.0": "Vista",
					"5.2": "XP 64-bit",
					"5.1": "XP",
					"5.0": "2000"
				}, f = [
					{
						name: "Windows",
						regex: /Windows NT/,
						transform: (N) => R[N.split(" ")[2]],
						index: 0
					},
					{
						name: "Mac OS X",
						regex: /Mac OS X/,
						transform: (N) => N.replace("_", ".").split(" ")[3],
						index: 0
					},
					{
						name: "Linux",
						regex: /Linux/,
						transform: () => "Unknown",
						index: 0
					},
					{
						name: "Android",
						regex: /Android/,
						transform: (N) => N.split(" ")[1],
						index: 0
					},
					{
						name: "iOS",
						regex: /iPhone/,
						transform: (N) => N.split(" ")[1].replace("_", "."),
						index: 0
					}
				];
				for (const N of f) if (N.regex.test(l[0])) {
					A = N.name, console.log(`osDetails: ${l}`), C = N.transform ? N.transform(l[1]) : N.versionMap[l[1].split(" ")[N.index]];
					break;
				}
			}
		} catch (r) {
			console.error("Could not retrieve user agent data", r);
		}
		return {
			browserName: g,
			browserVersion: I,
			osName: A,
			osVersion: C,
			wasm: typeof WebAssembly == "object",
			relaxedSimd: await $g(),
			simd: await AB()
		};
	}
	var gB = tA((() => {})), ZI, BB = tA((() => {
		ZI = (() => {
			var g = self.location.href;
			return (function(I = {}) {
				var A = I, C, B;
				A.ready = new Promise((i, Q) => {
					C = i, B = Q;
				});
				var E = Object.assign({}, A), r = !0, o = !1, t = "";
				function a(i) {
					return A.locateFile ? A.locateFile(i, t) : t + i;
				}
				var c;
				(r || o) && (o ? t = self.location.href : typeof document < "u" && document.currentScript && (t = document.currentScript.src), g && (t = g), t.indexOf("blob:") !== 0 ? t = t.substr(0, t.replace(/[?#].*/, "").lastIndexOf("/") + 1) : t = "", o && (c = (i) => {
					var Q = new XMLHttpRequest();
					return Q.open("GET", i, !1), Q.responseType = "arraybuffer", Q.send(null), new Uint8Array(Q.response);
				})), A.print || console.log.bind(console);
				var w = A.printErr || console.error.bind(console);
				Object.assign(A, E), E = null, A.arguments && A.arguments, A.thisProgram && A.thisProgram, A.quit && A.quit;
				var n;
				A.wasmBinary && (n = A.wasmBinary), A.noExitRuntime, typeof WebAssembly != "object" && j("no native wasm support detected");
				var h, D, l = !1, R, f;
				function N() {
					var i = h.buffer;
					A.HEAP8 = R = new Int8Array(i), A.HEAP16 = new Int16Array(i), A.HEAP32 = new Int32Array(i), A.HEAPU8 = f = new Uint8Array(i), A.HEAPU16 = new Uint16Array(i), A.HEAPU32 = new Uint32Array(i), A.HEAPF32 = new Float32Array(i), A.HEAPF64 = new Float64Array(i);
				}
				var M = [], k = [], v = [];
				function u() {
					if (A.preRun) for (typeof A.preRun == "function" && (A.preRun = [A.preRun]); A.preRun.length;) K(A.preRun.shift());
					V(M);
				}
				function x() {
					V(k);
				}
				function P() {
					if (A.postRun) for (typeof A.postRun == "function" && (A.postRun = [A.postRun]); A.postRun.length;) T(A.postRun.shift());
					V(v);
				}
				function K(i) {
					M.unshift(i);
				}
				function b(i) {
					k.unshift(i);
				}
				function T(i) {
					v.unshift(i);
				}
				var S = 0, p = null, H = null;
				function _(i) {
					S++, A.monitorRunDependencies && A.monitorRunDependencies(S);
				}
				function $(i) {
					if (S--, A.monitorRunDependencies && A.monitorRunDependencies(S), S == 0 && (p !== null && (clearInterval(p), p = null), H)) {
						var Q = H;
						H = null, Q();
					}
				}
				function j(i) {
					A.onAbort && A.onAbort(i), i = "Aborted(" + i + ")", w(i), l = !0, i += ". Build with -sASSERTIONS for more info.";
					var Q = new WebAssembly.RuntimeError(i);
					throw B(Q), Q;
				}
				var X = "data:application/octet-stream;base64,";
				function AA(i) {
					return i.startsWith(X);
				}
				var q = "data:application/octet-stream;base64,";
				AA(q) || (q = a(q));
				function rA(i) {
					if (i == q && n) return new Uint8Array(n);
					var Q = uA(i);
					if (Q) return Q;
					if (c) return c(i);
					throw "sync fetching of the wasm failed: you can preload it to Module['wasmBinary'] manually, or emcc.py will do that for you when generating HTML (but not JS)";
				}
				function CA(i, Q) {
					var e, s = rA(i);
					return e = new WebAssembly.Module(s), [new WebAssembly.Instance(e, Q), e];
				}
				function QA() {
					var i = { a: MA };
					function Q(e, s) {
						var Y = e.exports;
						return D = Y, h = D.b, N(), D.i, b(D.c), $("wasm-instantiate"), Y;
					}
					if (_("wasm-instantiate"), A.instantiateWasm) try {
						return A.instantiateWasm(i, Q);
					} catch (e) {
						w("Module.instantiateWasm callback failed with error: " + e), B(e);
					}
					return Q(CA(q, i)[0]);
				}
				var V = (i) => {
					for (; i.length > 0;) i.shift()(A);
				}, BA = (i) => {
					j("OOM");
				}, EA = (i) => {
					f.length, i >>>= 0, BA(i);
				};
				function IA(i) {
					return A["_" + i];
				}
				var gA = (i, Q) => {
					R.set(i, Q);
				}, iA = (i) => {
					for (var Q = 0, e = 0; e < i.length; ++e) {
						var s = i.charCodeAt(e);
						s <= 127 ? Q++ : s <= 2047 ? Q += 2 : s >= 55296 && s <= 57343 ? (Q += 4, ++e) : Q += 3;
					}
					return Q;
				}, oA = (i, Q, e, s) => {
					if (!(s > 0)) return 0;
					for (var Y = e, d = e + s - 1, G = 0; G < i.length; ++G) {
						var F = i.charCodeAt(G);
						if (F >= 55296 && F <= 57343) {
							var U = i.charCodeAt(++G);
							F = 65536 + ((F & 1023) << 10) | U & 1023;
						}
						if (F <= 127) {
							if (e >= d) break;
							Q[e++] = F;
						} else if (F <= 2047) {
							if (e + 1 >= d) break;
							Q[e++] = 192 | F >> 6, Q[e++] = 128 | F & 63;
						} else if (F <= 65535) {
							if (e + 2 >= d) break;
							Q[e++] = 224 | F >> 12, Q[e++] = 128 | F >> 6 & 63, Q[e++] = 128 | F & 63;
						} else {
							if (e + 3 >= d) break;
							Q[e++] = 240 | F >> 18, Q[e++] = 128 | F >> 12 & 63, Q[e++] = 128 | F >> 6 & 63, Q[e++] = 128 | F & 63;
						}
					}
					return Q[e] = 0, e - Y;
				}, eA = (i, Q, e) => oA(i, f, Q, e), nA = (i) => {
					var Q = iA(i) + 1, e = YA(Q);
					return eA(i, e, Q), e;
				}, y = typeof TextDecoder < "u" ? new TextDecoder("utf8") : void 0, L = (i, Q, e) => {
					for (var s = Q + e, Y = Q; i[Y] && !(Y >= s);) ++Y;
					if (Y - Q > 16 && i.buffer && y) return y.decode(i.subarray(Q, Y));
					for (var d = ""; Q < Y;) {
						var G = i[Q++];
						if (!(G & 128)) {
							d += String.fromCharCode(G);
							continue;
						}
						var F = i[Q++] & 63;
						if ((G & 224) == 192) {
							d += String.fromCharCode((G & 31) << 6 | F);
							continue;
						}
						var U = i[Q++] & 63;
						if ((G & 240) == 224 ? G = (G & 15) << 12 | F << 6 | U : G = (G & 7) << 18 | F << 12 | U << 6 | i[Q++] & 63, G < 65536) d += String.fromCharCode(G);
						else {
							var O = G - 65536;
							d += String.fromCharCode(55296 | O >> 10, 56320 | O & 1023);
						}
					}
					return d;
				}, DA = (i, Q) => i ? L(f, i, Q) : "", hA = function(i, Q, e, s, Y) {
					var d = {
						string: (m) => {
							var z = 0;
							return m != null && m !== 0 && (z = nA(m)), z;
						},
						array: (m) => {
							var z = YA(m.length);
							return gA(m, z), z;
						}
					};
					function G(m) {
						return Q === "string" ? DA(m) : Q === "boolean" ? !!m : m;
					}
					var F = IA(i), U = [], O = 0;
					if (s) for (var Z = 0; Z < s.length; Z++) {
						var lA = d[e[Z]];
						lA ? (O === 0 && (O = aA()), U[Z] = lA(s[Z])) : U[Z] = s[Z];
					}
					var yA = F.apply(null, U);
					function bA(m) {
						return O !== 0 && mA(O), G(m);
					}
					return yA = bA(yA), yA;
				}, NA = function(i, Q, e, s) {
					var Y = !e || e.every((d) => d === "number" || d === "boolean");
					return Q !== "string" && Y && !s ? IA(i) : function() {
						return hA(i, Q, e, arguments, s);
					};
				}, MA = { a: EA }, J = QA();
				J.c, A._kiss_fft_free = J.d, A._free = J.e, A._kiss_fft_alloc = J.f, A._malloc = J.g, A._kiss_fft = J.h, J.__errno_location;
				var aA = J.j, mA = J.k, YA = J.l;
				function HA(i) {
					try {
						for (var Q = atob(i), e = new Uint8Array(Q.length), s = 0; s < Q.length; ++s) e[s] = Q.charCodeAt(s);
						return e;
					} catch {
						throw new Error("Converting base64 string to bytes failed.");
					}
				}
				function uA(i) {
					if (AA(i)) return HA(i.slice(X.length));
				}
				A.ccall = hA, A.cwrap = NA;
				var sA;
				H = function i() {
					sA || fA(), sA || (H = i);
				};
				function fA() {
					if (S > 0 || (u(), S > 0)) return;
					function i() {
						sA || (sA = !0, A.calledRun = !0, !l && (x(), C(A), A.onRuntimeInitialized && A.onRuntimeInitialized(), P()));
					}
					A.setStatus ? (A.setStatus("Running..."), setTimeout(function() {
						setTimeout(function() {
							A.setStatus("");
						}, 1), i();
					}, 1)) : i();
				}
				if (A.preInit) for (typeof A.preInit == "function" && (A.preInit = [A.preInit]); A.preInit.length > 0;) A.preInit.pop()();
				return fA(), I;
			});
		})();
	})), SA, hI, zI, cI, _I, CB = tA((() => {
		BB(), SA = ZI({}), hI = SA.cwrap("kiss_fft_alloc", "number", [
			"number",
			"number",
			"number",
			"number"
		]), zI = SA.cwrap("kiss_fft", "void", [
			"number",
			"number",
			"number"
		]), cI = SA.cwrap("kiss_fft_free", "void", ["number"]), _I = class {
			constructor(g) {
				this.size = g, this.fcfg = hI(g, !1), this.icfg = hI(g, !0), this.inptr = SA._malloc(g * 8 + g * 8), this.cin = new Float32Array(SA.HEAPU8.buffer, this.inptr, g * 2);
			}
			fft = function(g) {
				const I = SA._malloc(this.size * 8), A = new Float32Array(SA.HEAPU8.buffer, I, this.size * 2);
				this.cin.set(g), zI(this.fcfg, this.inptr, I);
				let C = new Float32Array(this.size * 2);
				return C.set(A), SA._free(I), C;
			};
			dispose() {
				cI(this.fcfg), cI(this.icfg), SA._free(this.inptr);
			}
		};
	}));
	function gI(g) {
		this.size = g, this._csize = g << 1;
		for (var I = new Array(this.size * 2), A = 0; A < I.length; A += 2) {
			const t = Math.PI * A / this.size;
			I[A] = Math.cos(t), I[A + 1] = -Math.sin(t);
		}
		this.table = I;
		for (var C = 0, B = 1; this.size > B; B <<= 1) C++;
		this._width = C % 2 === 0 ? C - 1 : C, this._bitrev = new Array(1 << this._width);
		for (var E = 0; E < this._bitrev.length; E++) {
			this._bitrev[E] = 0;
			for (var r = 0; r < this._width; r += 2) {
				var o = this._width - r - 2;
				this._bitrev[E] |= (E >>> r & 3) << o;
			}
		}
		this._data = null;
	}
	var QB = tA((() => {
		gI.prototype.fft = function(I) {
			this._data = I, this._out = new Float32Array(2 * this.size);
			var A = this._csize, C = 1 << this._width, B = A / C << 1, E, r, o = this._bitrev;
			if (B === 4) for (E = 0, r = 0; E < A; E += B, r++) {
				const n = o[r];
				this._singleTransform2(E, n, C);
			}
			else for (E = 0, r = 0; E < A; E += B, r++) {
				const n = o[r];
				this._singleTransform4(E, n, C);
			}
			for (C >>= 2; C >= 2; C >>= 2) {
				B = A / C << 1;
				var t = B >>> 2;
				for (E = 0; E < A; E += B) for (var a = E + t, c = E, w = 0; c < a; c += 2, w += C) {
					const n = c, h = n + t, D = h + t, l = D + t, R = this._out[n], f = this._out[n + 1], N = this._out[h], M = this._out[h + 1], k = this._out[D], v = this._out[D + 1], u = this._out[l], x = this._out[l + 1], P = R, K = f, b = this.table[w], T = this.table[w + 1], S = N * b - M * T, p = N * T + M * b, H = this.table[2 * w], _ = this.table[2 * w + 1], $ = k * H - v * _, j = k * _ + v * H, X = this.table[3 * w], AA = this.table[3 * w + 1], q = u * X - x * AA, rA = u * AA + x * X, CA = P + $, QA = K + j, V = P - $, BA = K - j, EA = S + q, IA = p + rA, gA = S - q, iA = p - rA;
					this._out[n] = CA + EA, this._out[n + 1] = QA + IA, this._out[h] = V + iA, this._out[h + 1] = BA - gA, this._out[D] = CA - EA, this._out[D + 1] = QA - IA, this._out[l] = V - iA, this._out[l + 1] = BA + gA;
				}
			}
			return this._out;
		}, gI.prototype._singleTransform2 = function(I, A, C) {
			const B = this._data[A], E = this._data[A + 1], r = this._data[A + C], o = this._data[A + C + 1];
			this._out[I] = B + r, this._out[I + 1] = E + o, this._out[I + 2] = B - r, this._out[I + 3] = E - o;
		}, gI.prototype._singleTransform4 = function(I, A, C) {
			const B = C * 2, E = C * 3, r = this._data[A], o = this._data[A + 1], t = this._data[A + C], a = this._data[A + C + 1], c = this._data[A + B], w = this._data[A + B + 1], n = this._data[A + E], h = this._data[A + E + 1], D = r + c, l = o + w, R = r - c, f = o - w, N = t + n, M = a + h, k = t - n, v = a - h;
			this._out[I] = D + N, this._out[I + 1] = l + M, this._out[I + 2] = R + v, this._out[I + 3] = f - k, this._out[I + 4] = D - N, this._out[I + 5] = l - M, this._out[I + 6] = R - v, this._out[I + 7] = f + k;
		};
	})), EB = tg({ default: () => $I }), wI, $I, iB = tA((() => {
		kg(), Sg(), mg(), Jg(), pg(), Pg(), _g(), gB(), CB(), QB(), wI = [
			4,
			8,
			16,
			32,
			64,
			128,
			256,
			512,
			1024,
			2048,
			4096,
			8192,
			16384,
			32768,
			16384,
			32768,
			65536,
			131072
		], $I = class {
			constructor(g = 128, I = "indutnyJavascript", A = !0) {
				if (!wI.includes(g)) throw new Error("Size must be a power of 2 between 4 and 131072");
				this.size = g, this.outputArr = new Float32Array(2 * g), this.subLibrary = I, this.fftLibrary = void 0;
				const C = this.getCurrentProfile();
				C && A ? this.setSubLibrary(C.fastestSubLibrary) : this.setSubLibrary(I);
			}
			availableSubLibraries() {
				return [
					"kissWasm",
					"indutnyModifiedJavascript",
					"indutnyJavascript",
					"crossWasm",
					"mljsJavascript",
					"nockertJavascript",
					"nayuki3Wasm",
					"nayukiJavascript",
					"kissfftmodifiedWasm"
				];
			}
			availableSubLibrariesQuick() {
				return ["kissWasm", "indutnyModifiedJavascript"];
			}
			getCurrentProfile() {
				if (!(typeof localStorage > "u") && localStorage.getItem("webfftProfile")) return JSON.parse(localStorage.getItem("webfftProfile"));
			}
			setSubLibrary(g) {
				switch (g) {
					case "nayukiJavascript":
						this.fftLibrary = new pI(this.size);
						break;
					case "nayuki3Wasm":
						this.fftLibrary = new VI(this.size);
						break;
					case "kissWasm":
						this.fftLibrary = new JI(this.size);
						break;
					case "crossWasm":
						this.fftLibrary = new qI(this.size), this.size > 16384 && (this.fftLibrary = new sI(this.size));
						break;
					case "nockertJavascript":
						this.fftLibrary = new jI(this.size);
						break;
					case "indutnyJavascript":
						this.fftLibrary = new sI(this.size);
						break;
					case "mljsJavascript":
						this.fftLibrary = new OI(this.size);
						break;
					case "kissfftmodifiedWasm":
						this.fftLibrary = new _I(this.size);
						break;
					case "indutnyModifiedJavascript":
						this.fftLibrary = new gI(this.size);
						break;
					default: throw new Error("Invalid sublibrary");
				}
			}
			fft(g) {
				if (g.length !== 2 * this.size) throw new Error("Input array length must be == 2 * size");
				return this.outputArr = this.fftLibrary.fft(g), this.outputArr;
			}
			fftr(g) {
				var { outputArr: I, fftLibrary: A, size: C } = this;
				if (g.length !== C) throw new Error("Input array length must be == size");
				const B = new Float32Array(2 * C);
				B.fill(0);
				for (let E = 0; E < C; E++) B[2 * E] = g[E];
				return I = A.fft(B), I.slice(C, C * 2);
			}
			fft2d(g) {
				const I = g[0].length / 2, A = g.length;
				if (I !== this.size) throw new Error("Inner array length must be == 2 * size");
				if (!wI.includes(A)) throw new Error("Outter array length must be a power of 2 between 4 and 131072");
				let C = [];
				for (let r = 0; r < A; r++) this.outputArr = this.fft(g[r]), C.push(this.outputArr);
				this.dispose(), this.size = A, this.setSubLibrary(this.subLibrary);
				let B = [];
				for (let r = 0; r < I; r++) {
					const o = new Float32Array(2 * A);
					o.fill(0);
					for (let a = 0; a < A; a++) o[2 * a] = C[a][2 * r], o[2 * a + 1] = C[a][2 * r + 1];
					let t = new Float32Array(2 * A);
					t = this.fft(o), B.push(t);
				}
				let E = [];
				for (let r = 0; r < A; r++) {
					let o = new Float32Array(2 * I);
					for (let t = 0; t < I; t++) o[2 * t] = B[t][2 * r], o[2 * t + 1] = B[t][2 * r + 1];
					E.push(o);
				}
				return this.dispose(), this.size = I, this.setSubLibrary(this.subLibrary), E;
			}
			profile(g = 1, I = !0, A = !1) {
				if (!I && this.getCurrentProfile()) return this.getCurrentProfile();
				const C = performance.now();
				let B;
				A ? B = this.availableSubLibrariesQuick() : B = this.availableSubLibraries();
				let E = [];
				const r = g / B.length / 2;
				for (let c = 0; c < B.length; c++) {
					this.setSubLibrary(B[c]);
					const w = new Float32Array(2 * this.size);
					for (let D = 0; D < this.size; D++) w[2 * D] = Math.random() - .5, w[2 * D + 1] = Math.random() - .5;
					let n = performance.now();
					for (; (performance.now() - n) / 1e3 < r;) this.fft(w);
					n = performance.now();
					let h = 0;
					for (; (performance.now() - n) / 1e3 < r;) this.fft(w), h++;
					E.push(1e3 * h / (performance.now() - n)), this.dispose();
				}
				const o = (performance.now() - C) / 1e3;
				let t = E.indexOf(Math.max(...E));
				const a = {
					fftsPerSecond: E,
					subLibraries: B,
					totalElapsed: o,
					fastestSubLibrary: B[t]
				};
				return console.log("Setting sublibrary to", a.fastestSubLibrary), this.setSubLibrary(a.fastestSubLibrary), typeof localStorage < "u" && localStorage.setItem("webfftProfile", JSON.stringify(a)), a;
			}
			async checkBrowserCapabilities() {
				return await IB();
			}
			dispose() {
				this.fftLibrary && this.fftLibrary.dispose !== void 0 && this.fftLibrary.dispose();
			}
		};
	}));
	let lI = null, Ag = 0;
	async function rB(g) {
		try {
			const { default: I } = await Promise.resolve().then(() => (iB(), EB));
			lI = new I(g), await lI.profile(), Ag = g;
		} catch (I) {
			console.warn("[dspWorker] WebFFT not available, using Radix-2 fallback:", I), lI = null;
		}
	}
	let LA, KA, VA, jA, cA, wA, Ig, gg, Bg, Cg, UA, XA, ZA, BI, qA, CI, fI, QI, pA, TA, PA;
	const FI = 21;
	let Qg = FI, RI = 0, NI = [], yI = [], MI = [], GI = [], YI, kI, EI, iI;
	function Eg(g, I) {
		Qg = I, RI = 0, NI = Array.from({ length: I }, () => new Float32Array(g)), yI = Array.from({ length: I }, () => new Float32Array(g)), MI = Array.from({ length: I }, () => new Float32Array(g)), GI = Array.from({ length: I }, () => new Float32Array(g)), YI = new Float32Array(g), kI = new Float32Array(g), EI = new Float32Array(g), iI = new Float32Array(g);
	}
	function tB(g, I, A, C, B) {
		const E = RI;
		for (let r = 0; r < B; r++) {
			const o = g[r] * g[r] + I[r] * I[r], t = A[r] * A[r] + C[r] * C[r], a = g[r] * A[r] + I[r] * C[r], c = g[r] * C[r] - I[r] * A[r];
			YI[r] += o - NI[E][r], kI[r] += t - yI[E][r], EI[r] += a - MI[E][r], iI[r] += c - GI[E][r], NI[E][r] = o, yI[E][r] = t, MI[E][r] = a, GI[E][r] = c;
		}
		RI = (E + 1) % Qg;
	}
	function eB(g, I) {
		for (let A = 0; A < I; A++) {
			const C = EI[A] * EI[A] + iI[A] * iI[A], B = YI[A] * kI[A] + 1e-12;
			g[A] = Math.min(1, Math.max(0, Math.sqrt(C) / Math.sqrt(B)));
		}
	}
	let GA = 0, rI = 0, vA = null;
	const ig = new fg();
	let OA = null, tI = null, dI = "None", rg = 0, WA = null;
	function aB(g, I) {
		const A = g.length, C = (I % A + A) % A;
		if (C === 0) return;
		const B = new Float32Array(C);
		B.set(g.subarray(0, C)), g.copyWithin(0, C), g.set(B, A - C);
	}
	self.onmessage = (g) => {
		if (g.data && g.data.type === "run-dsp") try {
			const { measTimeDomain: I, refTimeDomain: A, BINS: C, FFT_SIZE: B, metrics: E, windowType: r, weightingType: o, averagingType: t, averagingDepth: a, averagingAlpha: c, averagingThresholdDb: w, enableSourceWindow: n, sourceWindowWidthMs: h, sourceWindowOffsetMs: D, sampleRate: l, compensationDelaySamples: R, autoDelayCompensation: f, inputGain: N, displayOffset: M, polarity: k, calibrationGain: v, inputFilter: u, besselSpeed: x, ppoSmoothing: P } = g.data, K = l || 48e3;
			if (!I || !A) return;
			B && B !== Ag && rB(B), (C !== GA || B !== rI) && (GA = C, rI = B, LA = new Float32Array(B), KA = new Float32Array(B), VA = new Float32Array(B), jA = new Float32Array(B), cA = new Float32Array(C), wA = new Float32Array(C), Ig = new Float32Array(B), gg = new Float32Array(B), Bg = new Float32Array(B), Cg = new Float32Array(B), UA = new Float32Array(C), XA = new Float32Array(C), ZA = new Float32Array(C), BI = new Float32Array(C), qA = new Float32Array(B), CI = new Float32Array(B), fI = new Float32Array(C), QI = new Float32Array(C), pA = new Float32Array(C), TA = new Float32Array(C), PA = new Float32Array(C), Eg(C, a || FI), vA = new cg(C, a || 16), WA = null), vA && vA.setDepth(a || 16);
			const b = new Set(E), T = new Float32Array(I), S = new Float32Array(A), p = vI(S), H = vI(T);
			if (R && R > 0 && aB(S, R), N && N !== 0) {
				const y = Math.pow(10, N / 20);
				for (let L = 0; L < B; L++) T[L] *= y;
			}
			if (k) for (let y = 0; y < B; y++) T[y] = -T[y];
			u && u !== "None" ? ((!OA || dI !== u || rg !== K) && (dI = u, rg = K, OA = mI(u, K), tI = mI(u, K)), OA && OA.process(T), tI && tI.process(S)) : OA && (OA = null, tI = null, dI = "None");
			const _ = r || "Hann";
			_ !== "Rectangular" && (ig.apply(T, _), ig.apply(S, _));
			let $ = 0, j = 0;
			for (let y = 0; y < B; y++) $ += T[y], j += S[y];
			$ /= B, j /= B;
			for (let y = 0; y < B; y++) T[y] -= $, S[y] -= j;
			if (HI(S, VA, jA), HI(T, LA, KA), b.has("Spectrum")) {
				for (let y = 0; y < C; y++) {
					const L = Math.sqrt(LA[y] * LA[y] + KA[y] * KA[y]);
					pA[y] = 20 * Math.log10(L / B * Math.SQRT2 + 1e-12);
				}
				if (M && M !== 0) for (let y = 0; y < C; y++) pA[y] += M;
			}
			const X = b.has("Magnitude") || b.has("Impulse") || b.has("Step"), AA = b.has("Phase") || b.has("Group Delay"), q = b.has("Impulse") || b.has("Step");
			if (X && ng(LA, KA, VA, jA, UA, cA, wA), t !== "None" && X) {
				if (t === "FIFO" && vA) {
					vA.processFIFO(cA, wA, TA, PA, w), cA.set(TA), wA.set(PA);
					for (let y = 0; y < C; y++) {
						const L = Math.sqrt(cA[y] * cA[y] + wA[y] * wA[y]);
						UA[y] = 20 * Math.log10(L + 1e-8);
					}
				} else if (t === "EMA" && vA) {
					vA.processLPF(cA, wA, TA, PA, c || .1), cA.set(TA), wA.set(PA);
					for (let y = 0; y < C; y++) {
						const L = Math.sqrt(cA[y] * cA[y] + wA[y] * wA[y]);
						UA[y] = 20 * Math.log10(L + 1e-8);
					}
				} else if (t === "LPF") try {
					WA || (WA = new Mg(C, x || "Medium")), WA.setFrequency(x || "Medium"), WA.process(cA, wA, TA, PA), cA.set(TA), wA.set(PA);
					for (let y = 0; y < C; y++) {
						const L = Math.sqrt(cA[y] * cA[y] + wA[y] * wA[y]);
						UA[y] = 20 * Math.log10(L + 1e-8);
					}
				} catch {}
			}
			if (M && M !== 0 && X) for (let y = 0; y < C; y++) UA[y] += M;
			if (v) {
				const y = new Float32Array(v);
				if (X) for (let L = 0; L < C; L++) UA[L] += y[L];
				if (b.has("Spectrum")) for (let L = 0; L < C; L++) pA[L] += y[L];
			}
			if (AA && sg(LA, KA, VA, jA, XA), tB(VA, jA, LA, KA, C), eB(ZA, C), q && (wg(LA, KA, VA, jA, qA, Ig, gg, Bg, Cg), n && lg(qA, h, D, K)), b.has("Step") && Dg(qA, CI, K), b.has("Group Delay")) {
				for (let y = 0; y < C; y++) fI[y] = XA[y] * Math.PI / 180;
				hg(fI, K / 2 / C, BI);
			}
			const rA = H.peakDb - H.rmsDb;
			QI.fill(Math.max(0, Math.min(30, rA)));
			let CA = 0;
			if (f && q) {
				let y = 0;
				for (let L = 0; L < qA.length; L++) {
					const DA = Math.abs(qA[L]);
					DA > y && (y = DA, CA = L);
				}
			}
			P && P > 0 && (X && aI(UA, C, K, P), AA && Gg(XA, C, K, P), aI(ZA, C, K, P), b.has("Spectrum") && aI(pA, C, K, P));
			const QA = UA.buffer, V = XA.buffer, BA = ZA.buffer, EA = BI.buffer, IA = qA.buffer, gA = CI.buffer, iA = QI.buffer, oA = cA.buffer, eA = wA.buffer, nA = pA.buffer;
			self.postMessage({
				type: "dsp-results",
				outputMagnitude: QA,
				outputPhase: V,
				outputCoherence: BA,
				outputGroupDelay: EA,
				outputImpulse: IA,
				outputStep: gA,
				outputCrestFactor: iA,
				outputSpectrum: nA,
				hReal: oA,
				hImag: eA,
				refPeakDb: p.peakDb,
				refRmsDb: p.rmsDb,
				measPeakDb: H.peakDb,
				measRmsDb: H.rmsDb,
				detectedDelaySamples: CA
			}, [
				QA,
				V,
				BA,
				EA,
				IA,
				gA,
				iA,
				nA,
				oA,
				eA
			]), UA = new Float32Array(GA), XA = new Float32Array(GA), ZA = new Float32Array(GA), BI = new Float32Array(GA), qA = new Float32Array(rI), CI = new Float32Array(rI), QI = new Float32Array(GA), pA = new Float32Array(GA), cA = new Float32Array(GA), wA = new Float32Array(GA);
		} catch (I) {
			console.error("[dspWorker] Error in run-dsp:", I);
		}
		g.data && g.data.type === "reset-averaging" && (GA > 0 && Eg(GA, FI), vA && vA.reset(), WA && WA.reset());
	};
})();
